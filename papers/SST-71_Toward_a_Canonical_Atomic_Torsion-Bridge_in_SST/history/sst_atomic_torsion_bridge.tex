...
\large A Minimal Field--Source--Atom Model Extracted from Earlier Drafts}
...
Define a scalar or pseudoscalar torsion-driving amplitude by
\begin{equation}
\label{eq:ThetaDef}
\Theta(\mathbf{r},t) \equiv \beta_\omega\,\omega_z(\mathbf{r},t),
\end{equation}
or equivalently, via~\eqref{eq:wzxi},
\begin{equation}
\label{eq:ThetaXi}
\Theta(\mathbf{r},t)=2\beta_\omega\Omega\,\partial_z\xi(\mathbf{r},t).
\end{equation}
...
The two branch sectors are introduced as
\begin{equation}
\label{eq:StBranches}
S_t^{(\sigma)}(\mathbf r,t)=S_{t,0}(r)+\sigma\,\chi\,\Theta(\mathbf r,t),
\qquad
\sigma=\pm 1,
\end{equation}
where $\sigma=+1$ corresponds to $\SwirlClock$ and $\sigma=-1$ to $\SwirlClockcw$.

The minimal clock potential is then taken to be
\begin{equation}
\label{eq:Vclock}
\boxed{
V_{\rm clock}^{(\sigma)}
=
\lambda_0\Bigl[1-\bigl(S_t^{(\sigma)}\bigr)^2\Bigr]
+\lambda_1\,\abs{\nabla S_t^{(\sigma)}}^2.
}
\end{equation}
This form is chosen because it is scalar, branch-sensitive, and consistent with SST's existing use of $1-S_t^2$ as the natural departure measure from the unshifted clock state.
...
\approx
-2\sigma\lambda_0 S_{t,0}\chi\Theta
+2\sigma\lambda_1\chi\,\nabla S_{t,0}\cdot \nabla\Theta,
\end{equation}
so a torsion packet induces opposite linear shifts in the two branches.
...
\end{equation}
Thus the atomic bridge model retains the tested size law while reinterpreting $a_0$ as a medium-derived coherence scale.
...
\begin{enumerate}[label=(P\arabic*)]
    ...
    \item no hidden retuning: the couplings $\chi$, $\lambda_0$, and $\lambda_1$ must ultimately be fixed by canon rather than re-fit per atom.
\end{enumerate}
...
\section{Status}
This article does not claim a first-principles derivation of fermionic exchange, nor a canonical derivation of the couplings $\chi$, $\lambda_0$, or $\lambda_1$. It is therefore not yet a closed atomic theory. What it does provide is a sharply reduced and internally consistent starter model:
\begin{equation}
\boxed{
