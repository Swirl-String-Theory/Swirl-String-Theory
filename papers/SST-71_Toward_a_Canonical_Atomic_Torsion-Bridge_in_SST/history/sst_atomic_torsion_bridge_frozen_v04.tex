...
\end{equation}
This choice is not yet canonically derived from a deeper action, but it has three advantages: it makes $\Theta$ dimensionless, it ties the atomic driving amplitude to the retained torsion field rather than inserting it by hand, and it identifies the same helical packet profile with both a field configuration and an atomic perturbation source. For the localized packet profiles retained in the present paper one imposes the asymptotic condition
\begin{equation}
\label{eq:ThetaAsymptotic}
\Theta(\mathbf r,t)\to 0 \qquad (\lVert \mathbf r\rVert\to\infty),
\end{equation}
which implies $S_t^{(\sigma)}\to 1$ and hence $V_{{\rm clock},0}^{(\sigma)}(r)\to 0$ for both branches. Equation~\eqref{eq:continuum-threshold} should therefore be read as contingent on the explicit asymptotic condition~\eqref{eq:ThetaAsymptotic}, not as an independent postulate.
...
\end{equation}
This is the smallest available SST length and therefore the most conservative nonzero freeze of the branch source strength; it removes the last free prefactor in the sourced field equation without importing a new fitted scale. With this choice the right-hand side of Eq.~\eqref{eq:Awave-source} has the same dimensions as the left-hand side because $\mathbf J_{\rm br}$ carries dimensions of inverse area-times-velocity and $g_J$ contributes the required single power of length. The corresponding branch current is
\begin{equation}
\label{eq:Jbr}
\boxed{
\mathbf J_{\rm br}
...
\end{equation}
where $h=\pm 1$ labels the handedness of the incoming torsion packet. For the helical packet ansatz~\eqref{eq:ThetaPacket} at fixed propagation direction, the handedness is identified with the sign of the azimuthal winding number,
\begin{equation}
\label{eq:h-sign-m}
\boxed{h\equiv \mathrm{sgn}(m).}
\end{equation}
Combined with the orbital rule $\Delta m_\ell=m$ from Eq.~\eqref{eq:dm}, the starter model therefore yields a branch-preserving same-handed overlap preference at linear order. In the present draft this should be read as a working overlap statement rather than a derived theorem. The corresponding selection rule is
\begin{equation}
\label{eq:handed-selection}
 h=+1:\ \sigma_i=\sigma_f=+1\ \text{dominant},
 \qquad
 h=-1:\ \sigma_i=\sigma_f=-1\ \text{dominant},
\end{equation}
while opposite-handed channels are suppressed by reduced helical overlap and branch-flip channels are absent at linear order. If $\mathcal A(\omega)=0$ identically once the couplings are frozen, then the present branch interpretation adds no atomic content beyond standard spin bookkeeping. If instead $\mathcal A(\omega)\neq 0$ with a definite symmetry and scale, then the theory acquires a genuine SST-specific test.
...
\begin{enumerate}[label=(P\arabic*)]
    ...
    \item frozen starter couplings: in the present v0.1 freeze, $\Theta$, $\eta_0$, $g_J$, and $V_{\rm clock}^{(\sigma)}$ are given explicitly by Eqs.~\eqref{eq:ThetaDef}, \eqref{eq:StBranches}, \eqref{eq:gJfreeze}, and~\eqref{eq:Vclock}, with no atom-by-atom retuning.
\end{enumerate}
