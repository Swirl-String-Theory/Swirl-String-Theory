...
\large v0.5 Circulation-Foundational Extension from the SST Canons}
...
\end{equation}
This means the natural canon-first hierarchy is circulation $\to$ clock, not clock $\to$ circulation. In the same canons, delay-selected circulation modes precede knot selection, and Kelvin protection of circulation is treated as foundational rather than optional \cite{IskandaraniCanon078,IskandaraniCanon060}. The present v0.5 extension therefore uses $\Gamma_0$ as the primary normalization of the atomic driver rather than the earlier purely scalar clock shift.
...
The new v0.5 choice is to normalize the source by local circulation, not by local clock shift. Using Eq.~\eqref{eq:utheta}, define the local loop circulation by
\begin{equation}
\label{eq:GammaLoc}
\boxed{
\Gamma_{\rm loc}(r,z,t)
...
\end{equation}
may still be used as an auxiliary bookkeeping field. In v0.5, however, it is no longer the primary canonical source variable.
...
The genuinely circulation-foundational addition in v0.5 is an orbit-coupled term. To avoid introducing a new fitted constant, the same small canonical factor $\eta_0$ is reused and the coupling is frozen at the same atomic energy scale. The minimal Hermitian choice is the anticommutator form
\begin{equation}
\label{eq:HintCirc}
\boxed{
\delta H_{\rm circ}^{(\sigma)}
=
E_R\eta_0\,\sigma\,\frac{1}{2\hbar}
\left\{\hat L_z,\gamma\right\}.
}
\end{equation}
The anticommutator is important: for an initial $s$-state with $\hat L_z\psi_{1s}=0$, the operator still yields a nonzero contribution because $\hat L_z$ can act on the helical packet factor inside $\gamma$.
...
\end{equation}
This is the main physics upgrade from v0.4 to v0.5: the atomic sector now responds directly to handed circulation, not only to a scalar branch shift.
...
\Delta m_\ell = m.
\end{equation}
The new feature is that the anticommutator term in Eq.~\eqref{eq:HintCirc} is explicitly odd under $m\mapsto -m$. For a helical packet with $\gamma\propto e^{im\phi}$,
\begin{equation}
\label{eq:Lzgamma}
\hat L_z\,\gamma = m\hbar\,\gamma + \text{(envelope terms)},
\end{equation}
...
The next required step is to close the dynamical link between the propagating torsion field and the circulation driver $\gamma$. A minimal v0.5 closure is to extract vorticity from the field velocity $\mathbf u=\partial_t\mathbf A$ and then build the loop circulation from that vorticity. Define
\begin{equation}
\label{eq:omegaA}
...
\end{equation}
This is the key v0.5 replacement for the older scalar-insertion strategy. The field now drives the atom through canon-normalized circulation.
...
internal technical reference, 2026.

\bibitem{IskandaraniCanon060}
O. Iskandarani,
\textit{Swirl-String Theory Canon v0.6.0},
internal technical reference, 2025.
