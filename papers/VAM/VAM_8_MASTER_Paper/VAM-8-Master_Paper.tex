...
\section{Swirl Kinematics and Classical Gravitation}\label{sec:swirl-kinematics-and-classical-gravitation}
    ...
    \begin{equation}
        E_{\text{swirl}} = \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} \int_{\mathcal{V}} |\vec{\omega}|^2 \, d^3x
...
\section{Particles as Vortex Knots — Mass and Topology}
    ...
    \begin{equation}
    E_{\text{knot}} = \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2 \cdot V_{\text{knot}}
    \label{eq:raw_swirl_energy}
    \end{equation}

    Here \( \rho_{\scriptscriptstyle\boldsymbol\!m} \) is the mass-equivalent æther density~(see Appendix~\ref{sec:constants_derivation}), \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) is the characteristic swirl speed, and \( V_{\text{knot}} = V_i \cdot V_{\text{torus}} \) is the effective volume of the knotted structure.
    ...
    \begin{equation}
    M = \left( \frac{4}{\alpha \varphi} \right) \cdot \xi(n) \cdot \left( \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2 \sum_i V_i \right)
    ...
    \begin{equation}
    M_{u,d} = \frac{4}{\alpha \varphi} \cdot \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2 \cdot V_{u,d}
    \end{equation}
    ...
    \begin{equation}
    M(n, m, \{V_i\}) = \left( \frac{4}{\alpha} \right) \cdot \left( \frac{1}{m} \right)^{3/2} \cdot \frac{1}{\varphi^s} \cdot n^{-1/\varphi} \cdot \sum_i V_i \cdot \left( \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2 \right)
...
\section{Standard Model from Swirl Symmetry}
    ...
    Crucially, these dynamics are captured by a unified Lagrangian formulation (see Eq.~\eqref{eq:VAM_lagrangian}), in which gravitational curvature and electromagnetic helicity both emerge from geometric properties of the swirl field \( \vec{v} \) and its vorticity \( \vec{\omega} \). This demonstrates that the Standard Model gauge interactions, gravity, and inertia share a common geometric origin within VAM's topological fluid substrate.
      ...
      \subsection*{Unified Lagrangian Including Gravity and Electromagnetism}
        ...
        \begin{equation}
        \mathcal{L}_{\text{VAM}} = \tfrac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} |\vec{v}|^2 - \tfrac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} \lambda_g |\vec{\omega}|^2 + \tfrac{\alpha_e}{2} (\vec{v} \cdot \vec{\omega})^2 - V(\vec{\omega})
...
\section{Quantized Swirl Field Theory in the Vortex \AE{}ther Model}\label{sec:quantized_swirl_field_theory}
    ...
    \begin{equation}
    E_n = \hbar_{\text{VAM}} \omega_n, \quad \text{with} \quad \hbar_{\text{VAM}} = \rho_{\scriptscriptstyle\boldsymbol\!m} \Gamma_n r_c^2
    \end{equation}
    ...
    \begin{align}
    \hat{H} &= \frac{1}{2} \int \rho_{\scriptscriptstyle\boldsymbol\!m} |\vec{\omega}|^2 d^3x \quad \text{(Swirl energy)} \\
    ...
    \[
    \rho_{\scriptscriptstyle\boldsymbol\!m} \frac{d\vec{v}}{dt} = \rho_{\scriptscriptstyle\boldsymbol\!m} \lambda_g \nabla \times \vec{\omega} + \alpha_e (\vec{v} \cdot \vec{\omega}) \vec{\omega} - \nabla V(\vec{\omega})
        \label{eq:euler_lagrange_swirl}
    \]

    Each term corresponds to gravitational curvature~\footnote{Here, \textbf{\boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!m} \)} refers to the æther’s inertial (mass-equivalent) density, consistent with the swirl-core confinement scale defined in Appendix~\ref{sec:constants_derivation}.
...
\section{Benchmarking VAM Against General Relativity}
    ...
    \begin{equation}
    \frac{d\tau}{dt} = 1 - \frac{\alpha \, (\vec{v} \cdot \vec{\omega})}{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \, \omega_0}
    \label{eq:vam_time_dilation}
    \end{equation}

    where \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) is the maximum swirl speed, and \( \omega_0 \) is the internal rotation rate of an untensioned vortex. Time flows more slowly in regions with higher helicity, reproducing gravitational redshift effects from GR~\cite{volovik2003universe}.
    ...
    \[
        \frac{d\tau}{dt} \rightarrow 0 \quad \Rightarrow \quad \vec{v} \cdot \vec{\omega} \rightarrow \frac{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \, \omega_0}{\alpha}
    \]
    ...
    When the internal swirl speed of a knot approaches \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \), the æther compresses beyond recoverability. The maximum pressure in the core is:

    \begin{equation}
        P_{\text{max}} = \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2
    \end{equation}
...
\section{Experimental Proposals and Observational Tests}
    ...
    \begin{equation}
        \frac{d\tau}{dt} = 1 - \frac{\alpha \mathcal{H}}{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \omega_0}
    \end{equation}
    ...
    \begin{equation}
        \theta_r^{\text{VAM}} \sim \frac{1}{2} \cdot \frac{\nabla P_{\text{swirl}}}{\rho_{\scriptscriptstyle\boldsymbol\!m} v^2}
    \end{equation}
    ...
    \[
        \Delta n_{\text{VAM}} \sim \frac{\mathcal{H}}{\rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2}
    \]
...
%========================================================================================================%
    \appendix
        \section{Derivation of Fundamental Constants from Vortex Geometry}\label{sec:constants_derivation}
            ...
            The dimensionless fine-structure constant in VAM is derived geometrically. The swirl speed \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) of the æther sets the intrinsic velocity scale, and is related to the speed of light via:
            ...
            VAM reinterprets \( \hbar \) as angular momentum due to coherent vortex motion. For a knot with core radius \( r_c \), swirl velocity \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \), and electron mass \( m_e \), we obtain:
        
            \begin{equation}
            \hbar = m_e \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} r_c
            \label{eq:planck_vam}
            \end{equation}
            ...
            \begin{table}[H]
                ...
                \begin{tabular}{|l|l|l|}
                    ...
                    Planck \( \hbar \) & \( m_e \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} r_c \) & Angular momentum from swirl \\
                    Gravitational \( G \) & \( \frac{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} c^3 t_p^2}{r_c m_e} \) & Strain interaction via vortex fields \\
            ...
            \subsection{Deriving the Æther Fluid Density from First Principles \boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!f} \)}
                \label{sec:aether_fluid_density}
        
                We derive the base fluid density of the æther, \textbf{\boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!f} \)}, within the Vortex \AE{}ther Model (VAM) using only measurable physical constants. This provides the foundational parameter from which both the inertial mass density \textbf{\boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!m} \)} and energy density \textbf{\boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!E} \)} are defined.
                ...
                \[
                \rho_{\scriptscriptstyle\boldsymbol\!f} = \frac{2 m_e c^2}{\left( \alpha \cdot \frac{m_e c^2}{\hbar} \right)^2 \cdot \left( \frac{r_e^3}{3} \right)} \approx 7 \times 10^{-7} \, \text{kg/m}^3
                \]
                ...
                \begin{itemize}
                    \item \textbf{Mass density \boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!m} \):} Effective inertial density in confined vortex cores. Used in:
                    \[
                    m_e = \frac{8\pi \rho_{\text{\ae}}^{\text{mass}} r_c^3}{C_e} \cdot \sqrt{p^2 + q^2 + \gamma pq}
                    \]
                    \item \textbf{Energy density \boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!E} \):} Stored swirl energy under maximum confinement. Appears in:
                    \[
                    \epsilon_{\text{swirl}} = \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!E} C_e^2
                    \]
                \end{itemize}
                ...
                \subsection{Derivation of the Mass-Equivalent Æther Density \boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!m} \)}
                ...
                m_e = \rho_{\scriptscriptstyle\boldsymbol\!m} \cdot V_{\text{core}}
                \]
        
                Assuming a spherical vortex core of radius \( r_c \sim \alpha r_e \), we write:
        
                \[
                \rho_{\scriptscriptstyle\boldsymbol\!m} = \frac{m_e}{\frac{4}{3} \pi r_c^3}
                = \frac{m_e}{\frac{4}{3} \pi (\alpha r_e)^3}
                \approx 10^{18} \, \text{kg/m}^3
                \]
        
                \paragraph{Interpretation:}
                This density corresponds to the compressed, localized æther needed to form stable topological excitations (e.g., electrons, quarks). It is orders of magnitude higher than the background fluid density \( \rho_{\scriptscriptstyle\boldsymbol\!f} \), consistent with the sharp confinement and gravitational potential expected at particle cores.
        
                \subsection{Derivation of the Rotational Energy Density \boldmath\( \rho_{\scriptscriptstyle\boldsymbol\!E} \)}
        
                The energy density stored in the most intense vortex configurations arises from maximal swirl flow at the æther sound limit \( v = \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \). The kinetic energy density is given by:
        
                \[
                    \rho_{\scriptscriptstyle\boldsymbol\!E} = \frac{2 E_{\text{max}}}{C_e^2}
                    \quad \text{with} \quad
                ...
                \[
                \rho_{\scriptscriptstyle\boldsymbol\!E} = \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!m} C_e^2
                \]
        
                Using \( \rho_{\scriptscriptstyle\boldsymbol\!m} \approx 10^{18} \, \text{kg/m}^3 \) and \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \sim 10^8 \, \text{m/s} \), we estimate:
        
                \[
                \rho_{\scriptscriptstyle\boldsymbol\!E} \sim 5 \times 10^{34} \, \text{J/m}^3
                \]
                ...
                Throughout the Vortex \AE{}ther Model, different effective æther densities are used based on physical context. The most commonly applied value is the \textbf{mass-equivalent æther density} \( \rho_{\scriptscriptstyle\boldsymbol\!m} \), as it governs inertial response and is central to the mass–helicity relationship.
                ...
                \begin{table}[H]
                    ...
                    \begin{tabular}{|l|c|p{7.5cm}|}
                        ...
                        \hline
                        \( \rho_{\scriptscriptstyle\boldsymbol\!f} \) & \( \sim 7 \times 10^{-7} \, \text{kg/m}^3 \) & Global æther background; derived from Compton-scale vorticity and classical radius. Useful in cosmology and BEC analogs. \\
                        \hline
                        \( \rho_{\scriptscriptstyle\boldsymbol\!m} \) & \( \sim 10^{18} \, \text{kg/m}^3 \) & Most used in mass and helicity formulas. Represents the confined core density of topological particles like electrons. \\
                        \hline
                        \( \rho_{\scriptscriptstyle\boldsymbol\!E} \) & \( \sim 10^{35} \, \text{J/m}^3 \) & Maximal swirl energy density. Appears in gravitational field thresholds and black-hole-like configurations. \\
                        \hline
                    \end{tabular}
...
%========================================================================================================%
       \section{Vortex-Scale Gravitational Coupling and Ætheric Planck Units}
                ...
                Using the core radius \( r_c \), core swirl speed \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \), and force limit \( F_{\text{\ae}}^{\text{max}} \), we define a self-consistent system of physical units native to the ætheric model—analogous to the Planck system, but grounded in fluid dynamics.
                ...
                \end{equation}
                Using \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} = 1.09384563 \times 10^6~\text{m/s} \),
                \begin{equation}
...
%========================================================================================================%
        \section{Experimental Protocols and Clock Shift Predictions}\label{sec:experimental_protocols}
            ...
            \begin{equation}
            \frac{d\tau}{dt} = 1 - \frac{\alpha}{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \omega_0} \vec{v} \cdot \vec{\omega}
            ...
            \begin{equation}
                \boxed{
                    \theta_r^{\text{VAM}} \approx \frac{1}{2} \cdot \frac{\nabla P_{\text{swirl}}}{\rho_{\scriptscriptstyle\boldsymbol\!f} C_e^2}
                }
            ...
            \begin{equation}
                \Delta \phi = \int \frac{\alpha \mathcal{H}}{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \omega_0} dt
            \end{equation}
...
%========================================================================================================%
        \section{Glossary of Temporal Ontology and Ætheric Constructs}\label{sec:glossary}
            ...
            \begin{description}
                \item[$\rho_{\scriptscriptstyle\boldsymbol\!m}$:] Inertial (mass-equivalent) density of the æther; governs kinetic energy, wave propagation, and time dilation effects.
                \item[$\rho_{\scriptscriptstyle\boldsymbol\!f}$:] Compressibility-related density; appears in pressure gradient and Bernoulli terms.
...
%========================================================================================================%
        \section{Scaling Hierarchy of Observable Forces in Vortex \AE ther}\label{sec:force_scaling}
            ...
            \end{equation}
            With typical values \( \rho_{\text{\ae}}^{(\text{energy})} \sim 10^{35}~\text{J/m}^3 \) and \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \sim 10^8~\text{m/s} \), we obtain:
            \begin{equation}
...
%========================================================================================================%
        \section{Entropic Gravity from Swirl Fields}\label{sec:verlinde_mapping}
            ...
            \end{equation}
            and reinterpret all terms within the framework of vortex-based æther dynamics. The goal is to ground the entropic force in the structured vorticity fields of the Vortex \AE ther Model (VAM), using fundamental fluid variables such as local angular velocity $\Omega$, æther density $\rho_{\scriptscriptstyle\boldsymbol\!f}$\footnote{Throughout this appendix, \(\rho_{\scriptscriptstyle\boldsymbol\!f}\) denotes the compressible æther’s inertial (fluidic) density relevant to wave propagation and pressure gradients, while \(\rho_{\scriptscriptstyle\boldsymbol\!m}\) denotes the effective confined mass density of swirl knots. See Appendix~\ref{sec:constants_derivation} for derivation and values.}, and swirl-clock phase memory $S(t)$.
            ...
            \begin{equation}
                T_\text{eff} = \frac{1}{2k_B} \rho_{\scriptscriptstyle\boldsymbol\!f} \Omega^2 r^2.
            \end{equation}
            ...
            \begin{align}
                F &= T_\text{eff} \cdot \frac{\Delta S}{\Delta x} \\
                  &= \left( \frac{1}{2k_B} \rho_{\scriptscriptstyle\boldsymbol\!f} \Omega^2 r^2 \right) \cdot \left( \frac{d\Omega}{dx} \cdot \Delta t \right) \\
                  &\sim \frac{\rho_{\scriptscriptstyle\boldsymbol\!f}}{2k_B} \Omega^2 r^2 \cdot \frac{d\Omega}{dx} \cdot \Delta t.
            \end{align}

            \subsection*{Comparison with Pressure Gradient Force}
            VAM models forces as arising from Bernoulli-type swirl pressure:
            \begin{equation}
                F_\text{vortex} = -\nabla P = -\frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} \nabla |\vec{\omega}|^2.
            \end{equation}
            Letting $\vec{\omega} = \Omega(r) \hat{\theta}$, we compute:
            \begin{align}
                F &\sim -\frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} \nabla (\Omega^2 r^2) \\
                  &= -\rho_{\scriptscriptstyle\boldsymbol\!f} \left( \Omega \frac{d\Omega}{dr} r^2 + \Omega^2 r \right).
            \end{align}
            ...
            \begin{equation}
                F = \left( \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} \Omega^2 r^2 \right) \cdot \left( \frac{d\Omega}{dx} \cdot \Delta t \right)
            \end{equation}
            or, in full gradient form:
            \begin{equation}
                F = -\rho_{\scriptscriptstyle\boldsymbol\!f} \left( \Omega \frac{d\Omega}{dr} r^2 + \Omega^2 r \right).
            \end{equation}
            ...
            where:
            \begin{itemize}
                \item $E_\text{vortex} = \int_{A_v} \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} \Omega^2 \, dA$
            ...
            \begin{equation}
            E_\text{vortex} = \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} \int_0^{r_c} \Omega^2(r) \cdot 2\pi r \, dr
            \end{equation}
            ...
            \begin{equation}
            \boxed{
            \mathcal{S}(t) = \frac{\rho_{\scriptscriptstyle\boldsymbol\!f} \pi}{T} \cdot \frac{\int_0^{r_c} \Omega^2(r) r \, dr}{\int_0^t \Omega(t') dt'}
            }
            \end{equation}
            ...
            \begin{table}[H]
                ...
                \begin{tabular}{|c|p{5.0cm}|p{7cm}|}
                    ...
                    \hline
                    Temperature & $T = \frac{\hbar a}{2\pi c k_B}$ (Unruh) & $T_\text{eff} = \frac{1}{2k_B} \rho_{\scriptscriptstyle\boldsymbol\!f} \Omega^2 r^2$ \\
                    \hline
            ...
            \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} (\partial_t S)^2
            - \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} C_e^2 (\partial_x S)^2
            + \Lambda \, \{ S(t), t \}
            \right]
            }
            \end{equation}
            ...
            \begin{equation}
            \rho_{\scriptscriptstyle\boldsymbol\!f} (\partial_t^2 S - C_e^2 \partial_x^2 S) = - \Lambda \, \frac{d}{dt} \left[ \frac{S'''(t)}{S'(t)} - \frac{3}{2} \left( \frac{S''(t)}{S'(t)} \right)^2 \right]
            \end{equation}
            ...
            \begin{equation}
            G_\text{swirl} = \frac{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} c^5 t_p^2}{2 F_\text{max} r_c^2} \cdot f(R),
            \end{equation}
            ...
            \[
                c^2 \propto \frac{\partial P}{\partial \rho_{\scriptscriptstyle\boldsymbol\!f}} \sim \frac{F_\text{max}}{\rho_{\scriptscriptstyle\boldsymbol\!f}}
            \]

            In regions of concentrated swirl confinement ($\rho_{\scriptscriptstyle\boldsymbol\!m} \sim 10^{18}$~kg/m$^3$), the effective wave speed of light is reduced due to increased inertial loading of the æther medium; the local effective speed of light satisfies $c_\text{local} \ll c_\infty$.
            ...
            \[
                P(r) = P_\infty - \frac{1}{2} \rho_{\scriptscriptstyle\boldsymbol\!f} \omega^2(r) = 0
            \]
            This leads to a characteristic collapse radius:
            \[
                R_\text{vam} = \left( \frac{2 F_{\text{max}}}{\rho_{\scriptscriptstyle\boldsymbol\!f} \omega_0^2} \right)^{1/2}
            \]

            \subsection*{Incompressible Swirl Equilibrium}
            Stable spherical vortex structures satisfy a pressure equilibrium condition:
            \[
                \nabla P = \rho_{\scriptscriptstyle\boldsymbol\!f} \Omega^2(r) r = \frac{G M(r)}{r^2} \rho_{\scriptscriptstyle\boldsymbol\!f}
            \]
