...
\begin{document}
    ...
    \end{equation}
    such that \( \tau = 1 \) in regions of zero swirl, and \( \tau \to 0 \) as swirl approaches the limiting velocity \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \).
    ...
    Let \( \vec{v}(\vec{x}, t) \) be the local swirl velocity field of the æther, and \( \vec{\omega} = \nabla \times \vec{v} \) its associated vorticity. The æther has a fixed mass-equivalent density \( \rho_\text{\ae} \), and the limiting tangential swirl speed is \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \).
