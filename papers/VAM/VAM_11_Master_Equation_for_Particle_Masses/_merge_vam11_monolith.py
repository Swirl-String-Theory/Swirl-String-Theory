# Merge VAM-11 skeleton + sections/main_*.tex + thebibliography from ../references.bib
from __future__ import annotations

import re
from pathlib import Path

ROOT = Path(r"c:/workspace/projects/SwirlStringTheory/papers/VAM/VAM_11_Master_Equation_for_Particle_Masses")
MAIN_SKELETON = ROOT / "VAM-11-Master_Equation_for_Particle_Masses_skeleton.tex"
MAIN_OUT = ROOT / "VAM-11-Master_Equation_for_Particle_Masses.tex"
BIB = Path(r"c:/workspace/projects/SwirlStringTheory/papers/VAM/references.bib")

SECTION_FILES = ["sections/main_1", "sections/main_2"]
...
def field_to_latex_title(s: str) -> str:
    """Keep braces and backslashes (math, \\mathbb{C}, etc.)."""
    return clean_bib_tex(s)
...
def main() -> None:
    ...
    m_first = re.search(r"\\input\{sections/main_1\}", main_txt)
    m_last = re.search(r"\\input\{sections/main_2\}", main_txt)
    m_bib = re.search(
        r"\s*\\bibliographystyle\{[^}]+\}\s*\n\s*\\bibliography\{[^}]+\}",
        main_txt,
    )
    if not m_first or not m_last or not m_bib:
        raise SystemExit("Could not parse VAM-11 skeleton (inputs / bibliography).")
    ...
    cite_src = prefix + merged_sec + mid
    bib = parse_bib(BIB)
