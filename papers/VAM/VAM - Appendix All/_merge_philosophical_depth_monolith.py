# Merge VAM Philosophical Depth skeleton: inline \input sections + thebibliography from ../references.bib
...
MAIN_SKELETON = ROOT / "VAM-Philosophical_Depth_skeleton.tex"
MAIN_OUT = ROOT / "VAM-Philosophical_Depth.tex"
...
def sanitize_merged_body(s: str) -> str:
    ...
    s = s.replace(r"\rho_{\æ}", r"\rho_{\text{\ae}}")
    s = s.replace(r"\mathrm{Æ}", r"\mathrm{\ae}")
    s = s.replace(r"\frac{\Gamma}\text{(vortex", r"\frac{\Gamma}{\text{(vortex")
