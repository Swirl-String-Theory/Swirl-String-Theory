...
% -------------------------------------------------
    ...
    \[
        ds^2 = C_e^2 dT_v^2 - dr^2,
    \]
    analogous to the Minkowski line element, with \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) modulated by local fluid energy density. See ~\ref{sec:interpretation_c_e} for details on \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \).
...
\appendix
    \section*{Appendix A: Interpretation of \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) in the Swirl Metric}\label{sec:interpretation_c_e}
    \addcontentsline{toc}{section}{Interpretation of \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) in the Swirl Metric}
    ...
    \[
        ds^2 = c^2 dt^2 - dx^2,
    \]
    where \( c \) determines the causal boundary between time-like and light-like intervals, the VAM formulation uses \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) to establish a similar structure for vortex-based causality:
    ...
    \[
        \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \cdot dT_v = (1.0938 \times 10^6)\cdot(10^{-12}) = 1.0938 \times 10^{-6} \, \text{m}.
    \]
    ...
    \begin{center}
        \begin{tabular}{|c|c|c|}
            ...
            \hline
            Speed constant & \( c \) (light speed) & \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) (vortex core velocity) \\
            \hline
    ...
    The swirl interval metric, with \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) as a scaling factor, encodes internal dynamics of matter structures in the æther. While \( c \) governs relativistic signaling, \( \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} \) governs \textbf{vortex coherence}, suggesting a dual structure of causality: one external and radiative, the other internal and rotational.
