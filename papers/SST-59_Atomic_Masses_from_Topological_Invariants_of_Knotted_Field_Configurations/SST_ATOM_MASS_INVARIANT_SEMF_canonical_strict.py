...
Modes
-----
- canonical_only: strict Canon evaluation with a single electron calibration (fixes L_tot(e)).
  Uses fixed (s_u, s_d) from hyperbolic-volume assignments; no baryon-sector fitting or rescaling.
- Optional diagnostics: phi sweeps (and twist-knot k proxies) may be run to explore topology sensitivity.
...
def get_particle_topologies(cfg: Config, phi_val: float = phi0) -> Dict:
    # Lepton Generation Calibration (canonical-only)
    # Calibrate ONLY on the electron. Muon/tau are predicted by topology inputs,
    # not by back-solving L_tot from their measured masses.
    electron_sigma = 1
...
def main() -> None:
    ...
    print(f"kappa_R = {diag['kappa_R']:.6g}")
    ...
    out_csv = "SST_Invariant_Mass_Results_canonical.csv"
