...
This paper (Part~III) provides a reproducible benchmark study of a topology-indexed mass functional across a curated dataset of nuclei/atoms/molecules. It separates (i) constituent mass generation via a dimensionless topological kernel from (ii) binding-energy phenomenology via a documented bridge model.

\paragraph{Assumptions (benchmark semantics).}
\begin{itemize}
    \item \textbf{Kernel discreteness}: constituent masses are generated from discrete topology indices (no per-element continuous fitting).
    \item \textbf{Single normalization}: the absolute mass scale is anchored once (declared explicitly) and then held fixed across the benchmark set.
    \item \textbf{Binding bridge transparency}: nuclear binding is incorporated via an explicit, documented phenomenological term (e.g.\ SEMF/Bethe--Weizs\"acker), whose uncertainty is tracked separately from kernel error.
    \item \textbf{Reproducibility}: code, constants, inputs, and evaluation scripts are versioned to reproduce every reported number.
\end{itemize}

\paragraph{What is claimed vs.\ not claimed.}
\begin{itemize}
    \item \textbf{Claimed:} Under the above rules, the model yields a quantitative error distribution on a fixed dataset, allowing direct falsification by out-of-sample failure or instability under declared sensitivity tests.
    \item \textbf{Not claimed:} We do not claim a complete ab initio nuclear theory; the binding bridge is explicitly phenomenological and treated as a controlled, auditable approximation layer.
\end{itemize}

\paragraph{Observable definition (what is measured).}
The primary observables are the benchmark masses $M^{\rm (data)}_i$ and the model predictions $M^{\rm (model)}_i$ under a fixed normalization, summarized by the absolute relative error
\begin{equation}
\delta_i \;\equiv\; \left|\frac{M^{\rm (model)}_i - M^{\rm (data)}_i}{M^{\rm (data)}_i}\right|,
\qquad
\text{and its distribution over the benchmark set } \{i=1,\dots,N\}.
\label{eq:obs_partIII_error}
\end{equation}
Reporting the median (and tails/outliers) of $\{\delta_i\}$ constitutes the falsifiable quantitative claim of this part.
