...
variance, and we correct a dimensionally inconsistent estimate that has
appeared in informal treatments. We also give a minimal reproducible
...
thermodynamic accounting and requires no assumptions specific to the quantum regime.
\end{abstract}
...
time and $\Sdot$ the entropy-production rate. The bound
\eqref{eq:TUR} is rigorous for systems obeying Markovian rate equations
or overdamped Langevin dynamics, but most practical clocks are based on
an underdamped oscillatory degree of freedom and are not constrained by
...
the resolution, accuracy and Allan variance, correcting a dimensionally
inconsistent accuracy estimate; (iii) establish analytically that
$\Dphi(M)\propto M^{-1}$ in the collective limit, yielding
...
discriminators. No quantum or microscopic assumptions enter; the
...
define a delay dynamical system whose loop ``gain--delay product''
$K=\lambda_0\,\taud$ controls the sequence one-pass transport
$\to$ stable locking $\to$ multistability $\to$ irregular
response, in the standard manner of delayed nonlinear
loops~\cite{GiacomelliPoliti1996,YanchukGiacomelli2017}. We work
throughout in the stable-locking regime, where a unique attracting limit
cycle exists and the clock ticks regularly.
...
A clock that violates \eqref{eq:TUR} in one limit and saturates it in
...
$\mathcal{N}=\pi\omega/\Dphi$; a simpler estimate
$\mathcal{N}\sim\omega^2/\Dphi$ is dimensionally inconsistent
(units of $\mathrm{s}^{-1}$) and overcounts by a factor $\omega\avg{\tau}$,
because it replaces the dimensionless phase increment $\Delta\phi$ by the
frequency $\omega$.
...
Each delayed kick $A\delta(t-t_j)$ projects onto the phase via the
infinitesimal phase-response curve (PRC) $Z(\phi)$: for a harmonic
oscillator with sinusoidal PRC $Z(\phi)=Z_0\sin\phi$, the mean-field
average of the projected impulse at rate $\lambda_0$ gives the delayed
restoring term $-\kappa[\phi(t)-\phi(t-\taud)]$ in
Eq.~\eqref{eq:sim_phase}, with effective feedback gain
$\kappa=\lambda_0 A Z_0/2$~\cite{Kuramoto1984,Pikovsky2001}.
The delay enters the phase dynamics through the linear response of the
locked cycle to a pulse applied at the delayed phase. Write the phase
fluctuation as $\delta\phi(t)$ and denote the small-signal delayed loop
kernel by $K_{\taud}(s)$. The most general linearized scalar form needed
here is
\begin{equation}
  \dot{\delta\phi}(t)
  =
...
\end{equation}
Periodic dependence on $\omega\taud$ is not universal; it appears once
the feedback gain is evaluated on a periodically gated oscillator. A
minimal sinusoidal phase-response model gives
\begin{equation}
  \Lambda(\taud)
  \simeq
...
nonzero~\cite{Wadhia2025}. The contribution of the present framework is
...
direct Monte Carlo check of the collective scaling laws in the idealized
limit $\taud=0$, $\Gamma\to\infty$, and one tick per period
($\Delta\phi=2\pi$). The sampled points were generated from the exact
first-passage-time distribution of the phase drift--diffusion process with
$\omega=1$ and $D_1=0.05$, using $5\times 10^4$ intervals for each value of
$M$. The expected invariants $\Dphi(M)M$ and $\mathcal{N}(M)/M$ are flat
within sampling error, providing the minimal numerical support requested by
Eq.~\eqref{eq:numerical_collective_pass}.
...
idealized phase-diffusion limit ($\taud=0$, ideal detector, one tick per
period, $\omega=1$, $D_1=0.05$). For each $M$, $5\times 10^4$ inter-tick
intervals were sampled from the exact first-passage-time distribution of
Eq.~\eqref{eq:phase}. Left: the product $\Dphi(M)M$ is constant, consistent
with Eq.~\eqref{eq:DphiM}. Right: the normalized accuracy
$\mathcal{N}(M)/M$ is constant, consistent with Eq.~\eqref{eq:NM}.}
...
\item \textbf{Resolution--accuracy trade-off.}
The product $\mathcal{N}\,\nu$ (and $\mathcal{N}\,\nu^2$) must respect the
...
Equation \eqref{eq:hypothesis} is a directly testable inequality: vary
...
become tractable. First, phase reduction yields a corrected,
dimensionally consistent accuracy
