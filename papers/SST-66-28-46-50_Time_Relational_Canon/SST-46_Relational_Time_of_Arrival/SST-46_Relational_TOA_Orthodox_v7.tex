...
\begin{document}
    ...
    \abstract{%
        Time-of-arrival (TOA) in quantum theory sits at an awkward intersection: Pauli-type arguments obstruct a self-adjoint operator canonically conjugate to a semibounded Hamiltonian, while relativistic localization makes ``the'' time parameter detector- and frame-dependent. We propose a covariant alternative that treats TOA as a relational field observable rather than an operator. The construction uses a conserved matter current $J^\mu$ for detector crossings through a world-tube $\Sigma$ and an effective event-count current $j^\mu_{\mathrm{ev}}$ for the physical clock sector. Conservation of $j^\mu_{\mathrm{ev}}$ stabilizes event counts, but a continuum scalar clock exists only when the coarse-grained event-current one-form is locally integrable. We therefore separate conservation from integrability and identify the event-current vorticity $\Omega^{\mathrm{ev}}_{\mu\nu}=\partial_\mu(j^{\mathrm{ev}}_\nu)_\ell-\partial_\nu(j^{\mathrm{ev}}_\mu)_\ell$ as the obstruction to a single-valued infrared clock field $T(x)$. When $\Omega^{\mathrm{ev}}_{\mu\nu}=0$, TOA is the flux through $\Sigma$ conditioned on the local clock reading $T|_{\Sigma}$; when $\Omega^{\mathrm{ev}}_{\mu\nu}\neq0$, the scalar-clock observable is replaced by a path-conditioned TOA distribution $p_{\mathcal{P}}(\Theta)$ whose dependence on the chosen path family is controlled by the clock holonomy. In a $1+1$-dimensional example with a narrowband Klein--Gordon wavepacket and a gapped quadratic clock EFT, the TOA density reduces in the integrable, spectator-clock regime to a convolution of the semiclassical flux profile with the clock readout distribution. For a Gaussian detector-resolution kernel the regulated clock variance is evaluated explicitly, giving $\sigma_{\tau,\ell}^2=(4\pi)^{-1}K_0(\mu_\tau^2\ell^2)$, with logarithmic short-distance and exponentially suppressed large-smearing limits. Spacelike massive-clock correlators further supply a model-dependent exponential envelope for clock-correlation tails outside the light cone. The result is a covariant TOA framework in which detector dependence is made explicit through the geometry of $\Sigma$, the effective flux $J^\mu$, the admissible clock current, the chosen clock-path family in the non-integrable sector, the associated holonomy variance when such path families are sampled, and the clock-sector correlators.
    }
