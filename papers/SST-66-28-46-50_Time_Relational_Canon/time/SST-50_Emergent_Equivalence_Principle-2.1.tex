...
%===========================================================
% Q-DESICS REVIEW + SST APPENDIX (ORTHODOX FIRST)
%===========================================================
...
%-----------------------------------------------------------
% SST macro prelude (used ONLY in Appendix; safe to include)
%-----------------------------------------------------------
    \newcommand{\swirlarrow}{\mathchoice{\mkern-2mu\scriptstyle\boldsymbol{\circlearrowleft}}%
    {\mkern-2mu\scriptstyle\boldsymbol{\circlearrowleft}}%
    {\mkern-2mu\scriptscriptstyle\boldsymbol{\circlearrowleft}}%
    {\mkern-2mu\scriptscriptstyle\boldsymbol{\circlearrowleft}}}
    \newcommand{\swirlarrowcw}{\mathchoice{\mkern-2mu\scriptstyle\boldsymbol{\circlearrowright}}%
...
% Document
%-----------------------------------------------------------
    \section{q--Desics in Quantum Gravity: Operator--Averaged Motion Beyond the Mean Metric\\
    \large Orthodox Review with a Speculative Swirl--String Theory (SST) Appendix}



            In classical general relativity, freely falling test particles follow geodesics determined by the spacetime metric.
            In quantum-gravitational settings, it is common to approximate motion by geodesics of the expectation-value metric
            $\langle \hat g_{\mu\nu}\rangle$. Koch, Riahinia, and Rinc\'on introduce \emph{q--desics} as quantum-corrected analogs of
            geodesics, defined instead by the expectation value of an \emph{affine-connection operator} $\langle \hat\Gamma^\beta_{\mu\nu}\rangle$.
            A central structural result is that q--desics coincide with geodesics of $\langle \hat g\rangle$ if and only if certain covariances
            between metric-derived operators vanish. This article reviews the framework in orthodox language, derives the covariance criterion,
            and summarizes implications for static, spherically symmetric quantum backgrounds. An appendix gives a clearly flagged, speculative
            mapping of the covariance structure to Swirl--String Theory (SST) as an example of how ``beyond-mean-field'' geometry can be encoded
            in second-moment microstructure data.




%===========================================================
        \section{Classical baseline: geodesics from an action}
%===========================================================
            Let $(\mathcal{M},g_{\mu\nu})$ be a Lorentzian spacetime and $x^\mu(\lambda)$ a worldline.
            For a massive particle, the reparameterization-invariant action can be written as
            \begin{equation}
                S[x;g] \;=\; -m \int d\lambda\, \sqrt{g_{\mu\nu}(x)\,\dot x^\mu \dot x^\nu},
            \end{equation}
            where $\dot x^\mu \equiv dx^\mu/d\lambda$.
            Variation with respect to $x^\mu(\lambda)$ yields the geodesic equation
            \begin{equation}
                \ddot x^\beta + \Gamma^\beta_{\mu\nu}(g)\,\dot x^\mu \dot x^\nu = 0,
                \qquad
                \Gamma^\beta_{\mu\nu}(g) = \frac{1}{2} g^{\beta\alpha}\big(\partial_\mu g_{\nu\alpha}
                +\partial_\nu g_{\mu\alpha}-\partial_\alpha g_{\mu\nu}\big).
                \label{eq:classical_geodesic}
            \end{equation}
            Null geodesics arise from the massless limit, together with the null constraint $g_{\mu\nu}\dot x^\mu\dot x^\nu=0$.

%===========================================================
        \section{Common semiclassical proxy and its structural limitation}
%===========================================================
            In many semiclassical or effective treatments, the quantum-gravitational background is summarized by an expectation value
            $\langle \hat g_{\mu\nu}\rangle$ and test-particle motion is approximated by geodesics of $\langle \hat g\rangle$.
            This is a \emph{mean-field} replacement:
            \begin{equation}
                \Gamma^\beta_{\mu\nu}(g) \;\mapsto\; \Gamma^\beta_{\mu\nu}\big(\langle \hat g\rangle\big).
            \end{equation}
            A structural limitation is immediate: in general, \emph{nonlinear} functions of operators do not commute with taking expectation values,
            so $\langle f(\hat g)\rangle \neq f(\langle \hat g\rangle)$ for generic states.

            The q--desic framework makes this mismatch explicit at the level of the affine connection.

%===========================================================
        \section{q--Desics: definition via operator--level variational principle}
%===========================================================
            Koch, Riahinia, and Rinc\'on derive a quantum-corrected equation of motion by keeping the spacetime metric as an operator
            $\hat g_{\mu\nu}$ as long as possible, and taking the spacetime expectation value at the end.
            The resulting q--desic equation is (their Eq.\ (16))
            \begin{equation}
                \ddot x^\beta + \big\langle \hat\Gamma^\beta_{\mu\nu}\big\rangle\,\dot x^\mu \dot x^\nu = 0,
                \label{eq:qdesic}
            \end{equation}
            with an operator-valued Christoffel structure whose expectation value is defined (their Eq.\ (17)) by
            \begin{equation}
                \big\langle \hat\Gamma^\beta_{\mu\nu}\big\rangle
                \;:=\;
                \left\langle
                    \frac{1}{2}\,
                    \big(\partial_\mu \hat g_{\nu\alpha} + \partial_\nu \hat g_{\mu\alpha} - \partial_\alpha \hat g_{\mu\nu}\big)\,
                    \hat g^{\alpha\beta}
                \right\rangle.
                \label{eq:Gammahat_def}
            \end{equation}
            Here $\hat g^{\alpha\beta}$ denotes an inverse-metric operator, assumed to be well-defined in the chosen quantum-gravity setting.
            For massive vs.\ massless motion, the usual normalization constraint is replaced by the expectation-value constraint (their Eq.\ (25))
            \begin{equation}
                \langle \hat g_{\mu\nu}\rangle\,u^\mu u^\nu =
                \begin{cases}
                    1, & \text{massive},\\
            ...
            \paragraph{Interpretation.}
                Equation \eqref{eq:qdesic} shows that the ``effective geometry felt by a test particle'' can depend on
                $\langle \hat\Gamma\rangle$ rather than solely on $\Gamma(\langle \hat g\rangle)$.

%===========================================================
        \section{Covariance criterion: when q--desics reduce to g--eodesics}
%===========================================================
        Define the \emph{geodesic-of-the-mean} connection
        \begin{equation}
            \Gamma^\beta_{\mu\nu}\big(\langle \hat g\rangle\big)
            =
            \frac{1}{2}\,\langle \hat g\rangle^{\beta\alpha}\big(
            \partial_\mu \langle \hat g_{\nu\alpha}\rangle
            +
            \partial_\nu \langle \hat g_{\mu\alpha}\rangle
            -
            \partial_\alpha \langle \hat g_{\mu\nu}\rangle
            \big),
        \end{equation}
        and the \emph{q--desic covariance} (their Eq.\ (81)) as the difference
        \begin{equation}
            \mathrm{Cov}^{\beta}{}_{\mu\nu}(\Gamma)
            \;:=\;
            \big\langle \hat\Gamma^\beta_{\mu\nu}\big\rangle
            -
            \Gamma^\beta_{\mu\nu}\!\big(\langle \hat g\rangle\big).
        ...
        \begin{equation}
            \hat X_{\mu\nu\alpha}
            :=\frac{1}{4}\,\big(\partial_\mu \hat g_{\nu\alpha} + \partial_\nu \hat g_{\mu\alpha} - \partial_\alpha \hat g_{\mu\nu}\big).
            \label{eq:X_def}
        \end{equation}
        ...
        \begin{equation}
            \mathrm{Cov}^{\beta}{}_{\mu\nu}(\Gamma)
            =
            \left\langle 2\,\hat X_{\mu\nu\alpha}\,\hat g^{\alpha\beta}\right\rangle
            -
            2\,\left\langle \hat X_{\mu\nu\alpha}\right\rangle\,\left\langle \hat g^{\alpha\beta}\right\rangle
            \;+\;(\text{ordering/commutator terms}),
        \end{equation}
        i.e.\ it measures the failure of factorization of the mixed operator product.
        With Weyl ordering in observables, the remaining contribution can be expressed in terms of an anticommutator expectation
        (their Eq.\ (83)):
        \begin{equation}
            \mathrm{Cov}^{\beta}{}_{\mu\nu}(\Gamma)
            =\left\langle\!\left\langle \big\{\hat X_{\mu\nu\alpha},\,\hat g^{\alpha\beta}\big\}\right\rangle\!\right\rangle,
            \label{eq:Cov_anticom}
        \end{equation}
        where the double-bracket notation indicates ``connected'' (covariance-like) parts.

        \paragraph{Consequence.}
            If $\mathrm{Cov}^{\beta}{}_{\mu\nu}(\Gamma)=0$ for a given state, then
            \begin{equation}
                \big\langle \hat\Gamma^\beta_{\mu\nu}\big\rangle
                =
                \Gamma^\beta_{\mu\nu}\!\big(\langle \hat g\rangle\big),
...
%===========================================================
        \section{Static spherically symmetric quantum backgrounds (orthodox summary)}
%===========================================================
        The paper applies \eqref{eq:qdesic} to static, spherically symmetric quantum-gravitational backgrounds obtained via canonical quantization
        under a static equal-radius (SER) condition. In standard coordinates, one may parameterize a classical static, spherically symmetric line element as
        \begin{equation}
            ds^2 = -n(r)^2\,dt^2 + g(r)^2\,dr^2 + r^2(d\theta^2+\sin^2\theta\,d\phi^2),
            \label{eq:SSS_metric}
        \end{equation}
        and promote the radial functions to operators $\hat n(r)$, $\hat g(r)$.
        Inputs to q--desic motion are then expectation values such as $\langle \hat g\rangle$, $\langle \hat n^2\rangle$,
        and mixed products entering \eqref{eq:Gammahat_def}.

        A representative example of the structure of these expectation values (quoted in the paper's ``selected expectation values'' discussion)
        is an expansion of the form
        \begin{equation}
            \langle \hat g\rangle
            =
            (1+\epsilon_{0,0}) + \frac{C_{1,0}}{r} - \frac{\Lambda(1+\epsilon_{0,0})}{3}\,r^2 + \cdots,
            \label{eq:g_expect_generic}
        \end{equation}
        with state-dependent integration constants (e.g.\ $\epsilon_{i,j}$) and couplings (e.g.\ $C_{1,0}$, $\Lambda$).
        The q--desic observables depend not only on \eqref{eq:g_expect_generic} but also on covariances such as \eqref{eq:Cov_def}--\eqref{eq:Cov_anticom},
        which encode additional state information beyond the mean fields.

%===========================================================
        \section{Observable deviations: what changes in principle?}
%===========================================================
        Two broad classes of effects follow structurally:

        \subsection{Radial null motion and ``horizons''}
            For radial null propagation, a classical ``horizon'' location is typically inferred from the vanishing of an effective lapse function
            or from divergences in $dr/dt$ depending on coordinates. In q--desic motion, the relevant relation involves
            $\langle \hat\Gamma\rangle$ and can shift the location where coordinate speeds change qualitatively, even if
            $\langle \hat g\rangle$ resembles a classical form.
            Operationally, horizon-identification becomes \emph{state-dependent} through covariances.

        \subsection{Circular motion of massive particles}
            For circular motion, GR yields the standard relation between orbital velocity, enclosed mass, and radius
            in the weak-field regime. In the q--desic framework, the orbital relations receive corrections controlled by:
            (i) deviations in mean metric expectation values; and (ii) non-factorizing mixed correlators (the covariance terms).
            Thus, even far above the Planck scale, small but structured deviations can arise if the background quantum state
            has nontrivial correlations.

%===========================================================
        \section{Relation to other orthodox frameworks (brief)}
%===========================================================
        The paper emphasizes analogies to standard operator-vs-average issues (e.g.\ the Ehrenfest theorem in quantum mechanics),
        and notes links to:
        \begin{itemize}
            \item \textbf{Stochastic gravity:} where induced and intrinsic metric fluctuations are characterized by noise kernels and correlators;
            \item \textbf{Effective average action / FRG:} where scale-dependent effective metrics $\langle \hat g\rangle_k$ arise from $\Gamma_k[g]$;
            \item \textbf{Non-geodesic motion in GR:} e.g.\ Mathisson--Papapetrou--Dixon motion of spinning bodies (classical non-geodesicity from spin-curvature).
        \end{itemize}

%===========================================================
        \section{Conclusions (orthodox)}
%===========================================================
        q--desics provide a clean operator-defined extension of classical geodesic motion, replacing $\Gamma(\langle \hat g\rangle)$ by
        $\langle \hat\Gamma\rangle$. The key structural object is the covariance \eqref{eq:Cov_def}, which measures the failure of
        factorization of mixed metric-derived operators. Consequently, two quantum states with identical $\langle \hat g\rangle$
        may still yield different test-particle motion if their covariances differ. This isolates precisely what additional geometric
        information beyond the mean metric is required to predict motion in quantum spacetimes.

%===========================================================
        \appendix
        \section{Speculative SST appendix: mapping the covariance structure to swirl microstructure}
%===========================================================
        \subsection*{Scope and warning}
            This appendix is \textbf{not} orthodox GR/QG. It is a \textbf{speculative} structural translation into Swirl--String Theory (SST)
            language, intended to extract a reusable lemma:
            \begin{quote}
                \emph{Mean-field ``geometry'' is insufficient when motion depends on mixed correlators; one must model second moments of the microstructure.}
            \end{quote}

        \subsection{Minimal dictionary (structural, not ontological)}
            We treat the q--desic covariance as a template for how a coarse-grained continuum description can miss state information.
            A minimal SST-leaning dictionary is:
            \begin{center}
                \begin{tabular}{@{}ll@{}}
                    \toprule
                    ...
                    $\langle \hat\Gamma^\beta_{\mu\nu}\rangle$ & coarse-grained affine response of the medium \\
                    $\mathrm{Cov}(\Gamma)$ & second-moment microstructure data (mixed correlators) \\
                    \bottomrule
                \end{tabular}
            \end{center}

        \subsection{A concrete toy translation: ``inverse-times-gradient'' mismatch}
            In SST one often introduces a \emph{clock field} (time-scaling functional) built from a local medium variable (e.g.\ a swirl-clock proxy).
            Abstractly, suppose an effective lapse-like factor $N(x)$ is a nonlinear functional of a micro-variable $\chi(x)$,
            and appears in an effective metric component $g_{tt}=-N^2$.
            Then the classical connection component relevant for radial acceleration contains
            \begin{equation}
                \Gamma^{r}{}_{tt} \sim -\frac{1}{2}\,g^{rr}\,\partial_r(N^2).
            \end{equation}
            Promoting $N$ and $g^{rr}$ to micro-operators (or random fields) and coarse-graining suggests two inequivalent closures:
            \begin{align}
                \text{(mean-metric closure)}\quad & \Gamma^{r}{}_{tt}\big(\langle g\rangle\big)
                \;\propto\; \langle g^{rr}\rangle\,\partial_r\langle N^2\rangle,
                \\
                \text{(q--desic-like closure)}\quad & \langle \Gamma^{r}{}_{tt}\rangle
                \;\propto\; \left\langle g^{rr}\,\partial_r(N^2)\right\rangle
                \;=\; \langle g^{rr}\rangle\,\partial_r\langle N^2\rangle
                \;+\;\mathrm{Cov}\!\big(g^{rr},\partial_r(N^2)\big).
                \label{eq:SST_cov_template}
            \end{align}
            Equation \eqref{eq:SST_cov_template} is the SST-side moral equivalent of \eqref{eq:Cov_def}:
            \emph{the correction is controlled by a mixed correlator, not by the mean fields alone.}

        \subsection{Numerical scale note using SST constants}
            If a toy clock functional depends on a characteristic swirl speed scale $\vswirl$ via a dimensionless ratio $(\vswirl/c)^2$,
            then the small parameter is
            \begin{equation}
                \left(\frac{\vswirl}{c}\right)^2
                =
                \left(\frac{1.09384563\times 10^6~\mathrm{m/s}}{2.99792458\times 10^8~\mathrm{m/s}}\right)^2
                \approx 1.3313\times 10^{-5}.
            \end{equation}
            This does \emph{not} by itself fix the covariance magnitude, but it provides an SST-consistent reference scale for any clock-field
            nonlinearity expanded in $(\vswirl/c)^2$.

        \subsection{Falsifiable SST-facing lesson}
            ...
            \begin{equation}
                \mathrm{Cov}\!\big(g^{rr},\partial_r(N^2)\big)\approx 0
            \end{equation}
            for relevant states of the medium. Conversely, if SST allows persistent correlated microstructure, then deviations from
            mean-field motion become possible \emph{even when mean fields are unchanged}. This is a precise place to attach experimental
            or observational constraints: they target \emph{correlations}, not merely averages.

        \subsection*{10-year-old analogy}
            If you only look at the \emph{average} speed of a river you might think a boat goes straight, but if fast swirls tend to happen
            exactly where the water is also shallow, the boat gets pushed in a special way. The pushing depends on how those two things
            \emph{line up together}, not just on their averages.

%===========================================================
            \begin{thebibliography}{99}

                \bibitem{KochRiahiniaRincon2025}
                B.~Koch, A.~Riahinia, and A.~Rinc\'on,
                \emph{Geodesics in quantum gravity},
                Phys.\ Rev.\ D \textbf{112}, 084056 (2025),
                DOI:10.1103/w1sd-v69d; arXiv:2510.00117.

                \bibitem{Wald1984}
                R.~M.~Wald,
                \emph{General Relativity},
                University of Chicago Press (1984).
                (Publisher link: \url{https://press.uchicago.edu/ucp/books/book/chicago/G/bo5952261.html})
                ...
                Living Rev.\ Relativity \textbf{11}, 3 (2008),
                DOI:10.12942/lrr-2008-3; arXiv:0802.0658.
                ...
                A.~Papapetrou,
                \emph{Spinning test-particles in general relativity. I},
                Proc.\ Roy.\ Soc.\ Lond.\ A \textbf{209}, 248--258 (1951),
                DOI:10.1098/rspa.1951.0200.

                \bibitem{Dixon1970}
                W.~G.~Dixon,
                \emph{Dynamics of extended bodies in general relativity. I. Momentum and angular momentum},
                Proc.\ Roy.\ Soc.\ Lond.\ A \textbf{314}, 499--527 (1970),
