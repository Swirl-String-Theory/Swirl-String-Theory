    ...
    for i in range(len(E) - 1):
        ...
        if dy[i] == 0.0 or dy[i] * dy[i + 1] < 0.0:
            ...
            rows.append({
                ...
                "value_interp": y_root,
                "d2_interp": d2_root,
                "classification": "minimum" if d2_root > 0 else "maximum_or_saddle",
                "bracket_E_lo": E[i],
                "bracket_E_hi": E[i + 1],
            })

    if not rows:
        # Still report closest derivative point.
        ...
        rows.append({
            ...
            "classification": "no_sign_change_closest_abs_derivative",
            "bracket_E_lo": np.nan,
            "bracket_E_hi": np.nan,
        })
...
def save_plot(df: pd.DataFrame, candidates: pd.DataFrame, outdir: Path, reference_e0: Optional[float]) -> None:
    ...
    for _, row in candidates.iterrows():
        if row["observable"] == "sector_action":
            ax.axvline(row["E_candidate"], ls="--", alpha=0.7, label=f"candidate E={row['E_candidate']:.6g}")
...
def main() -> None:
    ...
    parser.add_argument("--reference-e0", type=float, default=274.074996, help="Optional reference line in plots; not used in computation.")
    ...
    print("\nInterpretation")
    print("--------------")
    print("If the candidate E values are not near 274.074996, that does not falsify the")
    print("full Hodge-Steklov program; it means this lightweight proxy kernel is not yet")
    print("the required proof-grade boundary-value solver. A validated FEM/BEM/DEC solver")
    print("on B_R \\ T_a(K) is the next step.")
