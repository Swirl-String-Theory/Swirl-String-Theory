...
def pressure_scale_from_mode(mode: str, ropelength: float, input_scale: Optional[float]) -> float:
    if mode == "input":
    ...
    if mode == "ropelength_16pi_over_3":
        # Geometric spherical-volume coefficient: (16*pi/3) * L/D.
...
def scan_E_values(args, coeffs, L_ref: float, D_ref: float, source: str) -> Tuple[pd.DataFrame, Dict[str, object]]:
    ...
    E_p = pressure_scale_from_mode(args.pressure_scale_mode, ropelength, args.pressure_Ep)
    ...
    for E in e_values:
        ...
        action_pressure = pressure_action(E, args.pressure_mode, args.pressure_kappa, E_p)
    ...
    summary = {
        ...
        "pressure_Ep": E_p,
        "pressure_kappa": args.pressure_kappa,
...
def main() -> None:
    ...
    parser.add_argument("--pressure-scale-mode", choices=["input", "ropelength_16pi_over_3", "ropelength_4pi", "ropelength_8pi"], default="ropelength_16pi_over_3")
    parser.add_argument("--pressure-Ep", type=float, default=None, help="Pressure equilibrium scale if scale-mode=input.")
    parser.add_argument("--pressure-kappa", type=float, default=1.0)
