...
Batch convergence run:
    outputs_E0_bem_pressure/batch_summary.csv
    outputs_E0_bem_pressure/batch_desired_table.csv
    outputs_E0_bem_pressure/batch_controls.csv
    outputs_E0_bem_pressure/batch_runs/<run_id>/...
"""
...
# ---------------------------------------------------------------------------
# Batch convergence / epsilon sweep.
# ---------------------------------------------------------------------------

def parse_eps_factors(text: str) -> List[float]:
    return [float(x.strip()) for x in text.split(",") if x.strip()]


def parse_resolution_grid(text: str) -> List[Tuple[int, int, int]]:
    """
    Parse a resolution grid string such as:
        "100:14:300,120:16:360,140:18:420"
    into [(n_s,n_theta,sphere_n), ...].
    """
    ...
    for chunk in text.split(","):
        ...
        if len(parts) != 3:
            raise ValueError(f"Bad resolution triple {chunk!r}; expected n_s:n_theta:sphere_n")
        triples.append((parts[0], parts[1], parts[2]))
    return triples


def clone_args_with_updates(args: argparse.Namespace, **updates) -> argparse.Namespace:
    data = vars(args).copy()
    data.update(updates)
    return argparse.Namespace(**data)


def interpolate_at_E(df: pd.DataFrame, E_value: float, cols: List[str]) -> Dict[str, float]:
    out: Dict[str, float] = {}
    E_arr = df["E"].to_numpy()
    logE = np.log(E_arr)
    x = math.log(E_value)
    for col in cols:
        y = df[col].to_numpy()
        if x < logE.min() or x > logE.max():
            out[col + "_at_E"] = np.nan
        else:
            out[col + "_at_E"] = float(np.interp(x, logE, y))
    return out


def run_one_batch_case(
    ...
    eps_factor: float,
    pressure_mode: str,
    pressure_scale_mode: str,
    pressure_kappa_mode: str,
) -> Dict[str, object]:
    run_out = outdir / "batch_runs" / run_id
    run_out.mkdir(parents=True, exist_ok=True)

    args = clone_args_with_updates(
        base_args,
        n_s=n_s,
        n_theta=n_theta,
        sphere_n=sphere_n,
        ...
        pressure_scale_mode=pressure_scale_mode,
        pressure_kappa_mode=pressure_kappa_mode,
        outdir=str(run_out),
    )

    df, geom_summary = scan_E_values(args, coeffs, L_ref, D_ref, source)
    df.to_csv(run_out / "bem_period_matrix_scan.csv", index=False)
    pd.DataFrame([geom_summary]).to_csv(run_out / "geometry_summary.csv", index=False)
    with open(run_out / "model_assumptions.json", "w", encoding="utf-8") as f:
    ...
    candidates.to_csv(run_out / "stationary_candidates.csv", index=False)

    derived = summarize_derived_E0(candidates, args.preferred_observable)
    pd.DataFrame([derived]).to_csv(run_out / "derived_E0_summary.csv", index=False)
    ...
    pd.DataFrame([alpha_summary]).to_csv(run_out / "alpha_from_derived_E0_summary.csv", index=False)

    save_plots(df, candidates, run_out, args.reference_e0)

    E_p = float(geom_summary.get("pressure_Ep", np.nan))
    E_star = float(derived.get("E_star_derived", np.nan)) if derived.get("status") == "E0_derived" else np.nan

    interp_cols = [
        "action_bem", "action_pressure", "action_total",
        "d_action_bem_dlogE", "d_action_pressure_dlogE", "d_action_total_dlogE",
        "d2_action_total_dlogE2",
    ]
    interp = interpolate_at_E(df, E_star, interp_cols) if np.isfinite(E_star) else {}
    ...
    result: Dict[str, object] = {
        ...
        "ropelength_L_over_D": ropelength,
        "E_p_NLS_or_pressure": E_p,
        "kappa_eff": float(geom_summary.get("pressure_kappa_eff", np.nan)),
        "E_star": E_star,
        "E_star_minus_Ep": E_star - E_p if np.isfinite(E_star) and np.isfinite(E_p) else np.nan,
        "E_star_over_Ep_minus_1": E_star / E_p - 1.0 if np.isfinite(E_star) and np.isfinite(E_p) else np.nan,
        "bracket_E_lo": float(derived.get("bracket_E_lo", np.nan)) if derived.get("status") == "E0_derived" else np.nan,
        "bracket_E_hi": float(derived.get("bracket_E_hi", np.nan)) if derived.get("status") == "E0_derived" else np.nan,
        "stationary_d2_log": float(derived.get("d2_interp", np.nan)) if derived.get("status") == "E0_derived" else np.nan,
        "alpha_inv": float(alpha_summary.get("alpha_cell_inverse_from_derived_E0", np.nan)),
        "alpha": float(alpha_summary.get("alpha_cell_from_derived_E0", np.nan)),
        "alpha_rel_error_vs_CODATA_2022": float(alpha_summary.get("relative_error_vs_CODATA_2022", np.nan)),
        "v_core_pred_m_per_s": float(alpha_summary.get("v_core_pred_m_per_s", np.nan)),
...
def batch_convergence_sweep(args: argparse.Namespace) -> None:
    outdir = Path(args.outdir)
    outdir.mkdir(parents=True, exist_ok=True)

    coeffs, L_ref, D_ref, source = get_coefficients(args)
    eps_values = parse_eps_factors(args.batch_eps_factors)
    resolution_grid = parse_resolution_grid(args.batch_resolution_grid)

    print("Batch convergence / epsilon sweep")
    print("=" * 76)
    print(f"source              : {source}")
    print(f"resolution grid     : {resolution_grid}")
    print(f"eps factors         : {eps_values}")
    print(f"E range             : {args.e_min} ... {args.e_max}, count={args.e_count}")
    ...
    # Main NLS-shell runs.
    for n_s, n_theta, sphere_n in resolution_grid:
        for eps in eps_values:
            run_id = (
                f"main_ns{n_s}_nt{n_theta}_sp{sphere_n}_eps{eps:.4f}"
                .replace(".", "p")
            )
            print(f"\n[MAIN] {run_id}")
            rows.append(run_one_batch_case(
                args, coeffs, L_ref, D_ref, source, run_id, outdir,
                n_s=n_s, n_theta=n_theta, sphere_n=sphere_n, eps_factor=eps,
                pressure_mode=args.pressure_mode,
                pressure_scale_mode=args.pressure_scale_mode,
                pressure_kappa_mode=args.pressure_kappa_mode,
            ))

    # Controls: BEM-only and pressure-only-like strong/weak diagnostic at representative resolution.
    controls: List[Dict[str, object]] = []
    if args.batch_include_controls:
        mid = resolution_grid[len(resolution_grid) // 2]
        eps_mid = eps_values[len(eps_values) // 2]

        control_specs = [
            ("control_bem_only", "none", args.pressure_scale_mode, args.pressure_kappa_mode),
            ("control_logcore", "spherical", "nls_shell", "nls_log_core"),
            ("control_geomean", "spherical", "nls_shell", "nls_geometric_mean"),
            ("control_shell_half", "spherical", "nls_shell", "nls_shell_stiffness_half"),
            ("control_shell_double", "spherical", "nls_shell", "nls_shell_stiffness_double"),
        ]
        for prefix, p_mode, scale_mode, k_mode in control_specs:
            run_id = f"{prefix}_ns{mid[0]}_nt{mid[1]}_sp{mid[2]}_eps{eps_mid:.4f}".replace(".", "p")
            print(f"\n[CONTROL] {run_id}")
            controls.append(run_one_batch_case(
                args, coeffs, L_ref, D_ref, source, run_id, outdir,
                n_s=mid[0], n_theta=mid[1], sphere_n=mid[2], eps_factor=eps_mid,
                pressure_mode=p_mode,
                pressure_scale_mode=scale_mode,
                pressure_kappa_mode=k_mode,
            ))

    summary = pd.DataFrame(rows)
    controls_df = pd.DataFrame(controls)
    summary.to_csv(outdir / "batch_summary.csv", index=False)
    controls_df.to_csv(outdir / "batch_controls.csv", index=False)

    # Desired compact table for manuscript/supplement.
    desired_cols = [
        "run_id",
        "status",
        "n_s", "n_theta", "sphere_n", "eps_factor",
        "ropelength_L_over_D",
        "E_p_NLS_or_pressure",
        "kappa_eff",
        "E_star",
        "E_star_minus_Ep",
        "E_star_over_Ep_minus_1",
        "stationary_d2_log",
        "alpha_inv",
        "alpha_rel_error_vs_CODATA_2022",
        "d_action_bem_dlogE_at_E",
        "d_action_pressure_dlogE_at_E",
        "d_action_total_dlogE_at_E",
        "run_outdir",
    ]
    desired = summary[[c for c in desired_cols if c in summary.columns]].copy()
    desired.to_csv(outdir / "batch_desired_table.csv", index=False)

    # Aggregate statistics for successful runs.
    ok = summary[summary["status"] == "E0_derived"].copy()
    if len(ok):
        agg = {
            "n_success": int(len(ok)),
            "n_total": int(len(summary)),
            "E_star_mean": float(ok["E_star"].mean()),
            "E_star_std": float(ok["E_star"].std(ddof=0)),
            "E_star_min": float(ok["E_star"].min()),
            "E_star_max": float(ok["E_star"].max()),
            "alpha_inv_mean": float(ok["alpha_inv"].mean()),
            "alpha_inv_std": float(ok["alpha_inv"].std(ddof=0)),
            "alpha_rel_error_mean": float(ok["alpha_rel_error_vs_CODATA_2022"].mean()),
            "alpha_rel_error_std": float(ok["alpha_rel_error_vs_CODATA_2022"].std(ddof=0)),
        }
    else:
        agg = {"n_success": 0, "n_total": int(len(summary)), "message": "No successful E0-derived runs."}
    pd.DataFrame([agg]).to_csv(outdir / "batch_aggregate_stats.csv", index=False)

    print("\nBatch complete")
    print("--------------")
    print(f"Main runs    : {len(summary)}")
    print(f"Controls     : {len(controls_df)}")
    print(f"Successes    : {agg.get('n_success', 0)} / {agg.get('n_total', len(summary))}")
    if len(ok):
        print(f"E_star mean  : {agg['E_star_mean']:.12g}")
        print(f"E_star std   : {agg['E_star_std']:.12g}")
        print(f"alpha^-1 mean: {agg['alpha_inv_mean']:.12g}")
    print("\nExported:")
    print(f"  {outdir / 'batch_summary.csv'}")
    print(f"  {outdir / 'batch_desired_table.csv'}")
    print(f"  {outdir / 'batch_controls.csv'}")
    print(f"  {outdir / 'batch_aggregate_stats.csv'}")
...
def main() -> None:
    ...
    parser.add_argument("--batch-sweep", action="store_true", help="Run convergence/epsilon batch sweep and export summary tables.")
    parser.add_argument("--batch-resolution-grid", default="100:14:300,120:16:360,140:18:420,160:20:480", help="Comma-separated n_s:n_theta:sphere_n triples.")
    parser.add_argument("--batch-eps-factors", default="0.04,0.05,0.06,0.08", help="Comma-separated epsilon factors for batch sweep.")
    parser.add_argument("--batch-include-controls", action="store_true", help="Also run BEM-only and alternative kappa control cases.")
    args = parser.parse_args()

    if args.batch_sweep:
        batch_convergence_sweep(args)
        return
