...
is a hypothesis/input or a consequence of the specified assumptions.  The
``canon_core_cycle`` model computes Φ_bg from the canonical SST core cycle,
Ω_core = |v_swirl|/r_c and T_core = 2π r_c/|v_swirl|, so Ω_core T_core = 2π.
...
------------
    ...
    python fs_core_holonomy_audit.py --models zero,canon_core_cycle,twoomega_transit \
...
def parse_models(s: str) -> List[str]:
    if str(s).lower().strip() == "all":
        return ["zero", "unit", "signed_unit", "canon_core_cycle", "twoomega_cycle", "twoomega_transit", "background"]
    out = []
    for part in str(s).split(','):
        part = part.strip().lower()
        if part:
            out.append(part)
    valid = {"zero", "unit", "signed_unit", "canon_core_cycle", "twoomega_cycle", "twoomega_transit", "background"}
...
def phi_for_model(model: str, row: Dict[str, object], entry: "cta.CurveEntry", args: argparse.Namespace,
    ...
    if model == "canon_core_cycle":
        # Canon-derived closure model.  It does not use pq+1 or the curve length.
        # It uses only the canonical local core cycle:
        #     Ω_core = |v_swirl| / r_c,
        #     T_core = 2π r_c / |v_swirl|,
        # so Ω_core T_core = 2π and n_core = Φ/(2π) = 1.
        # Equivalently, with vorticity ω_core = 2Ω_core,
        #     (1/4π)∫ω_core dt = 1.
        rc = float(args.canon_r_c)
        vs = float(args.canon_v_swirl)
        omega_core = vs / rc
        t_core = TAU * rc / vs
        phi = omega_core * t_core
        return HolonomyModelResult(
            model=model,
            phi_turns=phi / TAU,
            phi_rad=phi,
            status="CANON_CORE_CYCLE_CLOSURE",
            assumption=(
                f"computed from canonical core cycle: Ω_core=v_swirl/r_c={omega_core:.6g} s^-1, "
                f"T_core=2π r_c/v_swirl={t_core:.6g} s, ΩT/(2π)={phi/TAU:.12g}; "
                "does not use pq+1 or curve length"
            ),
        )
...
def model_epistemic_status(model_name: str, selected_n: int, required_n: Optional[int], h: HolonomyModelResult) -> str:
    ...
    if model_name == "canon_core_cycle":
        return "SELECTS_REQUIRED_BY_CANON_CORE_CYCLE"
...
def write_markdown(path: Path, rows: Sequence[Dict[str, object]], landscape: Sequence[Dict[str, object]], args: argparse.Namespace) -> None:
    ...
    lines.append("[CANON-CYCLE]   canon_core_cycle selects +1 from Ω_core=v_swirl/r_c and T_core=2πr_c/v_swirl; still a closure model, but uses canonical constants rather than pq+1.")
    ...
    lines.append("[OPEN]          justify why the canonical local core cycle is the physical holonomy attached to every closed swirl string, rather than a mere local period identity.")
    lines.append("```")
    lines.append("")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--models", default="zero,unit,canon_core_cycle,twoomega_cycle,twoomega_transit,background",
                    help="Comma list or 'all': zero,unit,signed_unit,canon_core_cycle,twoomega_cycle,twoomega_transit,background")
    ...
    # Canonical constants for canon_core_cycle, and physical/diagnostic parameters for twoomega_transit.
    ap.add_argument("--canon-r-c", type=float, default=R_C,
                    help=f"canonical core/return-flow radius r_c for canon_core_cycle; default {R_C:.9g} m")
    ap.add_argument("--canon-v-swirl", type=float, default=V_SWIRL,
                    help=f"canonical tangential swirl speed for canon_core_cycle; default {V_SWIRL:.9g} m/s")
