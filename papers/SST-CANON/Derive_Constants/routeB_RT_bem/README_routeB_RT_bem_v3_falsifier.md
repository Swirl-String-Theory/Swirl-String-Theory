...
```bash
python routeB_RT_bem_v3_falsifier.py --source sstcore --list-sstcore-knots --sstcore-max-knots 50
```
...
```bash
python routeB_RT_bem_v3_falsifier.py --source sstcore --outdir outputs_routeB_SSTcore
```

## Run multiple knots from SSTcore

```bash
python routeB_RT_bem_v3_falsifier.py \
  --source sstcore \
...
```bash
python routeB_RT_bem_v3_falsifier.py   --source file --ideal ideal.txt    --sstcore-knot-ids all   --sstcore-max-knots 20   --outdir outputs_routeB_SSTcore_all20
```

## Use a local coordinate ideal.txt instead

```bash
python routeB_RT_bem_v3_falsifier.py --source file --ideal ideal.txt --outdir outputs_routeB_file
```
