...
def parse_ideal_xml_fourier_file(path: Path) -> Dict[str, Dict[str, object]]:
    """
    Parse Brian Gilbert XML/Fourier ideal-knot database.

    v3.3 detail: multi-component link blocks are listed with metadata, but their
    nested <Component> coefficients are not collapsed into one centerline.  This
    BEM Route-B harness currently supports single closed centerlines only.
    """
    text = path.read_text(encoding='utf-8', errors='replace')
    blocks: Dict[str, Dict[str, object]] = {}
    for m in re.finditer(r'<AB\b([^>]*)>(.*?)</AB>', text, flags=re.S | re.I):
        ab_attrs = parse_attrs_from_tag(m.group(1))
        bid = ab_attrs.get('Id', f'AB_{len(blocks)+1}')
        body = m.group(2)

        try:
            n_components = int(str(ab_attrs.get('n', '1')).strip())
        except Exception:
            n_components = 1

        # Do not let nested component coefficients masquerade as one knot curve.
        direct_body = re.sub(r'<Component\b.*?</Component>', '', body, flags=re.S | re.I)

        A: Dict[int, np.ndarray] = {}
        B: Dict[int, np.ndarray] = {}
        for cm in re.finditer(r'<Coeff\b([^>]*)/?>', direct_body, flags=re.I):
            ca = parse_attrs_from_tag(cm.group(1))
            if 'I' not in ca or 'A' not in ca or 'B' not in ca:
                continue
            idx = int(ca['I'])
        ...
        def ffloat(key):
            try:
                return float(str(ab_attrs.get(key, '')).strip())
            except Exception:
                return None
        ...
        blocks[bid] = {
            ...
            'is_supported_single_component': bool(n_components == 1 and A and B),
            'A': A,
            'B': B,
...
def ideal_id_to_display_name(bid: str) -> str:
    parts = str(bid).split(':')
    if len(parts) >= 2 and parts[1] == '1' and parts[0] in {'0','3','4'}:
        return f'{parts[0]}_1'
    if len(parts) >= 2:
        return '_'.join(parts[:3])
    return str(bid).replace(':', '_')

def eval_ideal_fourier_block(block: Dict[str, object], n: int) -> np.ndarray:
    t = np.linspace(0.0, 2.0 * math.pi, int(n), endpoint=False)
    pts = np.zeros((len(t), 3), dtype=float)
    A: Dict[int, np.ndarray] = block['A']  # type: ignore[assignment]
    B: Dict[int, np.ndarray] = block['B']  # type: ignore[assignment]
    for I in sorted(set(A) | set(B)):
        av = A.get(I, np.zeros(3))
        bv = B.get(I, np.zeros(3))
        pts += np.cos(I * t)[:, None] * av[None, :]
        pts += np.sin(I * t)[:, None] * bv[None, :]
    return pts

def list_ideal_xml_blocks(path: Path, max_knots: int = 50) -> None:
    ...
    for i, (bid, b) in enumerate(blocks.items()):
        if i >= max_knots:
            ...
            break
        status = 'single' if b.get('is_supported_single_component') else f"skip_n={b.get('n_components')}"
        print(f"{i:4d}  Id={bid:12s} Conway={str(b.get('conway','')):8s} L={b.get('L')} D={b.get('D')} max_I={b.get('max_I')} {status}")

def load_knots_from_ideal_xml(path: Path, ids_csv: str, samples: int, max_knots: int) -> Dict[str, np.ndarray]:
    blocks = parse_ideal_xml_fourier_file(path)
    if not blocks:
    ...
    if len(ids) == 1 and ids[0].lower() == 'all':
        for bid, b in blocks.items():
            if b.get('is_supported_single_component'):
        ...
        for bid in ids:
            ...
            if not blocks[bid].get('is_supported_single_component'):
                raise ValueError(
                    f'AB Id={bid!r} is multi-component or has no direct Fourier centerline. '
                    'This BEM Route-B script supports single-component knots only.'
                )
            selected.append((bid, blocks[bid]))
    ...
    for bid, b in selected:
        ...
        while name in out:
            name = f'{base}__{k}'
            k += 1
        out[name] = eval_ideal_fourier_block(b, samples)
    return out
...
def load_knots_from_args(args, outdir: Path):
    ...
    Source policy:
      --source file:
          if --ideal is Brian Gilbert XML/Fourier ideal.txt, sample AB blocks;
          otherwise parse coordinate blocks.
      --source sstcore:
          load ideal Fourier data from SSTcore.
      --source auto:
          use --ideal if present; auto-detect XML/Fourier vs coordinate file;
          otherwise use SSTcore.
    """
    if args.make_demo:
    ...
    if args.list_ideal_xml_knots:
        list_ideal_xml_blocks(ideal, args.sstcore_max_knots)
        raise SystemExit(0)

    if args.source == "file":
        if is_ideal_xml_fourier_file(ideal):
            ids = args.ideal_xml_knot_ids or args.sstcore_knot_ids
            samples = args.ideal_xml_samples or args.sstcore_samples
            knots = load_knots_from_ideal_xml(ideal, ids, samples, args.sstcore_max_knots)
            sampled = outdir / "idealxml_sampled_ideal_used.txt"
            write_sampled_coordinate_file(sampled, knots, f"XML/Fourier ideal.txt={ideal}; ids={ids}; samples={samples}")
            return knots, sampled, f"XML/Fourier file:{ideal}"
        return parse_ideal_file(ideal), ideal, f"coordinate file:{ideal}"
...
def main():
    ...
    ap.add_argument("--ideal-xml-knot-ids", default=None, help="AB Ids for XML/Fourier ideal.txt, e.g. 3:1:1,4:1:1,5:1:1 or all. Defaults to --sstcore-knot-ids.")
    ap.add_argument("--ideal-xml-samples", type=int, default=None, help="raw Fourier samples per XML/Fourier knot. Defaults to --sstcore-samples.")
    ap.add_argument("--list-ideal-xml-knots", action="store_true", help="list AB blocks in XML/Fourier --ideal and exit")
