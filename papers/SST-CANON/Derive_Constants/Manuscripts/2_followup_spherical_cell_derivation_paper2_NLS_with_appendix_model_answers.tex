...
\(\alpha\), \(m_e\), \(\hbar\), \(e\), or \(R_\infty\), but the mode count
\(N_p=4\), the shell coefficient \(11/48\), and the radius closure
\(\chi_R=2\) are model assumptions of the minimal shell closure.  The present
paper therefore supplies a structured pressure-cell/NLS-shell closure, not a
stand-alone first-principles derivation of the electromagnetic fine-structure
constant.  The numerical extraction of the stationary aspect value \(E_\star\)
and the far-field identification are treated as companion results.
\end{abstract}
