...
The BEM-only negative control does not produce an interior aspect scale, so the
stationary point should be interpreted as the NLS pressure scale plus a small
BEM compliance shift, not as an independent spectral discovery.  The far-field
calculation is an internal Green-function consistency check conditional on the
stiffness normalization \(\mathcal K_{\rm cell}=E_{\rm eff}/(8\pi)\).
...
The finite-shell coefficient \(11/48\) is derived only within the unweighted
isotropic GP/NLS shell reduction.  The open microscopic gate is
\(w_\perp=1\).  The far-field Hodge calculation derives
\[
  q_\phi=1,
  \qquad
  \Lambda_\phi=4\pi R\mathcal K_{\rm cell},
\]
but the interior identity \(\Lambda_\phi=E_{\rm eff}/2\) remains open.
Consequently the numerical BEM/NLS stationary point should be read as a
pressure-selected stationary point with a conditional far-field normalization,
not as an independent spectral derivation of \(\alpha\).
...
the conditional Green-function implementation.  The remaining far-field gate is
the interior operator identity \(\Lambda_\phi=E_{\rm eff}/2\), without which
\(\mathcal K_{\rm cell}=E_{\rm eff}/(8\pi R)\) is conditional.


The one-cell phase-Hessian normalization required to make this theorem independent is isolated in Appendix~\ref{app:farfield-stiffness-target}. The appendix proves the exterior Green-function identity \(\Lambda_\phi=4\pi\mathcal K_{\rm cell}\) and states the remaining operator target \(\Lambda_\phi=E_{\rm eff}/2\).
...
multiplicity \(q_\phi=1\) of the leading \(1/r\) exterior phase mode.  What
remains to be independently derived is the operator Hessian value
\(\Lambda_\phi=E_{\rm eff}/2\), not the monopole multiplicity itself.
...
\item an independent one-cell phase-Hessian derivation of
      \(\mathcal K_{\rm cell}=E_{\rm eff}/(8\pi)\).
\end{enumerate}
...
\item obtain \(\Lambda_\phi=E_{\rm eff}/2\) from an independent one-cell
phase-Hessian operator.  The exterior Hodge calculation already gives
\(\Lambda_\phi=4\pi R\mathcal K_{\rm cell}\) and \(q_\phi=1\);
...
\texttt{derive\_gp\_core\_profile\_second\_variation.py}, and
\texttt{solve\_one\_cell\_hodge\_phase\_hessian.py}.  The numerical outputs are
stored in the corresponding \texttt{../code/outputs\_*} directories.
