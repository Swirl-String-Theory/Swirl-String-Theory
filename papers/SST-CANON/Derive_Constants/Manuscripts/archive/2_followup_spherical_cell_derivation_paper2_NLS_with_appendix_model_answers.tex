...
\(\alpha\), \(m_e\), \(\hbar\), \(e\), or \(R_\infty\); they are geometric/NLS inputs for the
companion finite-cell BEM eigenvalue problem.  The present paper therefore supplies the
analytical pressure-cell and NLS-shell closure.  The numerical extraction of the stationary
aspect eigenvalue \(E_\star\), and the far-field identification with the electromagnetic
coupling, are treated as companion results.
\end{abstract}
...
\(16\pi/3\) &
Closure hypothesis &
Derive from a specified spherical pressure-cell variational principle\\
Finite-shell correction &
\(11/(48\mathcal L_K^2)\) &
Closure hypothesis &
Derive from a controlled NLS/GP core-profile or shell expansion\\
Cell radius closure &
\(R_{\rm cell}=2L_K\) &
Closure hypothesis &
Derive from stationarity or include \(\chi_R\)-sensitivity\\
\bottomrule
\end{tabular}
...
\begin{equation}
  ...
  \label{eq:kappaNLS}
\end{equation}
The coefficient \(11/48\) is used here as a finite-shell closure coefficient, not as a fit to \(\alpha\).  At the present stage it should be read as a structured NLS-shell ansatz.  A first-principles derivation would require a controlled GP/NLS core-profile or shell expansion that produces this coefficient without reference to the empirical fine-structure constant.  The stiffness
