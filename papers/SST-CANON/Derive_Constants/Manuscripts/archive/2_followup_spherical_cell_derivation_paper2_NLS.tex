...
\begin{equation}
  ...
  \label{eq:kappaNLS}
\end{equation}
The coefficient \(11/48\) is a finite-shell correction, not a fit to \(\alpha\).  It is the
second-order correction required by the finite tube thickness when the spherical pressure
scale is expressed in terms of \(\mathcal L_K=L_K/D\).  The stiffness
...
\end{align}
These are dimensionless geometric/NLS quantities; no electron mass, Planck constant, charge,
Rydberg constant, or CODATA value is used to determine them.
...
\[
  ...
  \kappaNLS=6736.341149.
\]
These quantities are derived from the ropelength geometry and NLS-shell closure, not from
\(\alpha\), \(m_e\), \(\hbar\), \(e\), or \(R_\infty\).  The stationary aspect eigenvalue
