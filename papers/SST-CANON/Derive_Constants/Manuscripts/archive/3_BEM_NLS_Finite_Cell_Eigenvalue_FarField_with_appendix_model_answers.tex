...
The uniqueness of the \(H^0(S^2)\) far-field phase selection and its role in
the full coefficient chain are proved in Paper~IV.  The present paper keeps the
two-cell calculation separate: it tests the finite-cell stationary point and
the conditional Green-function implementation, while Paper~IV supplies the
minimal variational reduction that fixes \(q_\phi=1\) and
\(\mathcal K_{\rm cell}=E_{\rm eff}/(8\pi)\) within that reduction.
...
\begin{enumerate}
\item derive the shell coefficient \(11/48\) from a controlled GP/NLS
finite-core second variation;
\item obtain \(\Lambda_\phi=E_{\rm eff}/2\) from an independent one-cell
phase-Hessian operator;
\item repeat the pressure-cell pipeline with an \(\ell\le2\) pressure manifold
to test the \(\ell\le1\) cutoff;
