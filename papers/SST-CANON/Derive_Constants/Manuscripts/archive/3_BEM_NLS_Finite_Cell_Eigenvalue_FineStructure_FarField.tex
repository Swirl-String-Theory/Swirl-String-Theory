...
\title{A BEM/NLS Finite-Cell Eigenvalue for the Fine-Structure Scale\\
\large Batch Convergence and a Far-Field Coupling Certificate}
...
\alphacell.
\]
No electron mass, electric charge, Planck constant, Rydberg constant, or CODATA value is used in the derivation of \(\Estar\). Across 16 runs, the model yields \(\Estar=274.074903758\pm 5.51e-05\) and \(\alphacell^{-1}=137.035953090\pm 2.76e-05\). This is strong evidence that the missing aspect scale can be obtained from a finite-cell eigenvalue problem. A final claim that \(\alphacell\) is the electromagnetic fine-structure constant requires the far-field coupling certificate.
...
\section{Single-cell phase-stiffness certificate}
    \label{app:single-cell-phase-stiffness}

    The two-cell far-field calculation establishes the Coulomb form once the
    far-field phase stiffness is fixed.  This appendix gives the missing
    normalization in a one-cell form.

    Let \(u\) denote the massless exterior phase field.  In units of \(\hbar c\),
    write the far-field action as
    \begin{equation}
        \mathcal A_{\rm far}[u]
        =
        ...
        -
        q\,u(\mathbf x_0).
        \label{eq:far-action-single-cell}
    \end{equation}
    For a spherically normalized exterior phase amplitude \(\phi\),
    \begin{equation}
        u(r)=\frac{\phi}{r},
        \qquad r\ge 1 .
    \end{equation}
    ...
    \begin{equation}
        ...
        =
        4\pi\phi^2.
        \label{eq:radial-gradient-integral}
    \end{equation}
    The exterior quadratic action is thus
    \begin{equation}
        \mathcal A_{\rm ext}(\phi)
        =
        2\pi\mathcal K_{\rm cell}\phi^2.
        \label{eq:exterior-quadratic-action}
    \end{equation}
    If the one-cell phase Hessian is defined by
    \begin{equation}
        ...
        +
        O(\phi^4),
        \label{eq:phase-hessian-definition}
    \end{equation}
    matching Eqs.~\eqref{eq:exterior-quadratic-action} and
    \eqref{eq:phase-hessian-definition} gives
    \begin{equation}
        \boxed{\Lambda_\phi=4\pi\mathcal K_{\rm cell}.}
        \label{eq:lambda-K-relation}
    \end{equation}

    The BEM/NLS finite-cell calculation supplies the stationary aspect eigenvalue
    \(E_\star\).  The finite-core corrected aspect is
    \begin{equation}
        E_{\rm eff}
        =
        E_\star
        \left[
            1-\frac{\pi}{4E_\star^2}
            \left(
            ...
            \right)
        \right].
        \label{eq:Eeff-appendix}
    \end{equation}
    The single-cell phase-stiffness certificate is the statement
    \begin{equation}
        \boxed{\Lambda_\phi=\frac{E_{\rm eff}}{2}.}
        \label{eq:phase-certificate}
    \end{equation}
    Combining Eqs.~\eqref{eq:lambda-K-relation} and \eqref{eq:phase-certificate}
    gives
    \begin{equation}
        \boxed{
            \mathcal K_{\rm cell}
        ...
        \label{eq:Kcell-certificate}
    \end{equation}
    The two-cell Green-function interaction in three dimensions is then
    \begin{equation}
        V(r)
        =
        \hbar c\,
        ...
        +
        O(r^{-2}).
        \label{eq:farfield-potential-appendix}
    \end{equation}
    For unit topological charges \(q_1q_2=1\), Eq.~\eqref{eq:Kcell-certificate}
    implies
    \begin{equation}
        V(r)
        =
    ...
    \begin{equation}
        ...
        \label{eq:alpha-from-single-cell-certificate}
    \end{equation}

    In the reduced NLS-shell form the one-cell phase action is
    \begin{equation}
        \mathcal A_\phi(\phi)
        =
        \frac{E_{\rm eff}}{4}\phi^2
        +
        O(\phi^4),
        \label{eq:nls-phase-action}
    \end{equation}
    and therefore
    \begin{equation}
        \left.
            \frac{\partial^2\mathcal A_\phi}{\partial\phi^2}
        \right|_{\phi=0}
        =
        \frac{E_{\rm eff}}{2}.
    \end{equation}
    This is the single-cell phase-stiffness certificate.  Numerically it is tested
    by finite-differencing the one-cell action around \(\phi=0\) and verifying
    \begin{equation}
        \mathcal R_K
        =
        \frac{2\Lambda_\phi}{E_{\rm eff}}
        =
        1
    \end{equation}
    under resolution and regularization sweeps.
