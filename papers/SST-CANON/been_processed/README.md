...
| `v0.8.2/` … `v0.8.18/` | Incremental canon editions (`$out/` = build artifacts) |
...
python apply_v0818.py
```

Individual steps: `apply_v085.py` … `apply_v0818.py`.
...
```powershell
cd papers/SST-CANON/been_processed/v0.8.18
latexmk -pdf -interaction=nonstopmode -output-directory='$out' SST_CANON-v0.8.18.tex
```
...
```powershell
cd papers/SST-CANON/been_processed/v0.8.18
pdflatex -interaction=nonstopmode -output-directory='$out' SST_CANON-v0.8.18.tex
pdflatex -interaction=nonstopmode -output-directory='$out' SST_CANON-v0.8.18.tex
pdflatex -interaction=nonstopmode -output-directory='$out' SST_CANON-v0.8.18.tex
```

Output: `v0.8.18/$out/SST_CANON-v0.8.18.pdf` (auxiliary files stay in `$out/`; source `.tex` files remain in the edition folder).
