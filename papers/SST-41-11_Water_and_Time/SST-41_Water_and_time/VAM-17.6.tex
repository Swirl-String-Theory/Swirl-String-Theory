...
Experiments show per-photon OAM conservation in SPDC: $\ell_p=\ell_s+\ell_i$ via the azimuthal integral and a commuting OAM operator \cite{Kopf2025OAMConservation,Walborn2010SPDCReview}. VAM adopts the same symmetry logic for a knotted source of azimuthal index $\ell_\textrm{src}$:

\begin{equation}
\boxed{\ell_\textrm{src} = \sum_{j=1}^{n} \ell_j \quad \text{for an $n$-photon channel}}
...
\]
This anchors the fundamental scale; higher modes satisfy $E_{m\ell}=\chi_{m\ell}\times 0.510999\ \mathrm{MeV}$ (e.g.\ $\chi=10^{-2}\Rightarrow 5.11\ \mathrm{keV}$). Units are consistent: $[C_{e}/r_{c}]=\textrm{s^{-1}}$, $[\hbar\omega]=\textrm{J}$.

\section{Mapped source classes (examples)}

\begin{itemize}
\item \textbf{Trefoil ($T(2,3)$)-class}: dominant $\ell \simeq 3$ along axis; single-photon Stokes texture with $N_\textrm{sk} = \pm 2$ from the $\mathrm{LG}_{0,\mp 2}^{\sigma\pm} + \mathrm{LG}_{1,0}^{\sigma\mp}$-type superposition (as realized in cavity skyrmions \cite{Ma2025NanoPhotonSkyrmions}); multiphoton channels respect \eqref{eq:oam_additivity}.
...
\paragraph{(P1) Topological spectroscopy.} A trefoil-class emitter yields a biphoton OAM correlation matrix concentrated on $\ell_s+\ell_i=\ell_\textrm{src}\simeq 3$, with radial-index spread governed by waists/overlaps (no $p$ selection) \cite{Walborn2010SPDCReview,Kopf2025OAMConservation}.
...
\paragraph{(P3) Robustness of $N_\textrm{sk}^{(\mathrm{ph})}$.} Attenuators, polarizers, and phase shifters perturb intensity/polarization locally but preserve the integer $N_\textrm{sk}^{(\mathrm{ph})}$ (topological protection) \cite{Ma2025NanoPhotonSkyrmions}.
...
\item \textbf{Readout:} reconstruct single-photon Stokes maps $S_{1,2,3}(x,y)$ and OAM spectra; extract $N_\textrm{sk}^{(\mathrm{ph})}$ and verify \eqref{eq:oam_additivity}.
