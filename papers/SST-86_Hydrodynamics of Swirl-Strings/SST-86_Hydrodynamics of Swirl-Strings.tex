...
Starting from the classical electron radius $r_e = \alpha\hbar/(\me c)$ and identifying $\rc = r_e/2$, the Hooke closure assumes $m_e c^2 = \tfrac{1}{2} k r_e^2$.  The effective spring constant is therefore $k = 2 m_e c^2/r_e^2$.  The maximal swirl force is $F_{\rm swirl}^{\max} = k\,\rc = m_e c^2/(2\rc)$.  The core density can then be expressed as
