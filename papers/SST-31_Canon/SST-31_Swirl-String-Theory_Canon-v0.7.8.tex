...
%! Title = Swirl String Theory (SST) Canon v0.8
%! Date = Dec 26, 2025
%! Affiliation = Independent Researcher, Groningen, The Netherlands
%! License = © 2025 Omar Iskandarani. All rights reserved. This manuscript is made available for academic reading and citation only. No republication, redistribution, or derivative works are permitted without explicit written permission from the author. Contact: info@omariskandarani.com
%! ORCID = 0009-0006-1686-3961
%! DOI = 10.5281/zenodo.18913147

\newcommand{\canonversion}{v0.7.8} % Semantic versioning: vMAJOR.MINOR.PATCH
...
\newcommand{\paperdoi}{10.5281/zenodo.18913147}
...
% ======================================================================
        ...
        % --- Canon Version with Status Tags ---
    \subsection{Canonical constant chain with status tags}
        \label{sec:canon-constants-status}

        We classify each step as:
        \[
            \text{[T]}=\text{textbook identity},\quad
            \text{[S]}=\text{SST closure assumption},\quad
            \text{[D]}=\text{derived consequence}.
        \]
        ...
        \begin{align}
            ...
            && \text{[S]},\\
            \lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\rVert &= \omega_C \rc
            && \text{[D]},\\
            m_e c^2 &= \frac12 k r_e^2
            && \text{[S]},\\
            F_{\text{swirl}}^{\max} &= k\rc=\frac{m_e c^2}{r_e}=\frac{m_e c^2}{2\rc}
            && \text{[D]},\\
        ...
        This notation makes explicit which relations are inherited from standard physics and which are SST-specific closures.
...
% ======================================================================
    ...
    \subsection{The Topological Repulsion Penalty (Pauli Barrier)}
        ...
        The interaction energy evaluates to a massive, localized pseudo-potential:
        \begin{equation}
            \label{eq:VPauli}
        ...
        Thus, the Pauli exclusion principle is not a postulate of SST; it is the direct classical consequence of the fluid kinetic energy integral. Identically handed, identically placed vortex knots repel with an insurmountable dynamic pressure barrier.
...
% ======================================================================
    \section{Atomic and Nuclear Masses from Swirl Energy}
...
% ======================================================================
    ...
    Sections~\ref{sec:mass_expanded}--\ref{sec:knotSelection} establish invariant particle masses in Swirl--String Theory
    ...
    \subsection{Preferred-Frame Effects as Velocity-Dependent Mass}
    \label{sec:canon_preferred_frame_mass}

    In Swirl--String Theory (SST), preferred-frame effects do not correspond to
    additional forces, interaction terms, or violations of momentum conservation.
    Instead, they arise as kinematical corrections to the evaluation of the
    mass functional when a system is in motion relative to the clock--foliation
    field.

    In Canon form, inertial and gravitational mass are defined by an integral
    over the local swirl energy density,
    \begin{equation}
        M \;=\; \mathcal{C} \int_V \rho_{\!E}(\mathbf{x})\, dV ,
    \end{equation}
    where the evaluation of $\rho_{\!E}$ is implicitly referenced to the local
    foliation defined by the unit timelike vector field $u^\mu$.

    For an observer or laboratory moving with velocity $\mathbf{v}$ relative to
    this foliation, the same physical configuration is sampled through a boosted
    clock slicing. As a result, the effective mass entering dynamical observables
    ...
    \begin{equation}
        ...
        \right],
    \end{equation}
    where $\hat{u}$ denotes the spatial direction selected by the clock field and
    $\beta_1,\beta_2$ are dimensionless coefficients fixed by the SST dynamics.

    Accordingly, all preferred-frame observables in the weak-field regime are
    interpreted as corrections to the \emph{definition of mass} rather than as
    new gravitational interactions. In the limit $\beta_1=\beta_2=0$, the SST
    mass functional becomes velocity independent, reproducing the effective
    equivalence of inertial and gravitational mass assumed in General Relativity.

    \begin{theorem}[Electromagnetic Non-Generation of Gravity]
    \label{thm:canon_EM_no_gravity}

    In Swirl--String Theory (SST), electromagnetic fields \emph{cannot} directly
    modify gravitational mass, inertial response, or spacetime curvature.
    Any apparent electromagnetic influence on gravity must proceed
    \emph{indirectly} through the generation of a persistent foliation pressure
    in the underlying medium.

    More precisely, let $\chi$ denote the clock--foliation field and
    $\rho_{\!E}$ the swirl energy density.
    A necessary and sufficient condition for an electromagnetic configuration
    to induce a gravitational effect is the existence of a non-radiative,
    long-lived contribution
    ...
    Electromagnetic energy that is oscillatory, radiative, gauge-removable,
    or time-averaged to zero cannot satisfy this condition and therefore cannot
    generate or modify gravity in SST.
    \end{theorem}
...
For bounded tangential swirl fractions, an optional hyperbolic parameterization
$\beta=\tanh\xi$ and a kinematic ``golden-marker'' choice $\beta_g=\phi^{-1}$
may be used as a notation convenience (not a dynamical postulate); see
Appendix~\ref{app:golden_hyperbolic_parameterization}.
...
% ============================================================
% SST Canon v0.7.7 patch (Section 18): Faraday Rotation bridge
% Recommended placement: after Sec. 18.1, as new Sec. 18.2
% Status: Bridge / Constitutive ansatz (not foundational axiom)
% ============================================================

% --- Optional local macro prelude (remove if already defined globally) ---
    ...
    \subsection{Faraday Rotation in SST (Bridge Model)}
        \label{subsec:sst-faraday-bridge}

        \paragraph{Status.}
            \textbf{Bridge / constitutive ansatz} (not a foundational postulate). This subsection connects standard magneto-optic circular birefringence to the SST cutoff scale
            \[
                \omega_{\max}=\frac{\vswirl}{\rc}.
            \]
            It is dimensionally consistent and reduces to a low-order kinematic estimate in the deep sub-cutoff limit, but quantitative use requires fitting effective material parameters.

        \paragraph{Canonical SST cutoff scale.}
            We identify the characteristic cutoff/resonance scale governing polarization response as
            \begin{equation}
                \omega_{\max}=\frac{\vswirl}{\rc}.
                \label{eq:sst-faraday-omega-max}
            \end{equation}
            This relation is the SST-specific input used below.

        \paragraph{Circular-mode dispersive ansatz (quantitative bridge).}
            For weak-field Faraday rotation in a transparent dielectric regime, define the circularly polarized mode indices by
            \begin{equation}
                n_\pm^2(\omega,B)
                =
                1+\frac{S}{\left(\omega_{\max}\pm \eta \omega_L\right)^2-\omega^2-i\gamma\omega},
                \qquad
                \omega_L=\frac{eB}{2m_e},
                \label{eq:sst-faraday-npm}
            \end{equation}
            where \(S\) is an effective oscillator-strength density parameter, \(\gamma\) is an effective damping rate, and \(\eta\) is a dimensionless SST magneto-coupling factor (\(\eta=1\) is the minimal canonical choice).

            Define
            \begin{equation}
                D(\omega):=\omega_{\max}^2-\omega^2-i\gamma\omega .
                \label{eq:sst-faraday-D}
            \end{equation}
            For \(\omega_L\ll \omega_{\max}\) (linear response in \(B\)),
            \begin{equation}
                \left(\omega_{\max}\pm \eta\omega_L\right)^2
                =
                \omega_{\max}^2 \pm 2\eta\omega_{\max}\omega_L+\mathcal{O}(\omega_L^2),
                \label{eq:sst-faraday-cutoff-expansion}
            \end{equation}
            hence
            \begin{equation}
                n_\pm^2(\omega,B)\approx 1+\frac{S}{D(\omega)\pm 2\eta\omega_{\max}\omega_L}.
                \label{eq:sst-faraday-npm-denom-linearized}
            \end{equation}
            Using \(1/(D\pm \delta)\approx D^{-1}\mp \delta D^{-2}\), we obtain
            \begin{equation}
                n_\pm^2
                \approx
                1+\frac{S}{D}
                \mp
                \frac{2\eta S\omega_{\max}\omega_L}{D^2}.
                \label{eq:sst-faraday-npm-expanded}
            \end{equation}
            Therefore
            \begin{equation}
                \Delta(n^2):=n_-^2-n_+^2
                \approx
                \frac{4\eta S\omega_{\max}\omega_L}{D(\omega)^2}.
                \label{eq:sst-faraday-delta-n2}
            \end{equation}

            For weak circular splitting (\(n_\pm=n_0\pm \delta n/2\)),
            \begin{equation}
                \Delta n:=n_- - n_+
                \approx
                \frac{\Delta(n^2)}{2n_0}
                \approx
                \frac{2\eta S\omega_{\max}\omega_L}{n_0\,D(\omega)^2}.
                \label{eq:sst-faraday-delta-n}
            \end{equation}

        \paragraph{Faraday rotation and Verdet constant.}
            The Faraday rotation angle over length \(L\) is
            \begin{equation}
                \theta=\frac{\omega L}{2c}\,\Re\!\left[\Delta n(\omega,B)\right].
                \label{eq:sst-faraday-theta}
            \end{equation}
            Hence the Verdet constant is
            \begin{equation}
                \mathcal{V}_{\mathrm{SST}}(\omega)
                :=\frac{\theta}{BL}
                =
                \frac{\omega}{2cB}\,\Re\!\left[\Delta n(\omega,B)\right].
                \label{eq:sst-faraday-verdet-def}
            \end{equation}
            Substituting \(\omega_L=eB/(2m_e)\) into Eq.~\eqref{eq:sst-faraday-delta-n} gives the SST dispersive bridge formula:
            \begin{equation}
                \boxed{
                    \mathcal{V}_{\mathrm{SST}}^{\mathrm{disp}}(\omega)
                    \approx
                    \frac{\eta e\,\omega\,S\,\omega_{\max}}{2m_e c\,n_0}\,
                    \Re\!\left[\frac{1}{D(\omega)^2}\right]
                }
                \qquad
                \left(D(\omega)=\omega_{\max}^2-\omega^2-i\gamma\omega\right).
                \label{eq:sst-faraday-verdet-disp}
            \end{equation}

        \paragraph{Transparent, deep sub-cutoff limit (kinematic estimate).}
            If \(\gamma\omega\ll \left|\omega_{\max}^2-\omega^2\right|\), then \(D(\omega)\) is approximately real and
            \[
                D(\omega)\approx \omega_{\max}^2-\omega^2.
            \]
            Using the zero-field relation
            \begin{equation}
                n_0^2-1=\frac{S}{\omega_{\max}^2-\omega^2},
                \label{eq:sst-faraday-zero-field-index}
            \end{equation}
            Eq.~\eqref{eq:sst-faraday-verdet-disp} reduces to
            \begin{equation}
                \mathcal{V}_{\mathrm{SST}}^{\mathrm{disp}}(\omega)
                \approx
                \frac{\eta e\,\omega\,(n_0^2-1)\,\omega_{\max}}
                {2m_e c\,n_0\left(\omega_{\max}^2-\omega^2\right)}.
                \label{eq:sst-faraday-verdet-subcutoff}
            \end{equation}
            In the deep sub-cutoff limit \(\omega\ll \omega_{\max}\),
            \begin{equation}
                \boxed{
                    \mathcal{V}_{\mathrm{SST}}^{\mathrm{kin}}(\omega)
                    \approx
                    \frac{\eta e}{2m_e c}\,
                    ...
                    \frac{\eta e}{2m_e c}\,
                    \frac{\omega(n_0^2-1)}{n_0}\,
                    \left(\frac{\rc}{\vswirl}\right)
                }
                \label{eq:sst-faraday-verdet-kin}
            \end{equation}
            which is the corrected low-order SST kinematic estimate (note the factor \(1/2\)).

        \paragraph{Comparison box (standard vs.\ SST forms).}
            \begin{itemize}
                \item \textbf{Standard Lorentz magneto-optics (schematic):}
                \[
                    \Delta n \propto \frac{\omega\,\omega_L}{\left(\omega_0^2-\omega^2-i\gamma\omega\right)^2},
                    \qquad
                    \mathcal{V}=\frac{\omega}{2cB}\Re[\Delta n].
                \]
                \item \textbf{SST kinematic bridge (low-order):} Eq.~\eqref{eq:sst-faraday-verdet-kin}.
                \item \textbf{SST dispersive bridge (quantitative):} Eq.~\eqref{eq:sst-faraday-verdet-disp}.
            \end{itemize}

        \paragraph{Scope and non-claims.}
            This bridge model is intended for weak-field, transparent dielectric magneto-optics. It does not by itself establish validity in strongly absorptive UV/X-ray/\(\gamma\)-ray regimes, dense plasmas, or neutron-star matter, where the bound-oscillator dielectric approximation may fail.

        \paragraph{Minimal falsifiability checklist.}
            \begin{enumerate}
                \item Fit \(\mathcal{V}(\omega)\) over a broad optical band and test whether the data favor the SST kinematic scaling (Eq.~\eqref{eq:sst-faraday-verdet-kin}) or require the full dispersive form (Eq.~\eqref{eq:sst-faraday-verdet-disp}).
                \item Fit \(\gamma\) and \(\eta\) near absorption features to test the predicted line-shape dependence \(\Re[D(\omega)^{-2}]\).
                \item Compare extracted effective \(\omega_{\max}^{\mathrm{eff}}\) trends across material families against an SST stiffness interpretation.
            \end{enumerate}

        \paragraph{Numerical self-consistency (canonical cutoff).}
            Using canonical SST values
            \[
            ...
            we obtain
            \[
                \omega_{\max}=\frac{\vswirl}{\rc}=7.763440655\times 10^{20}\ \mathrm{rad\,s^{-1}}.
            \]
            For visible light \(\omega\sim 10^{15}\ \mathrm{rad\,s^{-1}}\), one has \(\omega/\omega_{\max}\sim 10^{-6}\), so the deep sub-cutoff expansion is self-consistent.

        \paragraph{Analogy (intuition).}
            A magnetic field makes the medium delay right- and left-handed light by slightly different amounts; over distance, that tiny mismatch accumulates and the polarization direction rotates.
% ======================================================================
...
\paragraph{Bridge-domain application note (delay loops).}
As an application example of the canonical delay-induced mode-selection mechanism of Sec.~\ref{sec:delay_mode_selection},
a condensed-matter pseudogap bridge model based on constrained many-body hydrodynamics and closed-loop delay locking is recorded in Appendix~\ref{app:pseudogap_delay_bridge};
this appendix is illustrative and does not introduce new canonical primitives.
...
For multi-component filament configurations (links, composites, or interacting tube ensembles), the helicity-type contribution in the canonical knot-energy/stability functional may be evaluated using the thin-core diagnostic decomposition and reporting protocol in Appendix~\ref{app:helicity_diagnostics_multicomponent}, unless a direct field-based evaluation of $\mathcal{H}=\int \mathbf{v}\cdot\boldsymbol{\omega}\,d^3x$ is performed.
...
\end{table}
    ...
    \section{Appendix: Topological Classification of SST Vortex Structures}
    \label{sec:knot_topology}

    Within the framework of Subquantum Spatial Tensor (SST) dynamics, the mathematical modeling of physical vortex structures requires a rigorous topological notation. This appendix formally partitions knot theory terminology into structural notations and topological operations, establishing the optimal convention for SST.

    \subsection{Vortex Stretching and Areal Energy Density Scaling}

        Within an incompressible fluid ($\nabla \cdot \mathbf{v} = 0$), a vortex tube of length $L$ and cross-sectional area $A$ maintains a constant volume $V$:
        \begin{equation}
            V = A \cdot L = \text{constant}
        \end{equation}
        According to Kelvin's circulation theorem, the circulation $\Gamma$ along the closed contour of the vortex cross-section is conserved under inviscid deformation:
        \begin{equation}
            \Gamma = \oint \mathbf{v} \cdot d\mathbf{l} = \int_{A} \boldsymbol{\omega} \cdot d\mathbf{A} = \text{constant}
        \end{equation}
        When the topological tension of a particle (e.g., the repulsion within a Hopf link $2^2_1$ or an internally twisted Unknot $0_1$) stretches the vortex tube longitudinally by a dimensionless stretch factor $\lambda$, we have:
        \begin{equation}
            L' = \lambda L \quad \implies \quad A' = \frac{A}{\lambda}
        \end{equation}
        Because the circulation $\Gamma$ is conserved, the local vorticity $\omega$ must increase linearly with the stretch:
        \begin{equation}
            \omega' = \frac{\Gamma}{A'} = \lambda \frac{\Gamma}{A} = \lambda \omega
        \end{equation}

        The total kinetic vortex energy $E$ of the filament is proportional to its length $L$:
        \begin{equation}
            E \approx \frac{\rho_{\!f} \Gamma^2 L}{4\pi} \ln\left(\frac{R}{r_c}\right) \implies E' = \lambda E
        \end{equation}
        When the invariant mass of the particle is evaluated as a projection of the energy density over the cross-sectional area (areal energy density $\Sigma_A = E / A$), a quadratic scaling emerges:
        \begin{equation}
            \Sigma_A' = \frac{E'}{A'} = \frac{\lambda E}{A / \lambda} = \lambda^2 \left(\frac{E}{A}\right) = \lambda^2 \Sigma_A
        \end{equation}

        For heavy bosons (such as the $Z$ boson at $\sim 91 \text{ GeV}$), where the initial C++ calculation projected an unconstrained Horn Torus at $\sim 909 \text{ GeV}$, the observed factor of $10$ represents exactly the inverse of this vortex stretching mechanism. A reduction in effective areal energy density by a factor of $\approx 10$ requires a geometric stretching of the vortex by $\lambda = \sqrt{10} \approx 3.16$.

        This proves that bosons in the Standard Model are not static, undeformed rings, but systems where an internal transversal twist or multi-component repulsion (links) stretches the vortex tubes by a factor of $\sim 3.16$, which dampens the hydrodynamic mass by exactly one order of magnitude \cite{Chorin1994}.

    \subsection{Taxonomy of Knot Notations}

        \textbf{Conway Notation} is the standard notation for SST. It decomposes knots into algebraic combinations of ``tangles'' (localized regions containing intertwined flux lines). In the SST fluid model, characterized by an effective fluid density $\rho_{\!f} = 7.0 \times 10^{-7} \text{ kg m}^{-3}$, a tangle maps rigorously to interacting regions of vortex tubes governed by the characteristic swirl speed $\mathbf{v}_{\!\boldsymbol{\circlearrowleft}} = 1.09384563 \times 10^6 \text{ m s}^{-1}$. Tangle algebra physically represents the recombination, linking, or twisting of these tubes.

        Conversely, \textbf{Dowker--Thistlethwaite Notation} generates a numerical sequence of even integers representing crossings; while optimal for algorithmic tabulation, it obscures the geometric fluid state. \textbf{Alexander--Briggs Notation} acts merely as a historical index (e.g., $3_1$ for the trefoil) and provides no structural insight into swirl energy density $\rho_{\!E}$.

    \subsection{Topological Operations and Invariants}

        The following terms define transformations and analytical methods rather than static notations:
        \begin{itemize}
            \item \textbf{Reidemeister Moves:} Three fundamental local deformations. Physically, these correspond to continuous, topology-preserving fluid diffeomorphisms where $\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}$ field lines bend without breaking, preserving invariants like circulation and helicity.
            \item \textbf{Skein Relation:} A recursive algebraic relation mapping topologically discrete state changes (e.g., crossing switches) to conserved physical quantities, such as discrete quanta of mass-equivalent density $\rho_{\!m} = \rho_{\!E}/c^2$.
            \item \textbf{Flype and Mutation:} Specific rigid isotopic transformations representing vortex reconnection events that alter macroscopic embedded geometry while preserving fundamental structural invariants.
            \item \textbf{Tabulation:} The algorithmic classification of knots using invariants derived from the aforementioned notations.
        \end{itemize}

    \subsection{Derivation of Topologically Conserved Helicity}

        To physically ground Conway notation within SST, we map tangle topology to the kinematic fluid helicity $\mathcal{H}$, which is conserved under ideal inviscid conditions.

        Consider an incompressible background fluid ($\rho_{\!f}$) and a localized knotted vortex tube $K$ carrying circulation $\Gamma$. The vorticity field is $\boldsymbol{\omega} = \nabla \times \mathbf{v}$. The total fluid helicity is:
        \begin{equation}
            \mathcal{H} = \int_{V} \mathbf{v} \cdot \boldsymbol{\omega} \, d^3x
        \end{equation}

        For a thin vortex tube parameterized by core radius $r_c = 1.40897017 \times 10^{-15} \text{ m}$, the vorticity is localized. We decompose the spatial integral into a longitudinal line integral along the knot's central axis $C$ and an area integral over the cross-section $S$:
        \begin{equation}
            \mathcal{H} = \oint_{C} \iint_{S} \mathbf{v} \cdot \boldsymbol{\omega} \, dS \, dl
        \end{equation}

        Since $\boldsymbol{\omega}$ is parallel to $d\mathbf{l}$ within the core, $\iint_S \boldsymbol{\omega} \cdot d\mathbf{S} = \Gamma$. Thus:
        \begin{equation}
            \mathcal{H} = \Gamma \oint_{C} \mathbf{v} \cdot d\mathbf{l}
        \end{equation}

        Applying the Biot-Savart law and the C\u{a}lug\u{a}reanu-White-Fuller theorem, the total helicity of the singular knot splits into the Linking number ($Lk$) of the vortex lines, which is the sum of the Writhe ($Wr$, macroscopic coiling) and the Twist ($Tw$, internal rotation of $\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}$ within the tube):
        \begin{equation}
            Lk = Wr + Tw
        \end{equation}
        \begin{equation}
            \mathcal{H} = \Gamma^2 Lk = \Gamma^2 (Wr + Tw)
        \end{equation}

        Algebraic operations on Conway tangles altering local crossing numbers directly map to integer steps in $\mathcal{H}$ through discrete topological transitions in $Lk$.


    \subsection{Hyperbolic Parameterization of Bounded Tangential Swirl Fractions}
    \label{app:golden_hyperbolic_parameterization}

    \statusCanonical\ This subsection records a canon-permitted \emph{kinematic}
    parameterization for bounded tangential swirl fractions and an optional
    golden-marker coordinate choice. It is a notation/normalization aid and
    does not by itself constitute a dynamical law, stability criterion, or
    spectral prediction.

    \paragraph{Bounded tangential fraction.}
    Define the dimensionless tangential fraction
    \begin{equation}
        \beta \;\equiv\; \frac{\lVert \mathbf{v} \rVert}{\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}} \rVert},
        \qquad 0 \le \beta < 1,
    \end{equation}
    where $\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}} \rVert$ is the canonical
    characteristic swirl speed and $\lVert \mathbf{v} \rVert$ is the local/sector speed
    being expressed as a bounded fraction.

    \paragraph{Hyperbolic coordinate map.}
    A bijective bounded-coordinate parameterization is
    \begin{equation}
        \beta \;=\; \tanh \xi,
        \qquad
        \xi \;=\; \operatorname{artanh}(\beta),
        \label{eq:beta_tanh_xi_appendix}
    \end{equation}
    with the correct limits $\xi\to 0 \Rightarrow \beta\to 0$ and
    $\xi\to \infty \Rightarrow \beta\to 1^{-}$.

    \paragraph{Optional golden-marker coordinate.}
    Let $\phi=(1+\sqrt{5})/2$ denote the golden ratio. Define
    \begin{equation}
        \xi_g \;\equiv\; \frac{3}{2}\,\operatorname{asinh}\!\left(\frac{1}{2}\right).
        \label{eq:xi_g_def_appendix}
    \end{equation}
    Then one has the identity
    \begin{equation}
        \tanh(\xi_g) \;=\; \phi^{-1}.
        \label{eq:tanh_xig_phi_appendix}
    \end{equation}
    This permits the optional shorthand
    \begin{equation}
        \beta_g \;\equiv\; \phi^{-1}
        \qquad\Longleftrightarrow\qquad
        \xi=\xi_g
    \end{equation}
    as a distinguished \emph{kinematic marker} in the hyperbolic coordinate.

    \paragraph{Derived kinematic scales (normalization only).}
    If one chooses to evaluate the marker against the canonical constants
    $\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}} \rVert$ and $r_c$, the associated
    speed and angular-frequency scales are
    \begin{equation}
        \lVert \mathbf{v} \rVert_g \;=\; \frac{\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}} \rVert}{\phi},
        \qquad
        \Omega_g \;=\; \frac{\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}} \rVert}{\phi\,r_c}.
        \label{eq:golden_marker_scales_appendix}
    \end{equation}
    These are \emph{dimensional/kinematic} normalization scales only. They are not,
    by themselves, a prediction of resonant frequencies, layer occupation, or
    dynamical selection without a sector-specific SST model.

    \paragraph{Canonical usage note.}
    \statusResearch\ The golden-marker parameterization may be used in DSI/golden-layer
    discussions as a bounded-coordinate convenience. Any claim that it determines
    a universal exponent, a preferred dynamical layer, or a measurable resonance
    requires separate derivation and quantitative falsifiers.


    \subsection{Canonical Helicity Diagnostics for Multi-Component Swirl Strings}
    \label{app:helicity_diagnostics_multicomponent}

    The preceding derivation establishes the single-tube relation between kinematic helicity and topological winding data in the thin-core ideal limit. For practical SST computations involving links or multi-component filament assemblies, we record here a canonical \emph{diagnostic} decomposition and reporting procedure. This subsection is included as a topology-support module for numerical work and for evaluation of helicity-type terms in the knot-energy/stability sector; it does not replace the canonical mass functional and does not by itself fix coupling coefficients.

    \paragraph{Assumptions (diagnostic regime).}
    The formulas below are used in the incompressible, inviscid, thin-core filament regime, with disjoint components and no net helicity flux through the chosen control volume. Exact conservation can fail under non-ideal reconnection dynamics; in such cases, direct field evaluation or corrected coarse-grained diagnostics should be preferred.

    \paragraph{Field definition.}
    The kinematic helicity is
    \begin{equation}
        \mathcal{H} \;=\; \int_V \mathbf{v}\cdot\boldsymbol{\omega}\,d^3x,
        \qquad
        \boldsymbol{\omega}=\nabla\times\mathbf{v}.
    \end{equation}

    \paragraph{Multi-component thin-core decomposition.}
    Consider \(N\) oriented, disjoint filament components indexed by \(i=1,\dots,N\), each carrying circulation \(\Gamma_i\). Let \(SL_i\) denote the self-linking number assigned to component \(i\) (with a stated framing convention), and let \(Lk_{ij}\) denote the Gauss linking number between components \(i\) and \(j\). In the thin-core regime, a practical helicity estimator is
    \begin{equation}
        \mathcal{H}_{\mathrm{est}}
        \;\approx\;
        \sum_{i=1}^{N}\Gamma_i^{\,2}SL_i
        \;+\;
        \sum_{1\le i<j\le N}2\,Lk_{ij}\,\Gamma_i\Gamma_j.
        \label{eq:helicity_multicomponent_decomp}
    \end{equation}
    For framed closed curves, one may decompose
    \begin{equation}
        SL_i = Tw_i + Wr_i,
        \label{eq:helicity_selflinking_split}
    \end{equation}
    where \(Tw_i\) is twist and \(Wr_i\) is writhe (with convention-dependent assignment that should be reported explicitly).

    If all components share a common circulation \(\Gamma_i=\Gamma\), then
    \begin{equation}
        \mathcal{H}_{\mathrm{est}}
        \;\approx\;
        \Gamma^2\!\left(
        \sum_{i=1}^{N}SL_i
        +2\sum_{1\le i<j\le N}Lk_{ij}
        \right).
        \label{eq:helicity_multicomponent_decomp_hom}
    \end{equation}

    \paragraph{Canonical role in the knot-energy/stability sector.}
    This subsection provides a computational realization for helicity-type contributions in the canonical knot-energy/stability functional,
    \begin{equation}
        E_{\mathrm{eff}}[K] = \alpha\,C(K)+\beta\,L(K)+\gamma\,\mathcal{H}(K),
    \end{equation}
    in the multi-component thin-core regime. It is a diagnostic/evaluation rule and reporting convention, not an independent replacement of the canonical mass kernel or a standalone derivation of particle stability.

    \paragraph{Normative reporting procedure (for reproducibility).}
    For any reported \(\mathcal{H}_{\mathrm{est}}\), list at minimum:
    \begin{enumerate}
        \item component orientations and indexing convention;
        \item circulations \(\Gamma_i\) (including estimation method/uncertainty);
        \item pairwise linking numbers \(Lk_{ij}\);
        \item framing convention used for \(SL_i\), or separate \(Tw_i,Wr_i\) values;
        \item discretization/embedding method and numerical resolution;
        \item uncertainty band for \(\mathcal{H}_{\mathrm{est}}\).
    \end{enumerate}

    \paragraph{Research-track variational note (not canonical spine).}
    A constrained variational ansatz of the form
    \begin{equation}
        \delta\!\left(E-\lambda\mathcal{H}\right)=0
    \end{equation}
    may be used as a bridge heuristic for helicity-constrained relaxation studies. In Canon v0.7.8 this is retained as research-track guidance only; it is not adopted here as a canonical derivation of relativistic stability, mass hierarchy, or persistence without additional SST-specific closure, calibration, and falsifier links.

    \subsection{Physical Analogy}
        Consider Conway notation as the structural chemical formula for a molecule, where ``tangles'' are functional groups snapping together. Alexander-Briggs is merely the common name (like ``water''), and Dowker-Thistlethwaite is a database registry number. Reidemeister moves represent bending or twisting the molecule without breaking any chemical bonds.

        \begin{thebibliography}{9}
            \bibitem{Moffatt1992} Moffatt, H. K., \& Ricca, R. L. (1992). Helicity and the C\u{a}lug\u{a}reanu invariant. \textit{Proceedings of the Royal Society of London. Series A: Mathematical and Physical Sciences}, 439(1906), 411-429. DOI: 10.1098/rspa.1992.0159
            \bibitem{Kauffman2001} Kauffman, L. H. (2001). \textit{Knots and Physics} (3rd ed.). World Scientific. DOI: 10.1142/4349
        \end{thebibliography}
...
% =====================================================================
    ...
    \section*{Appendix Y: Quark masses and CKM mixing as tunneling overlaps in the SST master formula}

    \subsection*{Y.1 SST master mass kernel and quark-sector bookkeeping}

        We extend the SST master mass formula (used for $e,\mu,\tau$) to quark effective masses
        $q\in\{u,d,s,c,b,t\}$ by writing
        \begin{equation}
            M_q \;=\; K_M\,V_q\,\xi_q,
            \qquad
            K_M \equiv \frac{1}{\varphi}\,\frac{4}{\alpha}\left(\frac12\,\rho_{\text{core}}\,\frac{\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\rVert^2}{c^2}\right).
            \label{eq:KM_kernel}
        \end{equation}
        As in the lepton sector, the kernel $K_M$ is universal; flavor dependence is delegated to
        an effective localization volume $V_q$ and a dimensionless structure/dressing factor $\xi_q$.
        For quarks, $M_q$ should be interpreted as an \emph{effective} inertial parameter (since quark masses
        are scheme- and scale-dependent in the SM).

    \subsection*{Y.2 Why a CKM-like matrix is structurally unavoidable}

        In the SM, charged-current weak interactions couple left-handed quark doublets.
        Diagonalizing the up- and down-type mass matrices yields a mismatch matrix
        $V_{\text{CKM}}$:
        \begin{equation}
            \mathcal{L}_{\text{CC}}
            \;=\;
            -\frac{g}{\sqrt{2}}\,
            \overline{u}_{L}\gamma^\mu V_{\text{CKM}}\, d_{L}\,W^+_\mu + \text{h.c.}
            \label{eq:SM_CKM_CC}
        \end{equation}
        with $u_L=(u_L,c_L,t_L)^T$ and $d_L=(d_L,s_L,b_L)^T$.

        In SST language, two bases are generically distinct:
        (i) an \emph{inertial basis} defined by extrema of an SST effective energy functional
        (which defines $M_q$ via \eqref{eq:KM_kernel}),
        and (ii) an \emph{interaction/tunneling basis} defined by which configurations are connected by the charged-current process.
        A nontrivial mixing matrix is the generic consequence.

    \subsection*{Y.3 SST definition of the mixing matrix from a tunneling operator}

        Let $\{|u_i\rangle\}$ and $\{|d_j\rangle\}$ be inertial eigenstates (mass-extremizing states)
        for up-type ($i=1,2,3$ corresponding to $u,c,t$) and down-type ($j=1,2,3$ corresponding to $d,s,b$).
        Let $\widehat{T}$ denote the SST charged-current tunneling operator that connects down-type to up-type sectors.
        Define the normalized overlap amplitudes:
        \begin{equation}
            V^{(\text{SST})}_{ij}
            \;\equiv\;
            \frac{\langle u_i|\widehat{T}|d_j\rangle}{\sqrt{\sum_{k=1}^3 |\langle u_i|\widehat{T}|d_k\rangle|^2}},
            \qquad
            \sum_{j=1}^3 |V^{(\text{SST})}_{ij}|^2=1.
            \label{eq:SST_CKM_def}
        \end{equation}
        If $\widehat{T}$ is effectively isometric on the three-dimensional subspace spanned by $\{|d_j\rangle\}$,
        then $V^{(\text{SST})}$ is approximately unitary; deviations signal leakage into additional channels.

    \subsection*{Y.4 An empirical ``distance matrix'' from the measured CKM hierarchy}

        To instantiate a concrete SST model without committing to a specific knot dictionary,
        we define the \emph{empirical tunneling distances} directly from the magnitudes:
        \begin{equation}
            D^{(\text{obs})}_{ij} \;\equiv\; -\ln |V_{ij}|,
            \label{eq:obs_distance_def}
        \end{equation}
        using the PDG global-fit CKM magnitudes (three-generation unitarity).
        With the PDG 2025 fit values \cite{PDG2025_CKM},
        \begin{equation}
            |V_{\text{CKM}}| \approx
            \begin{pmatrix}
            ...
            \end{pmatrix},
            \label{eq:PDG2025_CKM_matrix}
        \end{equation}
        one obtains numerically
        \begin{equation}
            D^{(\text{obs})} \approx
            \begin{pmatrix}
                0.0260 & 1.4916 & 5.5908 \\
                1.4922 & 0.0269 & 3.1741 \\
                4.7583 & 3.1915 & 0.000882
            \end{pmatrix}.
            \label{eq:PDG2025_distance_matrix}
        \end{equation}
        Interpretation: $(u\!\leftrightarrow\!d)$ and $(c\!\leftrightarrow\!s)$ are ``short-distance'' transitions
        ($D\sim 0.03$), whereas $(u\!\leftrightarrow\!b)$ and $(t\!\leftrightarrow\!d)$ are ``long-distance''
        ($D\gtrsim 4.7$--$5.6$).

        The PDG Wolfenstein parameters further quantify the hierarchy \cite{PDG2025_CKM}:
        \begin{equation}
            \lambda = 0.22501\pm0.00068,\quad
            A = 0.826^{+0.016}_{-0.015},\quad
            \bar{\rho}=0.1591\pm0.0094,\quad
            \bar{\eta}=0.3523^{+0.0073}_{-0.0071},
            \label{eq:PDG2025_wolf}
        \end{equation}
        and the Jarlskog invariant is
        \begin{equation}
            J = \left(3.12^{+0.13}_{-0.12}\right)\times 10^{-5}.
            \label{eq:PDG2025_J}
        \end{equation}

    \subsection*{Y.5 SST ansatz: distances as weighted gaps of SST invariants}

        We now propose an SST-computable model for $D_{ij}$ in terms of invariant gaps between the
        up-type and down-type configuration classes:
        \begin{equation}
            D^{(\text{SST})}_{ij}
            \;=\;
            \kappa_C\,\big|\Delta C_{ij}\big|
            +\kappa_L\,\big|\Delta L_{ij}\big|
            +\kappa_H\,\big|\Delta \mathcal{H}_{ij}\big|
            +\kappa_G\,\big|\Delta G_{ij}\big|.
            \label{eq:SST_distance_ansatz}
        \end{equation}
        Here $C$ is a complexity proxy (e.g.\ crossing/contact number or near-contact functional),
        $L$ is an effective filament length proxy,
        $\mathcal{H}$ is a helicity/linkedness proxy,
        and $G$ is a ``generation'' or layering index (an SST coarse-grained label capturing why $1\!\leftrightarrow\!2$
        mixing is parametrically larger than $1\!\leftrightarrow\!3$).
        The coefficients $\kappa_{C,L,H,G}>0$ encode the tunneling stiffness of each invariant channel.

        Given \eqref{eq:SST_distance_ansatz}, the SST mixing magnitudes are predicted by
        \begin{equation}
            |V^{(\text{SST})}_{ij}|
            \;=\;
            \frac{e^{-D^{(\text{SST})}_{ij}}}{\sqrt{\sum_{k=1}^3 e^{-2D^{(\text{SST})}_{ik}}}}.
            \label{eq:SST_V_from_D}
        \end{equation}

        \paragraph{Calibration strategy.}
            A minimal (and falsifiable) calibration is:
            \begin{itemize}
                \item choose an explicit SST knot/dictionary that assigns $(C,L,\mathcal{H},G)$ to each of
                $\{u,c,t\}$ and $\{d,s,b\}$;
                \item determine $\kappa_{C,L,H,G}$ by matching three independent magnitudes
                (e.g.\ $|V_{us}|$, $|V_{cb}|$, $|V_{ub}|$), then predict the remaining six;
                \item check whether the predicted $J$ (computed from $V^{(\text{SST})}$) agrees in order of magnitude with \eqref{eq:PDG2025_J}.
            \end{itemize}

    \subsection*{Y.6 CP phase as geometric holonomy (optional extension)}

    Beyond magnitudes, CP violation requires a rephasing-invariant complex phase.
    In SST, a natural source is a geometric (path-dependent) phase accumulated along inequivalent
    tunneling trajectories $\Gamma_{ij}$ in configuration space:
    \begin{equation}
        \theta_{ij} \;=\; \int_{\Gamma_{ij}} \mathcal{A}\cdot d\ell,
        \qquad
        V^{(\text{SST})}_{ij} = |V^{(\text{SST})}_{ij}|\,e^{i\theta_{ij}}.
        \label{eq:SST_holonomy_phase}
    \end{equation}
    A direct falsifier is whether the induced Jarlskog invariant
    \begin{equation}
        J_{\text{SST}}
        \equiv
        \mathrm{Im}\!\left(
                         V_{ud}\,V_{cs}\,V_{us}^\ast\,V_{cd}^\ast
        \right)
        \label{eq:SST_Jarlskog}
    \end{equation}
    can reproduce \eqref{eq:PDG2025_J} while respecting the hierarchy encoded in \eqref{eq:PDG2025_distance_matrix}.

    \paragraph{Child-level analogy.}
        Think of three ``easy swaps'' and three ``hard swaps'' between two sets of complicated knots:
        the CKM table is just the scoreboard of how hard each swap is, and the distances $D_{ij}$ measure that hardness.
        ...
        \begin{thebibliography}{9}
            ...
            C.~Jarlskog,
            \textit{Commutator of the Quark Mass Matrices in the Standard Electroweak Model and a Measure of Maximal CP Violation},
            Phys.\ Rev.\ Lett.\ \textbf{55}, 1039 (1985),
            doi:10.1103/PhysRevLett.55.1039.

            \bibitem{PDG2025_CKM}
            S.~Navas \textit{et al.} (Particle Data Group),
            \textit{CKM Quark-Mixing Matrix},
            Phys.\ Rev.\ D \textbf{110}, 030001 (2024) and 2025 update (PDG 2025 review),
            (see the PDG 2025 CKM review PDF).

        \end{thebibliography}

        % =========================
% Appendix: Charged-lepton specialization of the SST master mass law
% (Canon v0.6.0 + conceptual link to Canon v0.7.6)
% =========================

        \appendix
    \section{Charged-lepton specialization of the SST invariant mass law}
    \label{app:leptons_master_mass}

% --- SST house macros (minimal, to avoid undefined commands) ---
    \newcommand{\rhoF}{\rho_{\!f}}
    \newcommand{\rhoE}{\rho_{\!E}}
    \newcommand{\rhoM}{\rho_{\!m}}
    \newcommand{\rc}{r_c}

    \subsection{From the canonical mass definition to the topology-driven master formula}

        Canon v0.7.6 defines rest mass as core-localized kinetic swirl energy,
        \begin{equation}
            M(K)c^2 \equiv \int_{V_K}\frac12\,\rho_{\text{core}}\,|\mathbf v(\mathbf r)|^2\,d^3r,
            \label{eq:mass_core_integral}
        \end{equation}
        with a standard slender-filament reduction yielding a one-dimensional functional
        \begin{equation}
            M(K)\;\approx\;\frac{1}{2c^2}\,\rho_{\text{core}}\,\Gamma^2\,L(K)\,\Xi(K),
            \label{eq:mass_slender_reduction}
        \end{equation}
        where $L(K)$ is a (dimensionless) ropelength and $\Xi(K)$ summarizes geometry/core-profile effects.

        Canon v0.6.0 adopts an explicit topology-driven closure for this sector by writing the mass as an invariant
        energy functional $E(K)=u\,V(K)\,\Xi_{\text{top}}(K)$ with
        \begin{equation}
            u=\frac12\,\rho_{\text{core}}\,\|\vswirl\|^2,
            \qquad
            V(K)=\pi \rc^3\,L_{\text{tot}}(K),
            \qquad
            M(K)=\frac{E(K)}{c^2},
            \label{eq:mass_kernel_uv}
        \end{equation}
        and a discrete multiplier
        \begin{equation}
            \Xi_{\text{top}}(K)=\frac{4}{\alpha_\textrm{fs}}\,b^{-3/2}\,\varphi^{-g}\,n^{-1/\varphi},
            \label{eq:Xi_top}
        \end{equation}
        where $\alpha_\textrm{fs}$ is the fine-structure constant, $\varphi=(1+\sqrt5)/2$ is the golden ratio,
        and $(b,g,n)\in\mathbb Z$ are Canon topology labels (components/link count, layer index, and circulation quantum index).

        Collecting factors gives the Canon v0.6.0 invariant mass law used in the code:
        \begin{equation}
            M(K)=
            \left(\frac{4}{\alpha_\textrm{fs}}\right)
            b^{-3/2}\,\varphi^{-g}\,n^{-1/\varphi}\;
            \frac{u\,\pi \rc^3}{c^2}\;L_{\text{tot}}(K),
            \qquad
            u=\frac12\rho_{\text{core}}\|\vswirl\|^2.
            \label{eq:master_mass_law}
        \end{equation}

    \subsection{Charged leptons: inversion to solve $L_{\text{tot}}$ for $e,\mu,\tau$}

        In the Canon taxonomy, charged leptons correspond to torus-knot classes.
        For a charged lepton $\ell\in\{e,\mu,\tau\}$ with assigned labels $(b_\ell,g_\ell,n_\ell)$ and measured mass $M_\ell^{(\mathrm{\exp})}$,
        invert Eq.~\eqref{eq:master_mass_law} to obtain the implied ropelength:
        \begin{equation}
            L_{\text{tot}}^{(\ell)}=
            \frac{M_\ell^{(\mathrm{\exp})}c^2}{
                \left(\frac{4}{\alpha_{\rm fs}}\right)
                b_\ell^{-3/2}\,\varphi^{-g_\ell}\,n_\ell^{-1/\varphi}\;
                u\,\pi \rc^3 }.
            \label{eq:Ltot_inversion}
        \end{equation}

        It is often convenient to absorb all discrete labels into a single dimensionless factor
        \begin{equation}
            \mathcal{G}_\ell \;\equiv\;
            b_\ell^{-3/2}\,\varphi^{-g_\ell}\,n_\ell^{-1/\varphi}\,L_{\text{tot}}^{(\ell)}.
            \label{eq:Gell_def}
        \end{equation}
        Then the charged-lepton mass spectrum reduces to
        \begin{equation}
            M_\ell=
            \left(\frac{4}{\alpha_{\rm fs}}\right)\frac{u\,\pi \rc^3}{c^2}\;\mathcal{G}_\ell,
            \qquad
            u=\frac12\rho_{\text{core}}\|\vswirl\|^2.
            \label{eq:lepton_mass_Gell}
        \end{equation}

    \subsection{Numerical specialization (canonical constants; extracting $\mathcal{G}_\ell$)}

        Using the canonical constants $\|\vswirl\|=1.09384563\times10^6\,\mathrm{m/s}$,
        $\rc=1.40897017\times10^{-15}\,\mathrm{m}$,
        $\rho_{\text{core}}=3.8934358266918687\times10^{18}\,\mathrm{kg/m^3}$,
        and $c=299792458\,\mathrm{m/s}$, define the universal lepton prefactor
        \begin{equation}
            A \;\equiv\; \left(\frac{4}{\alpha_{\rm fs}}\right)\frac{u\,\pi \rc^3}{c^2}
            \;=\;\left(\frac{4}{\alpha_{\rm fs}}\right)\frac{\left(\frac12\rho_{\text{core}}\|\vswirl\|^2\right)\pi \rc^3}{c^2}.
            \label{eq:A_prefactor}
        \end{equation}
        Then $\mathcal{G}_\ell = M_\ell/A$ follows immediately from experimental masses.

        For illustration, taking representative lepton masses
        $m_e c^2\simeq 0.51099895\,\mathrm{MeV}$,
        $m_\mu c^2\simeq 105.6583755\,\mathrm{MeV}$,
        $m_\tau c^2\simeq 1776.93\,\mathrm{MeV}$,
        one obtains dimensionless values
        \begin{equation}
            \mathcal{G}_e \approx 1.18\times 10^{-2},\qquad
            \mathcal{G}_\mu \approx 2.44,\qquad
            \mathcal{G}_\tau \approx 4.11\times 10^{1},
            \label{eq:Gell_numbers}
        \end{equation}
        and hence the measured hierarchy is encoded entirely in $\mathcal{G}_\ell$.

    \subsection{What this buys you: a clean target for ``generation physics'' in SST}

        Equation~\eqref{eq:lepton_mass_Gell} shows that explaining the charged-lepton spectrum in SST reduces to
        explaining the discrete (knot-selection) values of $\mathcal{G}_\ell$.
        Two immediately falsifiable model-completions are:

        \begin{enumerate}
            \item \textbf{Fixed topology, varying layer:} $b_\ell,n_\ell$ fixed across generations, $g_\ell$ varies
            (e.g. a discrete-scale layer index), with $L_{\text{tot}}$ remaining $\mathcal O(1)$.
            This predicts near-log-linear structure of $\log M_\ell$ in the layer index.
            \item \textbf{Fixed layer, varying ropelength/geometry:} $g_\ell$ fixed and the hierarchy is attributed to
            discrete jumps in $L_{\text{tot}}$ (or, equivalently, in the v0.7.6 $\Xi(K)$ factor).
            This predicts that numerical minimization of the knot-energy functional selects torus-knot embeddings with
            large, discrete ropelength separations between $(3_1)$-like, $(\cdot)$-like higher torus families.
        \end{enumerate}

        Either way, the generation problem becomes a \emph{knot-selection} problem: compute the stable minima of
        $E_{\rm eff}[K]$ and read off the discrete $\mathcal{G}_\ell$ values.

    \subsection*{A child-level analogy (one sentence)}
        Think of three knots tied in the same kind of string: they all use the same string material, but the muon- and tau-knots
        are ``tied in ways that trap much more swirling motion,'' so they weigh more.

% -------------------------
% Bibliography entries (appendix-local)
% -------------------------
        \begin{thebibliography}{9}

            \bibitem{ATLAS_Hmumu_PRL2025}
            ATLAS Collaboration (2025),
            \textit{Evidence for the Dimuon Decay of the Higgs Boson in pp Collisions with the ATLAS Detector},
            Phys.\ Rev.\ Lett.\ (uploaded PRL PDF in this project).

            \bibitem{SST_Canon_v06}
            O.~Iskandarani (2025),
            \textit{Swirl String Theory (SST) Canon v0.6.0: Core Axioms, Postulates, Constants, Master Equations, and Lagrangian Framework},
            (Internal manuscript / project PDF).

            \bibitem{SST_Canon_v076}
            O.~Iskandarani (2025),
            \textit{Swirl String Theory (SST) Canon v0.7.6},
            (Internal manuscript / project PDF).
...
\end{itemize}
    ...
    \subsection{Dispersive SST refinement of Faraday rotation (quantitative model)}
        
        The kinematic estimate derived above is useful as an SST intuition bridge, since it makes the dependence on the canonical swirl stiffness scale explicit through
        \[
            \omega_{\max}=\frac{\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}}{r_c}.
        \]
        However, for quantitative comparison with magneto-optic data, one must retain the full dispersive structure (including damping and polarization-resolved resonance splitting). We therefore introduce a minimal SST dispersive model.
        
        \paragraph{Assumptions (transparent dielectric regime).}
            We assume:
            (i) a weak applied magnetic field \(B\) (linear response),
            (ii) a dilute/weakly damped effective oscillator response for the circulation-loop background,
            (iii) propagation sufficiently far from strong absorption bands,
            (iv) circular birefringence generated by a polarization-dependent shift of the SST cutoff scale.
        
        \paragraph{SST circular-mode dispersion.}
            Let
            \[
                \omega_L=\frac{eB}{2m_e}
            \]
            denote the Larmor frequency. We model the right/left circularly polarized modes by
            \[
                n_\pm^2(\omega,B)
                =
                1+\frac{S}{(\omega_{\max}\pm \eta \omega_L)^2-\omega^2-i\gamma\omega},
            \]
            where:
            \begin{itemize}
                \item \(S\) is an effective oscillator-strength density parameter,
                \item \(\gamma\) is an effective damping rate,
                \item \(\eta\) is a dimensionless SST magneto-coupling factor (with \(\eta=1\) as the minimal canonical choice),
                \item \(\omega_{\max}=\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}/r_c\) is the SST cutoff scale.
            \end{itemize}
        
        \paragraph{Linearized birefringence (\(B\to 0\)).}
            Define
            \[
                D(\omega):=\omega_{\max}^2-\omega^2-i\gamma\omega.
            \]
            For weak fields \((\omega_L\ll \omega_{\max})\),
            \[
                (\omega_{\max}\pm \eta\omega_L)^2
                =
                \omega_{\max}^2 \pm 2\eta\omega_{\max}\omega_L + \mathcal{O}(\omega_L^2),
            \]
            so
            \[
                n_\pm^2(\omega,B)
                \approx
                1+\frac{S}{D(\omega)\pm 2\eta\omega_{\max}\omega_L}.
            \]
            Using
            \[
            ...
            \[
                \Delta(n^2):=n_-^2-n_+^2
                \approx
                \frac{4\eta S\omega_{\max}\omega_L}{D^2}.
            \]
            
            For weak circular splitting, \(n_\pm=n_0\pm \delta n/2\) and
            \[
                \Delta n:=n_- - n_+ \approx \frac{\Delta(n^2)}{2n_0},
            \]
            with \(n_0:=n(\omega,B=0)\). Therefore
            \[
                \boxed{
                    \Delta n(\omega,B)
                    \approx
        ...
        \paragraph{Faraday rotation and Verdet constant.}
            The Faraday rotation angle over path length \(L\) is
            \[
                \theta=\frac{\omega L}{2c}\,\Re\!\left[\Delta n(\omega,B)\right].
            \]
            Therefore the Verdet constant is
            \[
                \mathcal{V}_{\mathrm{SST}}(\omega)
                :=\frac{\theta}{BL}
                =
                \frac{\omega}{2cB}\,\Re\!\left[\Delta n(\omega,B)\right].
            \]
            Substituting \(\omega_L=eB/(2m_e)\) gives
            \[
                \boxed{
                    \mathcal{V}_{\mathrm{SST}}(\omega)
                    \approx
            ...
            This is the quantitative SST dispersive formula. It reduces to the kinematic estimate only after additional approximations and parameter elimination.
        
        \paragraph{Transparent, weak-damping limit (\(\gamma\to 0\)).}
            If \(\gamma\omega\ll |\omega_{\max}^2-\omega^2|\), then \(D(\omega)\approx \omega_{\max}^2-\omega^2\) is real, and
            \[
                \mathcal{V}_{\mathrm{SST}}(\omega)
                \approx
                \frac{\eta e\,\omega\,S\,\omega_{\max}}{2m_e c\,n_0(\omega_{\max}^2-\omega^2)^2}.
            \]
            Using the zero-field relation
            \[
                n_0^2-1=\frac{S}{\omega_{\max}^2-\omega^2},
            \]
            we may eliminate \(S\) to obtain
            \[
                \boxed{
                    \mathcal{V}_{\mathrm{SST}}(\omega)
                    \approx
                    \frac{\eta e\,\omega\,(n_0^2-1)\,\omega_{\max}}
                    {2m_e c\,n_0(\omega_{\max}^2-\omega^2)}
                }.
            \]
            In the deep sub-cutoff limit \((\omega\ll \omega_{\max})\),
            \[
                ...
                \frac{\eta e}{2m_e c}\,
                \frac{\omega (n_0^2-1)}{n_0}\,
                \left(\frac{r_c}{\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}}\right).
            \]
            For \(\eta=1\), this reproduces the corrected kinematic SST estimate.

        \paragraph{Interpretation in SST language.}
            The factor \(\omega_{\max}^{-1}=r_c/\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\) sets the inverse swirl stiffness timescale of the electron core. In this picture, larger \(\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\) (at fixed \(r_c\)) suppresses magneto-optic rotation, while larger \(r_c\) enhances it, provided the effective dielectric response \((S,n_0,\gamma)\) remains comparable.

        \paragraph{Regime validity and non-claims.}
            This model is intended for transparent dielectric media in weak-field linear response. It does \emph{not} by itself justify claims in strongly absorbing UV/X-ray/\(\gamma\)-ray regimes, dense plasmas, or neutron-star matter, where the bound-oscillator dielectric approximation can fail.
        
        \paragraph{Falsifiable predictions (minimal).}
            \begin{enumerate}
                \item \textbf{Scaling exponent test:} Measure \(\mathcal{V}(\omega)\) across a broad band and fit a local power law \(\mathcal{V}\propto \omega^p\). Deviations from a pure \(p=1\) law diagnose dispersive corrections \((\omega/\omega_{\max})^2\) and damping.
                \item \textbf{Line-shape test:} Near an absorption edge, the model predicts a dispersive enhancement governed by \(\Re[D^{-2}]\) with damping-dependent sign/shape.
                \item \textbf{SST stiffness test (effective):} Across material families, extract an effective \(\omega_{\max}^{\mathrm{eff}}\) and check whether inferred trends are consistent with an SST stiffness interpretation.
            \end{enumerate}
        
        \paragraph{Status.}
            \textbf{Status: Bridge model (SST constitutive ansatz).}
            The derivation is dimensionally consistent and reduces to the corrected kinematic estimate in the appropriate limit, but quantitative validation requires fitting \((S,\gamma,\eta)\) to data.
        
        \paragraph{Numerical check (canonical SST cutoff).}
            ...
            \[
                ...
                =
                7.763440655\times 10^{20}\ \mathrm{rad\,s^{-1}}.
            \]
            For visible frequencies \((\omega\sim 10^{15}\ \mathrm{rad\,s^{-1}})\), the ratio is
            \[
                \frac{\omega}{\omega_{\max}}\sim 10^{-6},
            \]
            so the deep sub-cutoff expansion is self-consistent for the kinematic limit.
        
        \paragraph{Analogy (intuition).}
            Like two nearly identical swings pushed at slightly different rhythms, the magnetic field makes right- and left-handed light drift out of sync as they move through the medium, so the overall polarization slowly turns.


    \section*{Appendix Y: Delay-Loop Mode Selection in Constrained Many-Body Fluids (Bridge Example)}
    \label{app:pseudogap_delay_bridge}
    \addcontentsline{toc}{section}{Appendix Y: Delay-Loop Mode Selection in Constrained Many-Body Fluids (Bridge Example)}

    \subsection*{Y.1 Scope and status}
    \textbf{Status: Bridge example (application note).}
    This appendix records a condensed-matter application of the canonical delay-induced mode-selection mechanism from Sec.~\ref{sec:delay_mode_selection}.
    It is included to document cross-domain reuse of the delay-loop formalism in a constrained many-body fluid setting relevant to pseudogap phenomenology.
    No new SST primitives, axioms, or universal couplings are introduced here.

    \subsection*{Y.2 Minimal bridge formulation}
    Consider a closed collective-circulation path of effective length \(L\) with collective transport speed \(v\) in a constrained two-dimensional many-body fluid.
    The finite return time
    \begin{equation}
        \tau = \frac{L}{v}
    \end{equation}
    induces delayed self-interaction of the phase variable around the loop.
    In the long-delay regime, the same universality logic used in Sec.~\ref{sec:delay_mode_selection} applies:
    delay selects admissible phase-locked circulation modes, while geometry/topology and dissipation determine which of those modes remain observable.

    \subsection*{Y.3 Application-specific interpretation (non-canonical)}
    In the pseudogap bridge interpretation, the delay-selected collective modes provide a hydrodynamic route to discrete collective energy scales and spectral-weight suppression without assuming that quasiparticles remain the fundamental carriers in the crossover regime.
    This is an \emph{application-sector} claim only.
    The effective quantities \((v,L,\tau)\) are phenomenological bridge parameters unless explicitly derived from a separate condensed-matter closure model.

    \subsection*{Y.4 Kinematic scale estimate and caution}
    A useful application estimate is the delay scale
    \begin{equation}
        \Delta E_{\tau} \sim \frac{h}{\tau},
    \end{equation}
    which may be used to organize candidate collective features in spectroscopy.
    This estimate is a dimensional bridge heuristic and is not, by itself, a canonical SST prediction for pseudogap materials.

    \subsection*{Y.5 Canonical role of this appendix}
    The purpose of this appendix is strictly organizational:
    it identifies a domain in which the canonical delay-loop mechanism may be reused, while preserving the Canon distinction between
    (i) universal SST structure and
    (ii) sector-specific effective modeling.


        \input{previous/part-end_References.tex}
