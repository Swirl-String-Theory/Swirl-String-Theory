...
\chapter*{\paragraph{SST circular-mode dispersion.}
Let
[
...
\chapter*{\paragraph{Linearized birefringence ((B\to 0)).}
Define
[
...
\chapter*{\paragraph{Faraday rotation and Verdet constant.}
...
[
\boxed{
\mathcal{V}\textit{{\mathrm{SST}}(\omega)
\approx
\frac{\eta e,\omega,S,\omega}{\max}}{2m_e c,n_0},
...
\chapter*{\paragraph{Transparent, weak-damping limit ((\gamma\to 0)).}
If (\gamma\omega\ll |\omega_{\max}^2-\omega^2|), then (D(\omega)\approx \omega_{\max}^2-\omega^2) is real, and
[
\mathcal{V}\textit{{\mathrm{SST}}(\omega)
\approx
\frac{\eta e,\omega,S,\omega}{\max}}{2m_e c,n_0(\omega_{\max}^2-\omega^2)^2}.
]
...
\mathcal{V}\textit{{\mathrm{SST}}(\omega)
\approx
\frac{\eta e,\omega,(n_0^2-1),\omega}{\max}}
...
The factor (\omega_{\max}^{-1}=r_c/\mathbf{v}\textit{{!\boldsymbol{\circlearrowleft}}) sets the inverse swirl stiffness timescale of the electron core. In this picture, larger (\mathbf{v}}{!\boldsymbol{\circlearrowleft}}) (at fixed (r_c)) suppresses magneto-optic rotation, while larger (r_c) enhances it, provided the effective dielectric response ((S,n_0,\gamma)) remains comparable.
...
\paragraph{Status.}
\textbf{Status: Bridge model (SST constitutive ansatz)}. The derivation is dimensionally consistent and reduces to the corrected kinematic estimate in the appropriate limit, but quantitative validation requires fitting (S,\gamma,\eta) to data.

\chapter*{\paragraph{Numerical check (canonical SST cutoff).}
Using the canonical SST values
[
\mathbf{v}\textit{{!\boldsymbol{\circlearrowleft}}=1.09384563\times 10^6\ \mathrm{m,s^{-1}},\qquad
r_c=1.40897017\times 10^{-15}\ \mathrm{m},
]
one obtains
[
\omega}{\max}}
\chapter*{\frac{\mathbf{v}_{!\boldsymbol{\circlearrowleft}}}{r_c}}
