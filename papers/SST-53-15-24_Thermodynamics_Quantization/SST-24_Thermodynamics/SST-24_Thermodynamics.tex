...
\newcommand{\vscore}{v_{\mkern-2mu\scriptscriptstyle\boldsymbol{\circlearrowleft}}}                    % shorthand: |v_swirl| at r=r_c
...
%==============================================================
        ...
        \subsection{The Primitive Triad}
            ...
            \begin{equation}
                \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} = \frac{\Gamma_0}{2\pi \rc}
        ...
        \subsection{The Mass Kernel as Equation of State}
            ...
            \end{equation}
            with $\Lambda_0 \propto \rhof \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}}^2 \rc^3$, a topological
...
%==============================================================
        ...
        \subsection{Two-Scale Geometry: $\rc \rightarrow a_0$}
            ...
            \begin{equation}
                a_0 = \frac{c^2}{2\,\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}}^2}\, \rc,
            \end{equation}
...
%==============================================================
        ...
        \subsection{Two-Vacuum SST Interpretation}
            ...
            characteristic velocity $\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}}$.
...
%==============================================================
    \section*{Appendix E (Corrected): Thermodynamic near-field motivation and Poisson-mediated far-field}
%==============================================================

        Assume two oppositely-oriented swirl strings with circulation $\pm\Gamma_0$.
        Thermodynamic arguments can motivate a \emph{regularized core} and an attractive near-field pressure coupling,
        but they do not, by themselves, fix a $1/r$ far-field law from Euler/Bernoulli alone.
        In SST, the $1/r$ tail is modeled via a local mediator (clock/foliation mode) satisfying a Poisson equation on $\mathbb{R}^3$,
        whose Green function yields the universal $1/r$ potential.
        ...
        The canonically consistent matching coefficient is
        \begin{equation}
        \Lambda = 4\pi\,\rho_{\text{core}}\,\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\rVert^{2}\,r_c^{4}
        \qquad (\mathrm{J\cdot m}).
        \end{equation}
        Thus, Coulomb binding is modeled as a Poisson-mediated long-range interaction with a hydrodynamically-motivated
        soft-core regularization near $r\sim r_c$.
...
%========================================
% 1. Swirl Clock and Time Dilation
%========================================
% Local swirl velocity field: \mathbf{v}(\mathbf{x})
            \newcommand{\vloc}{\mathbf{v}(\mathbf{x})}

% Swirl Clock factor: proper-time rate relative to the condensate frame
            \begin{equation}
                S_t(\mathbf{x})
                \;\equiv\;
                \frac{d\tau}{dt}
                \;=\;
                \sqrt{1 - \frac{\lVert \vloc \rVert^2}{\cLight^2}}.
                \label{eq:SwirlClock}
            \end{equation}

% Chronos–Kelvin invariant (schematic form, incompressible / inviscid / barotropic)
% D/Dt denotes the material derivative in the condensate frame.
            \begin{equation}
                \frac{D}{Dt}\!\left( R^2 \,\omega\, S_t \right) = 0,
                \label{eq:ChronosKelvin}
            \end{equation}
            where \(R\) is a characteristic orbital radius, \(\omega\) the local vorticity magnitude,
            and \(S_t\) the Swirl Clock factor \eqref{eq:SwirlClock}. This is the SST analogue of
            Kelvin’s circulation theorem, promoted to include relativistic time-dilation.

%========================================
% 2. Swirl Temperature as Geometric Strain
%========================================
% Orbital strain relative to the Bohr radius a_0
            \begin{equation}
                \varepsilon_R
                \;\equiv\;
                \frac{R - a_0}{a_0},
                \label{eq:strain_def}
            \end{equation}
            with \(a_0\) the equilibrium (Bohr) radius of the ground state.

% Define Swirl Temperature as a linear measure of strain
            \begin{equation}
                T_{\text{swirl}}
                \;\equiv\;
                \Theta\,\varepsilon_R,
                \label{eq:Tswirl_def}
            \end{equation}
            where \(\Theta\) is a stiffness modulus of the condensate with dimensions of temperature.
            Thus
            \[
                T_{\text{swirl}} = 0 \;\Leftrightarrow\; R = a_0
            \]
            and excited orbitals with \(R > a_0\) have \(T_{\text{swirl}} > 0\).

% Elastic free-energy cost of swelling (small-strain expansion)
            \begin{equation}
                \Delta \mathcal{F}_{\text{strain}}(R)
                \;\simeq\;
                \frac{1}{2}\,K_R \,(R - a_0)^2
                \;=\;
                \frac{1}{2}\,K_R a_0^2 \,\varepsilon_R^2
                \;=\;
                \frac{1}{2}\,K_R a_0^2 \,\left(\frac{T_{\text{swirl}}}{\Theta}\right)^2,
                \label{eq:Fstrain}
            \end{equation}
            with \(K_R\) an effective radial stiffness. Equation \eqref{eq:Fstrain} implements the
            statement that the stored elastic energy scales as \(E \propto T_{\text{swirl}}^2\).

%========================================
% 3. Kelvin Entropy and Topological Entropy
%========================================
% Kelvin-mode (vibrational) entropy of the electron filament
            \begin{equation}
                S_{\mathrm{K}}
                \;=\;
                -\kb
                \sum_{k} p_k \ln p_k,
                \label{eq:KelvinEntropy}
            \end{equation}
            where \(p_k\) is the population of the \(k\)-th Kelvin mode along the filament.
            A “cold” ground-state electron has \(p_0 = 1\), \(p_{k>0} = 0\), hence \(S_{\mathrm{K}}=0\).

% Golden-weighted topological probabilities
            \begin{equation}
                w(K)
                \;\propto\;
                \exp\!\left[-\,\beta_{\phi}\,g(K)\right]
                \;\phi^{-2\,g(K)},
                \label{eq:GoldenWeight}
            \end{equation}
            where \(K\) labels the knot type, \(g(K)\) a topological complexity measure
            (e.g.\ genus or crossing number), \(\phi\) the Golden Ratio, and \(\beta_{\phi}\)
            an effective inverse “topological temperature”.

% Normalized probabilities P(K) and Topological Entropy
            \begin{equation}
                P(K)
                \;=\;
                \frac{w(K)}{\sum_{K'} w(K')},
                ...
                S_{\mathrm{top}}
                \;=\;
                -\kb \sum_{K} P(K)\,\ln P(K).
                \label{eq:Stop_def}
            \end{equation}

% Total entropy of an orbital–core system
            \begin{equation}
                S_{\mathrm{tot}}
                \;=\;
                S_{\mathrm{K}} + S_{\mathrm{top}}.
                \label{eq:Stot_def}
            \end{equation}

%========================================
% 4. SST First Law and Thermodynamics of Excited States
%========================================
% Abe–Okuyama decomposition mapped to SST
            \begin{equation}
                dE_{\mathrm{orb}}
                \;=\;
                \underbrace{\sum_k E_k(R,\rc)\,dp_k}_{\displaystyle \delta Q_{\mathrm{SST}}}
                \;+\;
                \underbrace{\sum_k p_k
                \left(
                    \frac{\partial E_k}{\partial R}\,dR
                +
                    \frac{\partial E_k}{\partial \rc}\,d\rc
                \right)}_{\displaystyle \delta W_{\mathrm{SST}}},
                \label{eq:SSTFirstLaw}
            \end{equation}
            where:
            \begin{itemize}
                \item \(\delta Q_{\mathrm{SST}}\): “Swirl heat” from redistributing Kelvin-mode
                populations at fixed geometry \((R,\rc)\).
                \item \(\delta W_{\mathrm{SST}}\): mechanical work from swelling/compressing the
                orbital radius and core radius against the condensate pressure.
            \end{itemize}

% Helmholtz free energy for orbital n at small strain
            \begin{equation}
                \mathcal{F}_n(R, T_{\text{swirl}})
                \;=\;
                E_n^{(0)}
                + \Delta \mathcal{F}_{\text{strain}}(R)
                - T_{\text{swirl}}\,S_{\mathrm{K}}(T_{\text{swirl}})
                - T_{\text{swirl}}\,S_{\mathrm{top}},
                \label{eq:FreeEnergyOrbital}
            \end{equation}
            with \(\Delta \mathcal{F}_{\text{strain}}\) given by \eqref{eq:Fstrain} and
            \(E_n^{(0)}\) the zero-strain (Bohr-like) energy level.

% Low-temperature heat capacity of the vortex core:
% assume U_core ~ a T_swirl^2 => C_V ~ 2 a T_swirl
            \begin{equation}
                U_{\mathrm{core}}(T_{\text{swirl}})
                \;\simeq\;
                U_{\mathrm{core}}(0)
                + A_{\mathrm{K}}\,T_{\text{swirl}}^{2},
                \qquad
                C_V
                \;=\;
                \left(\frac{\partial U_{\mathrm{core}}}{\partial T_{\text{swirl}}}\right)_V
                \simeq
                2 A_{\mathrm{K}}\, T_{\text{swirl}},
                \label{eq:HeatCapacity}
            \end{equation}
            where \(A_{\mathrm{K}}\) encodes the Kelvin-mode spectrum. Equation \eqref{eq:HeatCapacity}
            encodes the prediction \(C_V \propto T_{\text{swirl}}\) at low “swirl temperature”.

%========================================
% 5. Swirl–Coulomb Potential and Force Balance
%========================================
% Swirl–Coulomb potential per unit mass (far field, r >> r_c)
            \begin{equation}
                V_{\mathrm{SST}}(r)
                \;=\;
                -\,\frac{\kappa_{\mathrm{SST}}}{r},
                \qquad r \gg \rc,
                \label{eq:VSST_def}
            \end{equation}
            where \(\kappa_{\mathrm{SST}}\) is an effective swirl–Coulomb constant with
            dimensions of \(\mathrm{length}^3/\mathrm{time}^2\). The corresponding inward
            acceleration is
            \begin{equation}
                a_{\mathrm{SST}}(r)
                \;=\;
                -\,\frac{\partial V_{\mathrm{SST}}}{\partial r}
                \;=\;
                -\,\frac{\kappa_{\mathrm{SST}}}{r^2}.
                \label{eq:aSST}
            \end{equation}

% Orbital force balance for the electron mass m_e
            \begin{equation}
                \frac{m_e v_n^2}{r_n}
                \;=\;
                m_e \,\frac{\kappa_{\mathrm{SST}}}{r_n^2}
                \quad\Rightarrow\quad
                r_n
                \;=\;
                \frac{\kappa_{\mathrm{SST}}}{v_n^{2}}.
                \label{eq:ForceBalance}
            \end{equation}

% Velocity quantization (n-th orbital as circulation multiple)
            \begin{equation}
                v_n
                \;=\;
                \frac{\vswirl}{n},
                ...
                \;=\;
                n\,\Gamma_0,
                \label{eq:VelocityQuantization}
            \end{equation}
            with \(\Gamma_0\) the circulation quantum and \(\vswirl\) the characteristic swirl speed.

% Master radius formula (hydrodynamic Bohr radius)
            \begin{equation}
                r_n
                \;=\;
                \frac{\kappa_{\mathrm{SST}}}{\vswirl^2}\;n^2
                \;\equiv\;
                a_0\,n^2,
                \label{eq:MasterRadius}
            \end{equation}
            which recovers the hydrogenic scaling \(r_n \propto n^2\) if
            \(a_0 = \kappa_{\mathrm{SST}}/\vswirl^2\) is identified with the Bohr radius.

% Master energy formula (n-th orbital)
            \begin{equation}
                E_n
                \;=\;
                -\,\frac{1}{2}\,m_e v_n^2
                \;=\;
                -\,\frac{1}{2}\,m_e \frac{\vswirl^2}{n^2}
                \;\equiv\;
                -\,\frac{E_1}{n^2},
                \label{eq:MasterEnergy}
            \end{equation}
            where \(E_1 = \tfrac{1}{2}m_e \vswirl^2\) is the SST Rydberg scale set by the
            dynamic pressure of the condensate at the stability limit.

%========================================
% 6. Relation of T_swirl to Orbital Radius and Stability
%========================================
% Combine strain definition and force balance
            Combining \eqref{eq:strain_def}, \eqref{eq:Tswirl_def}, and \eqref{eq:MasterRadius}
            yields an explicit relation between swirl temperature and orbital radius:
            \begin{equation}
                T_{\text{swirl}}(n)
                \;=\;
                \Theta \left( \frac{r_n - a_0}{a_0} \right)
                \;=\;
                \Theta \left( n^2 - 1 \right).
                \label{eq:Tswirl_n}
            \end{equation}
            Thus the ground state \(n=1\) corresponds to \(T_{\text{swirl}}=0\), and excited
            states \(n>1\) appear as discrete “swirl temperatures” governed by the same
            \(n^2\) hierarchy as the Bohr radii.

%========================================
% END OF EQUATION PACK
%========================================
