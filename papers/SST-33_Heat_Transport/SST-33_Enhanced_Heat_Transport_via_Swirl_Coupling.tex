...
%========================================================================================
\newcommand{\paperdoi}{10.5281/zenodo.18390333}
...
\begin{document}
    ...
    \section{Scales from SST}
        ...
        \end{align}
        When it aids readability, rates are reported in \(\Omega_0\) units; conversion uses \(r_{\rm SI}=\hat r\,\Omega_0\) and \(\tau_{\rm SI}=\hat\tau/\Omega_0\) (see App.~C.3). Spatial gradients in the Swirl Clock act as a multiplicative modulation in the electronic Hamiltonian \(H_e\) and do not change how observables are reported in SI.
    ...
    \section{Coupled transport in 1D and perturbative solution}
        We adapt the unified density-matrix framework for bosons \(N(\mathbf R,\mathbf q)\)~\cite{Simoncelli2022} to a charged two-level ``electron'' with density matrix \(f\), coupled in the Born–Markov rotating-wave limit:
        \begin{align}
        ...
        \end{align}
        For a small, uniform \(\partial_x T\) and a time-independent state, we linearize around \(N^{(0)}(T)\) and \(f^{(0)}(T)\). In a near-degenerate two-branch bosonic subspace \(s,s'\) with detuning \(\delta=\Omega_{s'}-\Omega_s\) and one electronic transition \(\Delta\), the off-diagonal coherence obeys
        \begin{equation}
            \mathcal C_{e\leftrightarrow b} \equiv -\frac{i}{\hbar}[M, f\otimes N]_{\mathrm{RWA}}\,.
        \end{equation}
    ...
    \section{1D slab: temperature field and \(\Delta\kappa/\kappa\)}
        ...
        \end{equation}
        A small change \(\Delta\kappa\) gives
        \begin{equation}
