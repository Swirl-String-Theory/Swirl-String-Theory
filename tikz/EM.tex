...
% Faraday (with swirl-EMF term)
    \node(Faraday)
    {$\nabla \times \mathbf{E} = -\,\partial_t \mathbf{B} \;-\; \mathbf{b}_{\rm sw}$\\
    \footnotesize $[\nabla\times\mathbf E]=\mathrm{V\,m^{-2}},\ [\partial_t\mathbf B]=\mathrm{T\,s^{-1}},\ [\mathbf b_{\rm sw}]=\mathrm{V\,m^{-2}}$};
...
{$\mathbf{b}_{\rm sw}$\\ \footnotesize swirl–EMF source $[\mathbf b_{\rm sw}]=\mathrm{V\,m^{-2}}$};
...
\draw[garrow] (rho.south west) -- node[above,sloped,inner sep=1pt]
    {\footnotesize swirl-gravity mediation $\ \mathcal{G}_{\rm sw}$}
(b.north);
...
\section*{TikZ Sketch: Fixed Swirl Flux, Shrinking Area $\Rightarrow$ Vortex Nucleation}
    ...
    \node[align=center] at (-3,-1.6)
        {$\Phi_{\rm sw}=\displaystyle\int_{A_0}\bm{\varrho}\!\cdot d\mathbf A = N\kappa$\\(fixed)};
