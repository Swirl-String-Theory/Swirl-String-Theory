...
\section{Lagrangian and Field-Theoretic Framework}\label{sec:lagrangian}
    ...
    \subsection*{Mass via a solitonic knot energy functional}
        ...
        \paragraph{Normalization and canonical limits.}
            ...
            % ==== SST Macros (minimal; safe in-document) ====
...
\section{Scale Dependence: Running Couplings and Topological \texorpdfstring{$\beta$}{beta}-Functions}
\label{sec:SST-RG}

\paragraph{Claim.} Couplings $g_i(\mu)$ inherit scale dependence from swirl-string self-interaction and near-contact topology. We posit
    \begin{equation}
    \mu\frac{d g_i}{d\mu} \equiv \beta_i(g,\mu)
    = -\frac{b_i}{2\pi}g_i^3 \;+\; \kappa_i\,\frac{\mathcal{H}(K)+\alpha\,\mathcal{C}(K)}{( \mu\,\rc )^{2}}\,g_i \;+\; \mathcal{O}(g_i^5),
    \label{eq:beta}
    \end{equation}
    where $\mathcal{H}(K)$ is helicity (dimensionless), $\mathcal{C}(K)$ a near-contact count, $\rc$ the core scale, and $b_i$ reduce to SM values in the Standard-Model limit. The second term is an SST topological correction suppressed by $(\mu \rc)^{-2}$, ensuring decoupling for $\mu\rc\gg 1$.

\paragraph{Asymptotics.} For SU(3) ($i=3$), $b_3>0$ gives asymptotic freedom as in QCD; for U(1), screening persists ($b_1<0$), with SST corrections testable at $\mu\sim \rc^{-1}$.

\paragraph{Falsifier.} Extract $g_i(\mu)$ from lattice-matched event shapes and compare with \eqref{eq:beta} fits including a universal $(\mu\rc)^{-2}$ tail; absence of any such tail within errors falsifies the proposed correction.

% ==== Macros as above ====

\section{Canonical Quantization of the Swirl Field and Directors}
\label{sec:SST-quantization}

\paragraph{Fields and Constraints.} Let $\vswirl(\mathbf{x},t)$ be divergence-free and $\{\hat{d}_a(\mathbf{x},t)\}_{a=1}^3$ the internal directors with $\hat{d}_a^\dagger \hat{d}_a=1$. A minimal Hamiltonian density (research track) is
    \begin{equation}
    \mathcal{H}=\frac{\rhoF}{2}\,\lVert \vswirl\rVert^2 + \frac{\lambda}{2}\sum_a \lVert \nabla \hat{d}_a\rVert^2 + \gamma\,\omega\cdot\mathbf{S}(\hat{d}) + \mathcal{V}_{\text{topo}}[K],
    \end{equation}
    with vorticity $\omega\!=\!\nabla\times \vswirl$ and spin-like $\mathbf{S}$ from director currents.

\paragraph{Equal-Time Algebra.} Using the transverse projector $P_{ij}(\mathbf{k})=\delta_{ij}-k_i k_j/k^2$,
    \begin{align}
    \big[ v_i(\mathbf{x},t), \Pi_j(\mathbf{y},t)\big] &= i\hbar\, P_{ij}(-i\nabla)\,\delta^{(3)}(\mathbf{x}-\mathbf{y}),\\
    \big[ \hat{d}_a(\mathbf{x}), \hat{d}_b^\dagger(\mathbf{y})\big] &= \delta_{ab}\,\delta^{(3)}(\mathbf{x}-\mathbf{y}),\quad
    \big[ \hat{d}_a(\mathbf{x}), \hat{d}_b(\mathbf{y})\big]=0,
    \end{align}
    with $\Pi$ the momentum density conjugate to $\vswirl$ and incompressibility enforced by a Lagrange multiplier (pressure).

\paragraph{Quanta.} Linearizing about a stationary background yields transverse modes with creation/annihilation operators $a_{\mathbf{k},\lambda}^{(\text{swirl})}$ and internal director excitations $b_{\mathbf{k},r}^{(a)}$. \emph{Particles} (knotted solitons) appear as nonperturbative Fock-space dressings; counting is by homotopy class $K$.

\paragraph{Falsifier.} If transverse mode spectra deviate from the predicted projector-enforced dispersion in Sec.~\ref{sec:SST-GW}, quantization scheme is invalid.

% ==== Macros as above ====

\section{Mass Generation: Scalar Swirl Condensate vs. Topological Tension}
\label{sec:SST-mass}

\subsection{Scalar Swirl Condensate (Higgs-analogue)}
    Introduce $S(x)$ with potential $V(S)=\lambda\big(|S|^2-S_0^2\big)^2$. Couplings
    \begin{equation}
    \mathcal{L}\supset |D_\mu S|^2 - V(S) - y_K\,S\,\Psi_K^\dagger \Psi_K + \text{h.c.}
    \end{equation}
    generate masses $m_K = y_K S_0$ for knot species $\Psi_K$ after $\langle S\rangle=S_0$.

\subsection{Topological Mass from Line Tension}
    Alternatively, the knot mass follows from swirl tension $\mathcal{T}$ and curvature contact energy,
    \begin{equation}
    m_K c^2 = \alpha\,\mathcal{C}(K) + \beta\,\mathcal{L}(K) + \gamma\,\mathcal{H}(K),
    \end{equation}
    with $\mathcal{L}$ length, $\mathcal{C}$ contact count, $\mathcal{H}$ helicity. \textbf{Falsifier:} a global fit that fails for leptons+quarks simultaneously.
...
\section{Gravitational Waves in SST: Dispersion and Bounds}
\label{sec:SST-GW}

Linearizing the Euler–SST equations about a homogeneous state with preferred clock, transverse swirl perturbations $\mathbf{u}_\perp$ obey
\begin{equation}
...
giving
\begin{equation}
\omega^2 = c^2 k^2 \Big( 1 + \eta_2 (\rc k)^2 + \mathcal{O}((\rc k)^4)\Big).
\end{equation}
\textbf{Constraint:} GW170817+$\gamma$-ray counterpart enforces $|\omega/k - c|/c \lesssim 10^{-15}$ at $f\sim 10^2$ Hz, implying $|\eta_2| (\rc k)^2 \lesssim 10^{-15}$. With $\rc=1.41\times 10^{-15}$ m and $k=2\pi f/c$, this yields $|\eta_2|\lesssim 10^{11}$ (very weak), motivating higher-frequency constraints (pulsar timing, LISA). \textbf{Falsifier:} measurable frequency-dependent GW speed beyond this bound.
...
\section{Numerical Program: CFD + Topology Tracking}
\label{sec:SST-sim}

\paragraph{Algorithmic Steps.}
    \begin{enumerate}
    \item Incompressible solver (pressure Poisson) with Rankine-core regularization at scale $\rc$.
    \item Vortex-line advection; adaptive remeshing; explicit reconnection rule when separation $<\chi\,\rc$.
    \item Topological diagnostics: helicity $\mathcal{H}$, writhe, linking; compute near-contact $\mathcal{C}$.
    \item EM coupling: Biot–Savart magnetic field and induced fields to recover $\Delta\Phi=n\Phi_\star$ statistics.
    \item Parameter scans over drive helicity and frequency to map $R\leftrightarrow T$ transitions in real time.
    \end{enumerate}
    \textbf{Falsifier:} failure to reproduce predicted $\Delta \Phi$ quantization or the helicity-odd attosecond delay map from Sec.~\ref{sec:SST-falsifiability}.
