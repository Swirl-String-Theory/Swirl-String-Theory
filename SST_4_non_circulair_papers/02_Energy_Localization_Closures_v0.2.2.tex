...
\thanks{ORCID: 0009-0006-1686-3961, DOI: 10.5281/zenodo.21792054 }
\affiliation{Independent Researcher, Groningen, The Netherlands}
\date{2 August 2026; manuscript version 0.2.2}
...
The analysis isolates five independent gates: dynamical selection and numerical provenance of the ideal ropelength, computation of the line-energy coefficient, constitutive identification of the relevant density, derivation of the circulation sector, and Maxwell/source matching including the electromagnetic self-energy shape factor.  The first four can fix an \(\alpha\)-free geometric candidate; only the Maxwell/source gate can identify that candidate with the electromagnetic fine-structure constant.
...
\end{equation}
The high-resolution trefoil study of Przybyl and Pieranski
\cite{PrzybylPieranski2014} reports the numerical estimate
\begin{equation}
  ...
  \label{eq:ropelength}
\end{equation}
The quoted value is a high-resolution numerical estimate associated with the reported finite-resolution/extrapolation procedure.  It is neither a rigorous lower bound nor an exact value of the unknown global minimizer.  Existence and \(C^{1,1}\) regularity of ropelength minimizers are established mathematically \cite{CantarellaKusnerSullivan2002}, but those theorems do not certify the decimal value in \cref{eq:ropelength}.
...
\begin{quote}
\textbf{[GATE G0].}
Before \(\calL_D^{(3_1)}\) is used as a physical input, (i) derive or numerically establish a ropelength-dominated limit of the actual SST energy functional, and (ii) propagate the numerical provenance and uncertainty of the high-resolution trefoil estimate.  A finite-resolution or extrapolated value must not be treated as an exact topological invariant.
\end{quote}
...
\hline
H & --- (no explicit circulation) & \(3.8934\times10^{18}\) & horn-envelope calibration\\
...
The present article supports the following structure: five theorem gates \textbf{G0}--\textbf{G4}, which must be discharged before any comparison with measurement, followed by a single out-of-sample comparison \textbf{G5}.  Only \textbf{G4} can identify a dimensionless model coefficient with the electromagnetic coupling; \textbf{G0}--\textbf{G3} fix the geometric and hydrodynamic input that \textbf{G4} consumes.

\begin{description}[leftmargin=2.3cm,style=nextline]
  \item[\textbf{G0: geometry and dynamics}]
  Show that the resolved SST energy selects the ideal-rope trefoil, or replace \(\calL_D^{(3_1)}\) by the dynamically selected value.  Propagate the numerical provenance and uncertainty of the trefoil estimate.
  ...
  \item[\textbf{G4: electromagnetic identification}]
  Derive B2a--B2c, compute the self-energy shape factor \(C_{\rm EM}\), and establish which \(\alpha\)-free geometric or hydrodynamic response coefficient is equal to the electromagnetic coupling coefficient.

  \item[\textbf{G5: out-of-sample comparison}]
  Only after G4 has produced an electromagnetic coupling candidate, compare it with \(\alpha_{\rm obs}\).  The measured \(\alpha\), \(r_e\), and all transitive proxies are forbidden during G0--G4 fitting, branch selection, or coefficient determination.
\end{description}

\begin{remark}[Gate ordering]
\label{rem:gate-ordering}
Because \(D_{\rm geom}:=\ell_*/\calL_D\) and
\(\ell_*:=(3/8\pi)\lC\), one has identically
\begin{equation}
  \alpha_{\rm geom}
  \equiv
  \frac{D_{\rm geom}}{\lC}.
  \label{eq:alpha-geom-definition}
\end{equation}
G0--G3 can, in principle, fix an \(\alpha\)-free geometric candidate by selecting the curve, the line-energy coefficient, the relevant density coefficient, and the circulation sector.  They cannot by themselves establish that this dimensionless ratio is the electromagnetic fine-structure constant.  Gate G4 is therefore the unique \emph{electromagnetic-identification gate}: it must connect the geometric or hydrodynamic response to the Maxwell kinetic coefficient and to the normalized source sector.  Gate G5 is then a genuine out-of-sample comparison of the G4 output with \(\alpha_{\rm obs}\).

Accordingly, any claimed derivation of the electromagnetic \(\alpha\) that bypasses B2a--B2c and \(C_{\rm EM}\) is, at leading order, only a construction of the ratio \(D/\lC\) or a restatement of its definition, not yet a derivation of the electromagnetic coupling.
\end{remark}
...
Trefoil decimal value as exact invariant & excluded; numerical provenance required\\
G4 as electromagnetic identification & unique coupling-identification gate\\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Within the frozen closure family, at fixed normalization coefficients and with a fixed geometric input, the bookkeeping contains one free coefficient and one comparison datum.  Once those conventions are unfrozen, however, at least the dynamical knot representative, the line-energy coefficient, the relevant density sector, the circulation map, the electromagnetic stiffness match, and the electrostatic shape factor reopen as independent gates.  The scoped one-parameter statement is therefore valid only for the frozen algebraic scaffold, not for the physical model.
