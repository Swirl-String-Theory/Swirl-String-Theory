...
\begin{itemize}
    \item \textbf{Claimed:} motion relative to the clock--foliation selects preferred-frame corrections that can be parameterized and bounded using established test frameworks (e.g.\ PPN preferred-frame parameters), yielding concrete null experiments.
    \item \textbf{Not claimed:} large or generic violations of relativistic phenomenology; the intent is a controlled mapping to existing bounds (including gravitational-wave speed constraints) and a delineation of the remaining experimentally accessible parameter space.
\end{itemize}

\paragraph{Observable definition.}
The observables are laboratory-accessible anisotropy signatures (e.g.\ sidereal-modulated frequency shifts in precision resonators) that constrain preferred-frame parameters (such as $\alpha_1,\alpha_2$ in PPN language) and thereby bound velocity-dependent corrections to the mass functional.
