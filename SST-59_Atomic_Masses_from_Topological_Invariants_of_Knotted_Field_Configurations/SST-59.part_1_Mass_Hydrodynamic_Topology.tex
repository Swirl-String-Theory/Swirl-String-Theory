...
\begin{itemize}
    \item \textbf{Claimed:} within the above idealization, the invariant rest mass of a localized filamentary excitation can be represented by a localized rest-energy functional (Hamiltonian density integrated over an effective core volume) that factorizes into a scale, a volume proxy, and a dimensionless topology-dependent kernel.
    \item \textbf{Not claimed:} that the microscopic vacuum \emph{is} literally an Euler fluid, nor that Standard Model mass generation is replaced here; this part is formulated strictly as a mathematically controlled effective model whose empirical content is deferred to subsequent parts.
\end{itemize}

\paragraph{Observable definition.}
The primary observable is the rest-energy functional (rest frame)
\begin{equation}
    E_0 \;\equiv\; \int_V \rho_{\!E}(\mathbf{x})\,dV,
    \qquad
    M \;\equiv\; \frac{E_0}{c^2},
\end{equation}
operationalized through inertial response measurements (e.g.\ calibrated force-to-acceleration or frequency-to-mass sensitivity in resonant systems) in regimes where the model assumptions apply.
