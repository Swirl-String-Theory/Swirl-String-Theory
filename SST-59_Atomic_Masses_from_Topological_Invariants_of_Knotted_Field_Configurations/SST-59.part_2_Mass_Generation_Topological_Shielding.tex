...
\paragraph{Assumptions (effective model).}
\begin{itemize}
    \item Euler regime (ideal, non-dissipative dynamics).
    \item Incompressible flow.
    \item Inviscid flow.
    \item Barotropic equation of state (pressure is a function of density only).
    \item No reconnection of circulation/vorticity lines (topological class preserved).
\end{itemize}

\paragraph{Claimed vs.\ not claimed.}
\begin{itemize}
    \item \textbf{Claimed:} concordance-style constraints can be used as a \emph{selection rule} on admissible topological sectors for a topology-indexed mass kernel, thereby restricting hierarchical mass assignments without introducing continuous Yukawa textures as free inputs.
    \item \textbf{Not claimed:} that any specific numerical normalization (e.g.\ a particular global prefactor) is uniquely \emph{derived} from concordance alone; rather, this part defines a falsifiable constraint program that can be accepted or rejected by explicit computation on test families.
\end{itemize}

\paragraph{Observable definition.}
The observable content is encoded in (i) an admissibility map $\mathsf{G}(T)\in\{0,1\}$ computed from 4D-topological criteria for a configuration class $T$, and (ii) the induced restriction this imposes on the space of allowed kernel assignments when compared against measured mass ratios in the benchmark datasets of Part~III.

% ---------- Scope \& status: Assumptions / Claims / Observable ----------
\paragraph{Assumptions (topological constraint program).}
We work in the smooth category and treat the topology-indexed kernel as an effective selection framework constrained by computable 4D-topological criteria:
\begin{itemize}
  \item \textbf{Category:} smooth knots/links in $S^3$, with concordance notions defined via smooth 4-manifolds.
  \item \textbf{Admissibility map:} a binary gate $\mathsf{G}(T)\in\{0,1\}$ is introduced as a \emph{decidable} topological admissibility indicator (not a narrative device).
  \item \textbf{Hard testbed family:} the double-twist family $K_{m,n}$ is used because rational sliceness/concordance admits a theorem-level characterization in that family, yielding a finite, checkable criterion.
\end{itemize}

\paragraph{What is claimed vs.\ what is not claimed.}
\begin{itemize}
  \item \textbf{Claimed:} (i) the kernel is restricted by an explicit, reproducible gate $\mathsf{G}(T)$ tied to concordance/rational sliceness, converting an otherwise qualitative ``shielding'' notion into a falsifiable decision rule; (ii) in the $K_{m,n}$ testbed, $\mathsf{G}$ is fixed by a sharp criterion and is therefore non-adjustable when that sector is invoked.
  \item \textbf{Not claimed:} (i) we do not claim topology \emph{uniquely determines} all mass hierarchies without auxiliary modeling inputs; rather, we provide a constraint program that restricts admissible kernel sectors; (ii) the physical interpretation of the gate is not asserted as a microscopic derivation unless separately established---it is an effective selection rule whose empirical adequacy is testable.
\end{itemize}

\paragraph{Observable definition (what is actually measured).}
The observable output of this part is \emph{not} a new measurement channel but a \emph{computable restriction} on the model's mass assignments:
\[
T \;\longmapsto\; \mathsf{G}(T)\in\{0,1\},\qquad
T \;\longmapsto\; M(T)\ \text{via the factorized kernel.}
\]
Given a proposed topology assignment $T$ for an excitation class, $\mathsf{G}(T)$ is decidable (and for $K_{m,n}$ fixed by an exact criterion), which in turn fixes whether the normalization factor is active in $M(T)$. The falsifiable content is therefore the conjunction:
(i) a topology assignment rule, and (ii) the concordance-computed gate value, jointly implying definite mass ratios/patterns.
% -----------------------------------------------------------------------
