...
def Gswirl():
    return alpha * rhoM * Ce**2 / Fmax_vam
...
def vam_effective_mass(r, Rc):
    return 4 * math.pi * rhoM * Rc**3 * (2 - (2 + r / Rc) * math.exp(-r / Rc))

def vam_adjusted_time(r, omega=0):
    Rc = rc  # Core radius
    M_eff = vam_effective_mass(r, Rc)
    G_swirl = alpha * rhoM * Ce ** 2 / Fmax_vam
...
def vortex_quantized_energy():
    ...
    return 0.5 * rhoM * (h / (M_e * lambda_c))**2
...
def vam_adjusted_time_hybrid(r, omega=0, mass=None):
    ...
    M_eff_vam = 4 * math.pi * rhoM * rc ** 3 * (2 - (2 + r / rc) * math.exp(-r / rc))
