    ...
    \subsection*{B.2\quad Coupling-channel summary table}
        ...
        \begin{figure}[h]
            ...
            \begin{tikzpicture}[
                ...
                % Nodes
                \node[box] (start) {Electron--swirl coupling $\hat M$\\(see App.~A)};
                \node[box, below=of start, xshift=-60mm, fill=black!3] (clock) {Clock (quadratic)\\ $\tilde g \propto (\rc^2/2c^2)\,\omega_e\,\omega_0\,\omega_s^{\rm zpf}$\\ \scriptsize negligible in bars};
                \node[box, below=of start, fill=black!3] (press) {Pressure-like (energy density)\\ $g \propto \rho_f V_e\,|\mathbf v_0|\,v_s^{\rm zpf}/\hbar$\\ \scriptsize SAW/MEMS target};
                \node[box, below=of start, xshift=60mm, fill=black!3] (gauge) {Emergent gauge (minimal)\\ $g \propto g_{\rm swirl}\!\int \!\langle \hat J^\mu\rangle W_{\mu,s}^{\rm zpf}$\\ \scriptsize model-dependent};

                \draw[arr] (start) -- (clock);
                \draw[arr] (start) -- (press);
                \draw[arr] (start) -- (gauge);
                \node[box, below=16mm of press, fill=black!5] (obs)  {Observable:\\ $\Delta\kappa(\delta,\Gamma)$, $\;\Delta\kappa_{\rm asym}$ (null-test)};
