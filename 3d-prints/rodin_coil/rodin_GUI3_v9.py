...
Version v9: adds an update-log table window for run summaries and lane summaries.
...
class RodinGUI3(tk.Tk):
    ...
    def _ensure_axes(self, show_heatmap: bool, force: bool = False):
        """Create fresh Matplotlib axes.

        With TkAgg, colorbar axes can survive ordinary ax.cla() calls. Forcing a
        full figure clear on every redraw is the safest way to prevent stacked
        colorbar/legend labels after repeated Update/Auto redraws.
        """
        show_heatmap = bool(show_heatmap)
        if (not force) and self._layout_heatmap == show_heatmap:
            return
        self.fig.clf()
        self._cbar_heat = None
        self._cbar_b = None
    ...
    def _populate_log_trees(self):
        if self._log_summary_tree is not None:
            ...
            for row in self._log_rows:
                ...
                self._log_summary_tree.insert("", "end", values=[row.get(c, "") for c in cols])
