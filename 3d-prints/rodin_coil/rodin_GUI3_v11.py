...
Version v11: fixes Rodin phase placement so 3-phase lanes are visibly distinct; adds Rodin phase placement mode.
...
def build_rodin_3phase(
    ...
    gap: float,
    ccw_start_mode: str,
    phase_placement: str = "Global rotation (visible lanes)",
) -> List[Coil]:
    """Build Rodin CW/CCW lane families.

    Important: the older cell-phase method shifts the torus-knot parameter t.
    For a closed full torus-knot path that is only a reparameterization of the
    same geometric locus, so all phases can look exactly stacked. The default
    here is therefore a rigid toroidal rotation around z for each phase. That
    creates distinct physical lanes while preserving p/q shape.
    """
    ...
    use_cell_phase = str(phase_placement).lower().startswith("cell")
    ...
    for i in range(phases):
        if use_cell_phase:
            # Legacy behavior: q-cell phase shift; can overlap for closed paths.
            cell_phase = i / phases
            theta_phase = 0.0
        else:
            # Visible physical lane separation: rigid rotation around z.
            cell_phase = 0.0
...
def build_starship(S: int, step_fwd: int, step_bwd: int, n_pairs: int, Rb: float, Rt: float,
                   ...
                   neutral_current_factor: float = 0.0) -> List[Coil]:
...
class RodinGUI3(tk.Tk):
    def __init__(self):
        ...
        self.var_rodin_phase_placement = tk.StringVar(value="Global rotation (visible lanes)")
    ...
    def _build_controls(self):
        ...
        self._add_labeled_combo(
            ...
            ["Global rotation (visible lanes)", "Cell phase legacy (may overlap)"],
        )
