...
def build_rodin_3phase(
    ...
    """Build Rodin-like CW/CCW torus-knot families.

    Important fix:
    a pure cell-phase shift on a closed torus-knot often traces the exact same
    geometric curve, only with a different start parameter. That makes phase 1,
    2, 3 appear stacked on top of each other. To create *visible* A/B/C lanes,
    we place each phase by a real global azimuthal rotation around the z-axis.
    """
    ...
    phase_angles = 2.0 * np.pi * np.arange(phases, dtype=float) / phases
    for i, ang in enumerate(phase_angles):
        pts = torus_knot_cellphase_polyline(
            R_major, r_minor, p, q, points, turns,
            cell_phase_frac=0.0, mirror=False, z_offset=z_cw, theta_offset=float(ang)
        )
        coils.append((pts, colors_cw[i % len(colors_cw)], "-", f"Rodin CW phase {i+1}", 1.0))
    for i, ang in enumerate(phase_angles):
        pts = torus_knot_cellphase_polyline(
            R_major, r_minor, p, q, points, turns,
            cell_phase_frac=0.0, mirror=True, z_offset=z_ccw,
            theta_offset=float(ang + theta_offset_ccw)
        )
...
def build_starship(S: int, step_fwd: int, step_bwd: int, n_pairs: int, Rb: float, Rt: float,
                   spacing: float, profile: str, power: float, curved: bool,
                   samples_per_seg: int) -> List[Coil]:
    # Same conceptual mapping as GUI-SawBowl.py: ABC + anti at 180 degrees, each with fwd/neu/bwd strands.
    ...
    specs = [
        ...
        (0, np.pi, True, "a", ["navy", "deepskyblue", "slateblue"]),
        (9, np.pi, True, "b", ["maroon", "tomato", "darkorange"]),
        (18, np.pi, True, "c", ["darkgreen", "limegreen", "olive"]),
    ]
...
class RodinGUI3(tk.Tk):
    ...
    def _build_controls(self):
        ...
        sec_preset = CollapsibleSection(frm, "Coil preset", True); sec_preset.pack(fill=tk.X)
        sec_generic = CollapsibleSection(frm, "Generic coil controls", False); sec_generic.pack(fill=tk.X)
        sec_rodin = CollapsibleSection(frm, "Rodin 3-phase controls", True); sec_rodin.pack(fill=tk.X)
        sec_shape = CollapsibleSection(frm, "SawShape / Starship shape sliders", True); sec_shape.pack(fill=tk.X)
        sec_compute = CollapsibleSection(frm, "Compute / field", True); sec_compute.pack(fill=tk.X)
        sec_viz = CollapsibleSection(frm, "Visual / export", True); sec_viz.pack(fill=tk.X)
        ...
        r = sec_rodin.body
        self._add_labeled_entry(r, "Major diameter [cm]", self.var_major_d_cm, key="major_d_cm")
        self._add_labeled_entry(r, "Minor radius [mm]", self.var_minor_r_mm, key="minor_r_mm")
        self._add_labeled_spin(r, "p", self.var_p, 1, 200, key="p")
        self._add_labeled_spin(r, "q", self.var_q, 1, 200, key="q")
        self._add_labeled_spin(r, "Turns", self.var_turns, 1, 50, key="turns")
        self._add_labeled_spin(r, "Phases per cell", self.var_phases, 1, 12, key="phases")
        ttk.Checkbutton(r, text="Separate CW/CCW families along z", variable=self.var_separate).pack(anchor="w", pady=2)
        self._add_slider(r, "Gap [cm]", self.var_gap_cm, 0.0, 25.0, lambda v: f"{v:.2f} cm")
        ttk.Label(r, text="CCW start relative to CW").pack(anchor="w")
        ttk.Combobox(r, textvariable=self.var_ccw_start, values=["Same", "Opposite (1->19)"], state="readonly").pack(fill=tk.X)

        s = sec_shape.body
        self._add_slider(s, "R bottom [mm]", self.var_shape_Rb_mm, 1.0, 80.0, lambda v: f"{v:.1f} mm")
        self._add_slider(s, "R top [mm]", self.var_shape_Rt_mm, 1.0, 80.0, lambda v: f"{v:.1f} mm")
        self._add_slider(s, "Height / spacing [mm]", self.var_shape_spacing_mm, 1.0, 80.0, lambda v: f"{v:.1f} mm")
        self._add_labeled_spin(s, "Layers / pairs", self.var_shape_layers, 1, 240, key="shape_layers")
        self._add_slider(s, "Bowl power", self.var_shape_power, 0.2, 6.0, lambda v: f"{v:.2f}")
        ttk.Label(s, text="Bowl profile").pack(anchor="w")
        ttk.Combobox(s, textvariable=self.var_shape_profile, values=["Exponential", "Linear", "Inverse Exp"], state="readonly").pack(fill=tk.X, pady=(0, 6))
        self._add_labeled_spin(s, "Saw S", self.var_saw_S, 3, 200, key="saw_S")
        self._add_labeled_spin(s, "Saw forward step", self.var_saw_fwd, -200, 200, key="saw_fwd")
        self._add_labeled_spin(s, "Saw backward step", self.var_saw_bwd, -200, 200, key="saw_bwd")
        self._add_labeled_spin(s, "Starship S", self.var_starship_S, 3, 200, key="starship_S")
        self._add_labeled_spin(s, "Starship forward step", self.var_starship_fwd, -200, 200, key="starship_fwd")
        self._add_labeled_spin(s, "Starship backward step", self.var_starship_bwd, -200, 200, key="starship_bwd")
        self._add_labeled_spin(s, "Samples per segment", self.var_samples_per_seg, 2, 120, key="samples_per_seg")
    ...
    def _bind_autoupdate(self):
        watch = [
            ...
            self.var_show_quiver, self.var_show_heatmap, self.var_quiver_step, self.var_arrow_scale, self.var_show_legend,
        ]
        for v in watch:
    ...
    def _commit_inputs(self):
        ...
        for key, var, lo, hi in [
            ...
            ("soft_m", self.var_soft_m, 0.0, None), ("arrow_scale", self.var_arrow_scale, 1e-6, None),
        ]:
