...
class ZenodoFileViewer:
    def __init__(self, root):
        ...
        self.papers_dir = get_papers_dir()
    ...
    def refresh_tree(self):
        ...
        sst_folders = []
        for folder in self.papers_dir.iterdir():
            ...
            try:
                result = subprocess.run(
                    [sys.executable, str(Path(__file__).parent / 'upload_single_pdf.py'), 
                     str(config_file.relative_to(self.papers_dir))],
                    cwd=self.tools_dir,
                    capture_output=True,
                    text=True,
                    encoding='utf-8',
    ...
    def render_and_upload_selected(self):
        ...
        def render_thread():
            try:
                ...
                result = process_config_file(config_file, automation, self.papers_dir)
            ...
            try:
                result = subprocess.run(
                    [sys.executable, str(Path(__file__).parent / 'sync_publication_date.py'),
                     '--config', str(config_file.relative_to(self.papers_dir)), '--update-zenodo'],
                    cwd=self.tools_dir,
                    capture_output=True,
                    text=True,
                    encoding='utf-8',
    ...
    def open_config_editor(self, config_file: Path, source_file: Path):
        ...
        if not config_data and source_file.suffix == '.tex':
            ...
            config_data = {
                ...
                'tex_file': str(source_file.relative_to(self.papers_dir)),
        ...
        def save_config():
            try:
                ...
                new_config = {
                    ...
                    'tex_file': config_data.get('tex_file', str(source_file.relative_to(self.papers_dir))),
