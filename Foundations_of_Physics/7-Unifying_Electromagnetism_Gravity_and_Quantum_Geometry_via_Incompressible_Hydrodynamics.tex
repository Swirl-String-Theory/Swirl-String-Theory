...
\section{Emergent Gravity: Swirl Pressure}

Gravity is derived as a pressure effect. In steady, axisymmetric swirling flows, radial pressure gradients balance centripetal acceleration\textsuperscript{21}. The radial Euler equation yields\textsuperscript{22}:
\begin{equation}
\frac{1}{\rho}\frac{dp}{dr} = \frac{v_{\theta}^2(r)}{r} - g_r(r)
\end{equation}

In the absence of external forces ($g_r=0$), the swirl induces an effective acceleration $g_{eff} = v_\theta^2/r$\textsuperscript{23}. Uniquely, for a swirl profile $v_\theta(r) = \sqrt{GM/r}$, the effective acceleration becomes exactly Newtonian ($GM/r^2$)\textsuperscript{24}. Thus, the gravitational potential $\Phi$ is isomorphic to the specific pressure field $p/\rho$ of the vacuum vortex condensate.
