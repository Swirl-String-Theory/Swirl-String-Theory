...
\chapter*{Thermodynamic Geometrodynamics of the Swirl Condensate: A Unified Master Equation Formulation for Hydrogenic Orbitals in Swirl-String Theory}
\section*{Abstract}
...
\end{itemize}
\subsection*{4.2 The Swirl-Coulomb Potential}
...
Integrating this from infinity to $r$ gives the pressure deficit $\Delta P$. Converting this to a potential energy $V_{SST}(r)$ per unit mass, and regularizing for the finite core size $r_c$, yields the Swirl-Coulomb potential 1:
...
where $\Lambda = 4\pi \rho_m v_{\mathcal{G}} r_c^3$ is the Swirl Coulomb Constant. In the far field ($r \gg r_c$), this recovers the $-1/r$ Coulomb potential, proving that electrostatics emerges from hydrodynamics.
