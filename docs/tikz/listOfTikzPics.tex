...
\node[draw,rounded corners,very thick,align=center,minimum width=4cm] (bulk) {$\tfrac12\rho_f\,\|\mathbf v\|^2-\rho_E$\\\footnotesize bulk swirl};
...
\node[draw,rounded corners,very thick,below=2.6cm of cont,align=center,minimum width=5.6cm] (em) {$\mathcal L_{\textrm EM}^{\textrm int}[A_\mu;\Sigma]$};
...
\begin{tikzpicture}
\begin{axis}[width=0.8\linewidth,height=5cm,
xlabel={$r$}, ylabel={$V_{\textrm eff}(r)$ (arb.)}, xmin=0,xmax=1, ymin=-1.0,ymax=1.0, grid=both]
...
\end{figure}
\section*{2) Circulation quantization on a ring}
\begin{figure}[t]
\centering
...
\node at (0,-2.8) {$\Gamma_n=\oint v\!\cdot d\ell = n\frac{h}{m_e}, \quad \lambda_{\textrm ring}=\frac{2\pi R}{n}=\frac{h}{p_\theta}$};
\end{tikzpicture}
...
\section*{2) Knot invariants overlay on a trefoil ($C(K)$, $\mathcal H(K)$)}
...
\section*{3) Core profile $\Omega(r)$ and Swirl Clock $dt_{\textrm local}/dt_\infty$}
...
\begin{axis}[
  width=\linewidth,height=5cm,
  xlabel={$v/c$}, ylabel={$dt_{\textrm local}/dt_\infty$},
...
\section*{4) Fringe geometry (slit separation $s$, distance $L$, screen coordinate $x$)}
...
\section*{6) Visibility vs.\ which-way coupling $\mathcal V=e^{-\Gamma\tau}$}
...
\begin{tikzpicture}[scale=1.0]
  \node[draw,very thick,rounded corners,align=center,minimum width=4.2cm,minimum height=1.0cm] (bulk)
    at (0,1.5) {$\tfrac12\rho_f\,\|\mathbf v\|^2 - \rho_E$ \\ bulk swirl};
  ...
  \node[draw,very thick,rounded corners,minimum width=5.4cm,minimum height=0.9cm] (em)
    at (0,-2.1) {$\mathcal L_{\textrm EM}^{\textrm int}[A_\mu;\Sigma]$};
