...
rotation_steps=0
rotation_angle = (2 * np.pi / 36) / 3
# Define provided base sequence
base_sequence = [1, 5, 9, 4, 8, 3, 7, 2, 6, 1]

# Create explicit sequences as per user request
sequence_A_forward = [((num * 3 - 0 - 1) % 36) + 1 for num in base_sequence]
sequence_A_neutral = [((num * 3 - 0 - 1) % 36) + 1 for num in base_sequence]
sequence_A_backward = [((num * 3 - 0 - 1) % 36) + 1 for num in base_sequence]

sequence_B_forward = [((num * 3 - 1 - 1) % 36) + 1 for num in base_sequence]
sequence_B_neutral = [((num * 3 - 1 - 1) % 36) + 1 for num in base_sequence]
sequence_B_backward = [((num * 3 - 1 - 1) % 36) + 1 for num in base_sequence]

sequence_C_forward = [((num * 3 - 2 - 1) % 36) + 1 for num in base_sequence]
sequence_C_neutral = [((num * 3 - 2 - 1) % 36) + 1 for num in base_sequence]
sequence_C_backward = [((num * 3 - 2 - 1) % 36) + 1 for num in base_sequence]

# Points setup
angles = np.linspace(0, 2 * np.pi, 37)[:-1]
positions = {i+1: (np.cos(angle), np.sin(angle)) for i, angle in enumerate(angles)}
segment_shift = ((2 * np.pi / 36) / 3)


# Recalculate positions with adjusted angles for rotation and reversed numbering
angles_rotated = np.linspace(0, 2 * np.pi, 37)[:-1] - np.pi*1.5 +(2*np.pi*(1/36)) # Rotate 90° counterclockwise
...
def generate_3d_wire(sequence, segment, z_layer, base_color, style, alpha=1.0):
    ...
    angles = np.linspace(0, 2 * np.pi, 37)[:-1] - np.pi * 1.5 + (2 * np.pi * (1 / 36))
    ...
    for i in range(len(sequence) - 1):
        ...
        angle_start = angles[(num - 1) % 36] + segment_shift * (segment - 1)
        angle_end = angles[(next_num - 1) % 36] + segment_shift * (segment - 1)
