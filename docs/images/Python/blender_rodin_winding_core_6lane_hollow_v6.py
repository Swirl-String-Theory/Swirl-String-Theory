...
OUTER_RADIUS = 45.0    # mm  (constraint: R_MAJOR + R_TUBE = OUTER_RADIUS)
HOLE_DIAMETER = 10.0   # mm  (inner hole = 2*(R_MAJOR - R_TUBE); must be > 0)

# Derived radii (mm):
#   R_MAJOR = (OUTER_RADIUS + HOLE_DIAMETER/2)/2
#   R_TUBE  = (OUTER_RADIUS - HOLE_DIAMETER/2)/2
R_MAJOR = 0.5*(OUTER_RADIUS + 0.5*HOLE_DIAMETER)
R_TUBE  = 0.5*(OUTER_RADIUS - 0.5*HOLE_DIAMETER)
if R_TUBE <= 0 or R_MAJOR <= R_TUBE:
    raise ValueError('Invalid radii: need R_MAJOR > R_TUBE > 0. Increase HOLE_DIAMETER or OUTER_RADIUS.')
# Lane placement around minor circle (degrees)
# 6 separate lanes (60° spacing): A={0,120,240}, B={60,180,300}
Q_BASE = Q  # base minor winding count
# 6 lanes: 3 with +Q (Group A) and 3 with -Q (Group B, geometrically mirrored handedness)
# Group A (3-phase): 0,120,240 deg
# Group B (mirrored): 60,180,300 deg, with Q -> -Q so the helix winds the opposite way
LANE_SPECS = [
    (+Q_BASE, 0), (+Q_BASE, 120), (+Q_BASE, 240),
    (-Q_BASE, 60), (-Q_BASE, 180), (-Q_BASE, 300),
]
# Groove sizing
WIRE_D = 1.20      # mm
CLEARANCE = 0.40   # mm
...
# Groove depth visible on the surface (must be <= GROOVE_R)
GROOVE_DEPTH = 0.45  # mm (try 0.35–0.65). Larger = deeper groove.

# Inward sink derived so the cutter still intersects the surface:
# surface depth ≈ GROOVE_R - SINK  =>  SINK = GROOVE_R - GROOVE_DEPTH
SINK = max(0.0, GROOVE_R - GROOVE_DEPTH)

print('Radii: OUTER_RADIUS=%.3f mm HOLE_DIAMETER=%.3f mm -> R_MAJOR=%.3f mm R_TUBE=%.3f mm (inner radius=%.3f mm)' % (OUTER_RADIUS, HOLE_DIAMETER, R_MAJOR, R_TUBE, (R_MAJOR-R_TUBE)))

print('Groove params: WIRE_D=%.3f mm CLEARANCE=%.3f mm GROOVE_R=%.3f mm GROOVE_DEPTH=%.3f mm => SINK=%.3f mm' % (WIRE_D, CLEARANCE, GROOVE_R, GROOVE_DEPTH, SINK))
...
# Show lane cutters for debugging/visual confirmation
SHOW_CUTTERS = True
...
# Output mode
# =========================
# If True, export a *winding core* (a torus body with grooves carved into it).
# If False, export the clamshell mold TOP/BOTTOM as before.
MAKE_WINDING_CORE = True

# Hollow the winding core (saves filament; relies on wall thickness for stiffness)
MAKE_HOLLOW = True
WALL_THICKNESS = 3.0   # mm (typical 2.2–3.5 with 0.4 mm nozzle)
INNER_CLEARANCE = 0.3  # mm safety so the inner void doesn't nick groove bottoms
...
def get_output_dir():
    ...
    Priority:
      1) OUTPUT_DIR if set (absolute or relative to current .blend)
      2) Folder of the current .blend ("//") if the file is saved
      3) Blender temp directory (bpy.app.tempdir)
    """
    if OUTPUT_DIR:
        d = bpy.path.abspath(OUTPUT_DIR)
    else:
        d = bpy.path.abspath("//")  # only valid if the .blend is saved
        if not d:
            d = bpy.app.tempdir
    ...
    try:
        ...
        # Fall back to temp dir if creation fails
        d = bpy.app.tempdir or "."
        os.makedirs(d, exist_ok=True)
...
def _enable_stl_addon_if_needed():
    """
    Blender 4.x/5.x may not expose STL ops unless the STL IO addon is enabled.
    Try enabling it; ignore errors if already enabled or unavailable.
    """
    try:
...
def _call_op_filtered(op, kwargs):
    """
    Call a bpy operator with kwargs filtered to valid RNA properties.
    This makes the script resilient across Blender versions where parameter names change.
    """
    props = None
    try:
...
def export_stl(obj, filepath):
    # Ensure absolute path and that the parent directory exists
    ...
    raise RuntimeError(
        "No STL export operator found. Enable the STL addon or use File > Export > STL manually."
    )
...
def create_curve_from_points(name, pts, bevel_radius, color=(0.8,0.8,0.8,1.0)):
...
def main():
    ...
    for idx, (q_val, deg) in enumerate(LANE_SPECS, start=1):
        ...
        color = (0.2, 0.6, 1.0, 1.0) if q_val > 0 else (1.0, 0.35, 0.2, 1.0)
        c = create_curve_from_points(f"LaneCurve_{idx:02d}_{tag}_Q{q_val:+d}_{deg}deg", pts, bevel_radius=GROOVE_R, color=color)
        lane_objs.append(c)

    # Convert to mesh and join
    cutter_meshes = [convert_to_mesh(o) for o in lane_objs]
    # Keep visible individual cutters for inspection (optional)
    if SHOW_CUTTERS:
        for o in cutter_meshes:
            o.display_type = 'WIRE'
            o.hide_set(False)
            o.hide_render = True
    # Create a separate joined cutter object for the boolean (so we don't destroy the per-lane objects)
    dupes = []
    for o in cutter_meshes:
        d = o.copy()
        d.data = o.data.copy()
        ...
        dupes.append(d)
    cutters = join_objects(dupes, "GrooveCutters")
    ...
    # =========================
    # WINDING CORE (inverse of cutters)
    # =========================
    if MAKE_WINDING_CORE:
        set_object_name(base, "WindingCore_solid")

        if MAKE_HOLLOW:
            # Inner torus to subtract (creates a hollow shell)
            # Inner minor radius r_in = R_TUBE - WALL_THICKNESS
            r_in = R_TUBE - WALL_THICKNESS
            if r_in <= 0:
                raise ValueError("WALL_THICKNESS is too large (R_TUBE - WALL_THICKNESS <= 0). Reduce WALL_THICKNESS.")
            # Add a small clearance so inner void doesn't nick groove bottoms
            r_in = max(0.1, r_in - INNER_CLEARANCE)
            ...
            inner = bpy.context.view_layer.objects.active
            set_object_name(inner, "InnerVoid")

            # Optional voxel remesh for robust boolean
            if USE_VOXEL_REMESH:
                add_voxel_remesh(inner, VOXEL_SIZE); apply_modifier(inner, "VoxelRemesh")
                add_voxel_remesh(base,  VOXEL_SIZE); apply_modifier(base,  "VoxelRemesh")

            # Difference: shell = base - inner
            boolean_difference(base, inner, name="WindingCore_hollow")
            deselect_all()
            inner.select_set(True)
            bpy.ops.object.delete()

        # Export winding core and save blend, then stop (skip clamshell)
        out_dir = get_output_dir()
        print("Output directory:", out_dir)

        core_stl = os.path.join(out_dir, "rodin_winding_core_hollow.stl" if MAKE_HOLLOW else "rodin_winding_core_solid.stl")
        export_stl(base, core_stl)

        blend_path = os.path.join(out_dir, "rodin_winding_core.blend")
        bpy.ops.wm.save_as_mainfile(filepath=blend_path)

        print("Done.")
        print("Saved:", blend_path)
        print("Exported:", core_stl)
        return


    # Optionally hide cutters (set SHOW_CUTTERS=False above)
    if not SHOW_CUTTERS:
        cutters.hide_set(True)
        cutters.hide_render = True
    else:
        cutters.hide_set(False)
        cutters.hide_render = False
        # make cutters easier to see
    ...
    out_dir = get_output_dir()
    print("Output directory:", out_dir)
    full_stl = os.path.join(out_dir, "rodin_mold_full.stl")
    top_stl  = os.path.join(out_dir, "rodin_mold_top.stl")
    bot_stl  = os.path.join(out_dir, "rodin_mold_bottom.stl")
    ...
    # Save blend
    blend_path = os.path.join(out_dir, "rodin_mold.blend")
    bpy.ops.wm.save_as_mainfile(filepath=blend_path)
