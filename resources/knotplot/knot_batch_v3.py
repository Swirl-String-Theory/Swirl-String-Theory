...
def parse_knot_id_from_filename(fname):
    """
    Robust parser for names like:
      knot_3.1.fseries        -> 3_1
      knot_3.1_ideal.txt      -> 3_1
      knot_10.123.fseries     -> 10_123
      knot_0.2.1.fseries      -> 0_2_1
      knot_0.2.1_ideal.txt    -> 0_2_1
...
def reconstruction_metrics(coeffs, samples=1500):
    ...
    return {
        "recon_rms_error": float(np.sqrt(np.mean(err**2))),
        "recon_max_error": float(np.max(err)),
        "bbox_diag": bbox_diag,
...
# ============================================================
# Deduplication / summary selection
# ============================================================
...
def build_summary(invariants_df, masses_df, prefer="fseries"):
    ...
    merged["format_rank"] = merged["input_format_detected"].map(lambda x: format_rank(x, prefer=prefer))
    merged["abs_closure_error"] = merged["closure_error"].abs()
    merged["abs_recon_rms_error"] = merged["recon_rms_error"].abs()
    ...
    # Add a simple source note
    summary["selected_reason"] = (
            "preferred_format="
            + prefer
            + "; then max harmonics; then min closure_error; then min recon_rms_error"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Recursive Fourier-knot analyzer v3: invariants.csv, masses.csv, summary.csv from .fseries and/or _ideal.txt"
    )
