...
def load_ideal_matrix(path):
    coeffs = []
    patt = re.compile(r'<Coeff\s+I="([^"]+)"\s+A="([^"]+)"\s+B="([^"]+)"')
...
# ============================================================
# Knot id parsing / file matching
# ============================================================
def parse_knot_id_from_filename(fname):
    base = os.path.basename(fname)

    m = re.match(r"^knot_(.+?)(?:_ideal)?(?:\.[^.]+)?$", base)
    if m:
        return m.group(1).replace(".", "_")

    m = re.match(r"^knot\.(.+?)(?:\.[^.]+)?$", base)
    if m:
        return m.group(1).replace(".", "_")

    stem = os.path.splitext(base)[0].replace("_ideal", "")
    return stem.replace(".", "_")


def possible_raw_names_from_analysis_file(path):
    ...
    if m:
        ...
        if stem.startswith("knot."):
            raw = "knot_" + stem[len("knot."):].replace("_", ".")
            return [f"{raw}.txt"]

    kid = parse_knot_id_from_filename(path)
    if kid:
        return [f"knot_{kid.replace('_', '.')}.txt"]

    return []


def find_matching_raw_txt(analysis_path, root_dir):
    candidates = possible_raw_names_from_analysis_file(analysis_path)
    if not candidates:
        return None

    analysis_dir = os.path.dirname(os.path.abspath(analysis_path))
    ...
    for p in glob.glob(os.path.join(root_dir, "**", "*.txt"), recursive=True):
        if os.path.basename(p) in wanted and not p.endswith("_ideal.txt"):
            return p
...
def eval_series(coeffs, t):
    ...
    N = coeffs.shape[0]
    n = np.arange(1, N + 1, dtype=float).reshape(-1, 1)
    nt = n * t.reshape(1, -1)
...
# Raw resampling
# ============================================================
def resample_raw_closed_curve(coords, num_points=1500):
    ...
    # Fallback: linear arc-length interpolation
    ...
    s_query = np.linspace(0.0, L, num_points, endpoint=False)

    out = np.zeros((num_points, 3), dtype=float)
    j = 0
    for i, sq in enumerate(s_query):
        while j + 1 < len(s_nodes) and s_nodes[j + 1] <= sq:
            j += 1
        j2 = min(j + 1, len(closed) - 1)
        ds = s_nodes[j2] - s_nodes[j]
        if ds <= 0:
            out[i] = closed[j]
        else:
            a = (sq - s_nodes[j]) / ds
            out[i] = (1 - a) * closed[j] + a * closed[j2]
    return out


# ============================================================
# Fit quality raw vs Fourier
# ============================================================
def kabsch_align(X, Y, allow_scale=False):
    """
    Align Y to X after centering.
    Returns Y_aligned, rotation R, scale lam.
    """
    ...
    if allow_scale:
        denom = np.sum(Yc**2)
        lam = float(np.sum(S) / denom) if denom > 0 else 1.0
    else:
        lam = 1.0

    Y_aligned = lam * (Yc @ R) + np.mean(X, axis=0, keepdims=True)
    return Y_aligned, R, lam


def fit_raw_vs_fourier(raw_pts, fs_pts, allow_scale=False, max_shifts=256):
    """
    Compares raw_pts and fs_pts with:
    - translation
    - rotation
    - cyclic shift
    - optional scale
    """
    if raw_pts.shape != fs_pts.shape or raw_pts.shape[0] == 0:
        return {
            "fit_rms_raw_vs_fs": np.nan,
            "fit_max_raw_vs_fs": np.nan,
            "fit_rel_bbox": np.nan,
            "fit_rel_length": np.nan,
            "fit_best_shift": np.nan,
            "fit_rotation_det": np.nan,
            "fit_scale_used": np.nan,
        }

    N = raw_pts.shape[0]

    raw_closed = np.vstack((raw_pts, raw_pts[0]))
    raw_length = float(np.sum(np.linalg.norm(np.diff(raw_closed, axis=0), axis=1)))

    bbox_min = np.min(raw_pts, axis=0)
    bbox_max = np.max(raw_pts, axis=0)
    bbox_diag = float(np.linalg.norm(bbox_max - bbox_min))

    shifts = list(range(N)) if N <= max_shifts else list(np.linspace(0, N - 1, max_shifts, dtype=int))

    best = None
    for k in shifts:
        fs_shift = np.roll(fs_pts, shift=k, axis=0)
        fs_aligned, R, lam = kabsch_align(raw_pts, fs_shift, allow_scale=allow_scale)

        delta = raw_pts - fs_aligned
        err = np.linalg.norm(delta, axis=1)

        rms = float(np.sqrt(np.mean(err**2)))
        mx = float(np.max(err))
        rel_bbox = rms / bbox_diag if bbox_diag > 0 else np.nan
        rel_length = rms / raw_length if raw_length > 0 else np.nan

        cand = {
            "fit_rms_raw_vs_fs": rms,
            "fit_max_raw_vs_fs": mx,
            "fit_rel_bbox": rel_bbox,
            "fit_rel_length": rel_length,
            "fit_best_shift": int(k),
            "fit_rotation_det": float(np.linalg.det(R)),
            "fit_scale_used": float(lam),
        }

        if best is None or cand["fit_rms_raw_vs_fs"] < best["fit_rms_raw_vs_fs"]:
            best = cand
...
# Diagnostics
# ============================================================
def internal_geometry_metrics(coeffs, samples=1500):
...
def load_meta_table(meta_path):
    ...
    std["knot_id"] = std["knot_id"].astype(str).str.replace(".", "_", regex=False)
...
# Summary selection
# ============================================================
def format_rank(fmt, prefer="fseries"):
    if fmt == prefer:
        return 0
    if fmt in ("fseries", "ideal"):
        return 1
    return 2


def build_summary(invariants_df, masses_df, prefer="fseries"):
    merged = invariants_df.merge(
        masses_df,
        on=["file", "relative_path", "input_format_detected", "knot_id", "harmonics_N"],
        suffixes=("_inv", "_mass"),
        how="inner",
    ).copy()

    merged["format_rank"] = merged["input_format_detected"].map(
        lambda x: format_rank(x, prefer=prefer)
    )

    # Use invariant-side fit metric explicitly after merge suffixing
    fit_col = "fit_rel_bbox_inv" if "fit_rel_bbox_inv" in merged.columns else None
    closure_col = "closure_error" if "closure_error" in merged.columns else None

    if fit_col is None:
        merged["fit_rank"] = np.inf
    else:
        merged["fit_rank"] = merged[fit_col].fillna(np.inf)

    if closure_col is None:
        merged["closure_rank"] = np.inf
    else:
        merged["closure_rank"] = merged[closure_col].abs()
    ...
    summary = merged.groupby("knot_id", as_index=False).first()

    summary["selected_reason"] = (
            "preferred_format="
            + prefer
            + "; then max harmonics; then min fit_rel_bbox; then min closure_error"
    )
...
def main():
    ap = argparse.ArgumentParser(
        description="Recursive Fourier-knot analyzer v4: invariants.csv, masses.csv, summary.csv with true raw-vs-Fourier fit quality"
    )
    ...
    ap.add_argument("--prefer-format", type=str, default="fseries", choices=["fseries", "ideal"])
    ap.add_argument("--invariants-out", type=str, default="invariants.csv")
    ap.add_argument("--masses-out", type=str, default="masses.csv")
    ap.add_argument("--summary-out", type=str, default="summary.csv")
    ...
    ap.add_argument("--samples", type=int, default=1500, help="Samples along Fourier curve")
    ap.add_argument("--fit-samples", type=int, default=1500, help="Samples for raw-vs-Fourier fit")
    ap.add_argument("--fit-max-shifts", type=int, default=256, help="Max cyclic shifts tested")
    ap.add_argument("--fit-allow-scale", type=str, default="no", choices=["yes", "no"])
    ...
    allow_scale = (args.fit_allow_scale == "yes")
    ...
    t_fit = np.linspace(0, 2 * math.pi, args.fit_samples, endpoint=False)
    ...
    for path in files:
        ...
        geom = internal_geometry_metrics(coeffs, samples=min(args.samples, 2000))
        ...
        ratio_energy_to_me = M_energy / m_e if m_e != 0 else np.nan
        ratio_energy_to_mp = M_energy / m_p if m_p != 0 else np.nan
        ratio_fluid_to_me = M_fluid / m_e if m_e != 0 else np.nan
        ratio_fluid_to_mp = M_fluid / m_p if m_p != 0 else np.nan

        raw_path = find_matching_raw_txt(path, root)
        fit_source_found = raw_path is not None

        if fit_source_found:
            raw_coords = load_raw_xyz(raw_path)
            raw_resamp = resample_raw_closed_curve(raw_coords, num_points=args.fit_samples)
            fs_fit, _ = eval_series(coeffs, t_fit)

            if raw_resamp.shape == fs_fit.shape and raw_resamp.shape[0] > 0:
                fit = fit_raw_vs_fourier(
                    raw_resamp,
                    fs_fit,
                    allow_scale=allow_scale,
                    max_shifts=args.fit_max_shifts,
                )
            else:
                fit = {
                    "fit_rms_raw_vs_fs": np.nan,
        ...
        inv_rows.append({
            ...
            "bbox_diag": geom["bbox_diag"],
            "rms_radius": geom["rms_radius"],
            "fit_source_txt_found": bool(fit_source_found),
            "fit_source_txt_path": "" if raw_path is None else os.path.relpath(raw_path, root),
            "fit_rms_raw_vs_fs": fit["fit_rms_raw_vs_fs"],
            "fit_max_raw_vs_fs": fit["fit_max_raw_vs_fs"],
            "fit_rel_bbox": fit["fit_rel_bbox"],
            "fit_rel_length": fit["fit_rel_length"],
            "fit_best_shift": fit["fit_best_shift"],
            "fit_rotation_det": fit["fit_rotation_det"],
            "fit_scale_used": fit["fit_scale_used"],
        })
        ...
        mass_rows.append({
            ...
            "mass_fluid_over_m_e": ratio_fluid_to_me,
            "mass_fluid_over_m_p": ratio_fluid_to_mp,
            "mass_energy_over_m_e": ratio_energy_to_me,
            "mass_energy_over_m_p": ratio_energy_to_mp,
            "fit_source_txt_found": bool(fit_source_found),
            "fit_rms_raw_vs_fs": fit["fit_rms_raw_vs_fs"],
            "fit_rel_bbox": fit["fit_rel_bbox"],
            "type_meta": type_meta,
    ...
    summary_df = build_summary(inv_df, mass_df, prefer=args.prefer_format)
    ...
    print("Unique knot_ids in summary:", len(summary_df))
    print(f"K_fluid = {K_fluid:.6e} kg/m")
    print(f"K_energy = {K_energy:.6e} kg/m")
    print(f"fit_allow_scale = {allow_scale}")
    print(f"SciPy available for spline resampling = {SCIPY_AVAILABLE}")
