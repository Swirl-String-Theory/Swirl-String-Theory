...
\label{canon58:emp-anchors}
\begin{align*}
 m_W &= 80.377~\mathrm{GeV}, & m_Z &= 91.1876~\mathrm{GeV},\\
 \sin^2\theta_W &= 0.23121 \pm 0.00004, & v_\Phi &\approx 246.22~\mathrm{GeV},\\
 \vnorm &= 1.09384563\times10^{6}~\mathrm{m/s}, & \rc &= 1.40897017\times10^{-15}~\mathrm{m},\\
 \rhof &= 7.0\times10^{-7}~\mathrm{kg/m^3}, & \rhom &= 3.8934358266918687\times10^{18}~\mathrm{kg/m^3},\\
 \FmaxEM &= 2.9053507\times10^{1}~\mathrm{N}, & \FmaxG &= 3.02563\times10^{43}~\mathrm{N}.
\end{align*}
\end{tcolorbox}
% Check: [units ok; limit → none]
\paragraph{Calibration Notes.}
$m_W,m_Z,\sin^2\theta_W$ fix $v_\Phi=2m_W/g$. The constants $\{\vnorm,\rc,\rhof,\rhom,\FmaxEM,\FmaxG\}$ are set by canonical definitions and experimental protocols.
...
\]
Defining $\mathbf E=-\partial_t\mathbf a$ and $\mathbf B=\nabla\times\mathbf a$ recovers the Maxwell wave equation and energy density
\[
    u_{\rm EM}=\tfrac{1}{2}\big(\mathbf E^2 + c^2\mathbf B^2\big).
\]
A photon is modeled as a pulsed, unknotted swirl string $K\cong S^1$ obeying the lossless condition $\nabla\cdot\mathbf a=0$ and carrying energy $\hbar\omega$ without core mass. See Appendix~E for the extended derivation.
...
Empirical values from the anchors box (Sec.~\ref{canon58:emp-anchors}) set
\[
    m_W=80.377~\mathrm{GeV},\quad m_Z=91.1876~\mathrm{GeV},\quad \sin^2\theta_W=0.23121\pm0.00004.
\]
Director-elasticity reduces to the mixing relations
\[
    A_\mu = \sin\theta_W\, W^3_\mu + \cos\theta_W\, B_\mu,\qquad \tan\theta_W=\frac{g'}{g},
\]
and masses
\[
    m_W=\tfrac12 g\,v_\Phi,\qquad m_Z=\tfrac12\sqrt{g^2+g'^2}\,v_\Phi.
\]
Using the anchors gives $v_\Phi\approx246.22~\mathrm{GeV}$ and reproduces the observed $\sin^2\theta_W$.
% Check: [units ok; limit → none]
...
\[
    \Gamma_{R\to T}=\int \chi\,u\,\mathcal F\,d^3x.
\]
In the near-field single-mode limit,
\[
    \Gamma_{R\to T}\approx \chi_{\rm eff}\,\frac{P}{A_{\rm eff}},
\]
with geometry entering only through $A_{\rm eff}$. Experimental bounds give $\chi_{\rm eff}\leq \chi_{\rm eff}^{\max}$.
% Check: [units ok; limit → none]
...
%========================================================================================
\section{Systematic Dimensional \& Recovery Checks}\label{canon58:checks}
Table~\ref{canon58:dim-checks} in Appendix~B summarizes unit and limit checks for canonical results.

% [STATUS: Canonical] [SOURCE: v0.4.4 §§1--6]

%========================================================================================
\appendix
\section{Swirl Hamiltonian Density}\label{canon58:appA}
Includes $\rc^2\|\omegas\|^2$ and $\rc^4\|\nabla\omegas\|^2$; Kelvin-compatibility proof.
\section{Dimensional Analyses \& Recovery Limits}\label{canon58:appendixB}
\begin{tabular}{|c|c|c|}
\hline
Item & Units & Limit \\
\hline
Chronos--Kelvin & $\mathrm{m^2\,s^{-1}}$ & Newtonian \\
$\rhof$ derivation & $\mathrm{kg\,m^{-3}}$ & Newtonian \\
Hydrogen soft-core & $\mathrm{J}$ & Coulomb/Bohr \\
Swirl pressure law & $\mathrm{Pa}$ & Newtonian \\
\hline
\end{tabular}
\captionof{table}{Dimensional and recovery checks.}\label{canon58:dim-checks}
\section{Derivation of $\rhof$}\label{canon58:appC}
Full coarse-graining calculation (Sec.~\ref{canon58:rho_f}).
\section{Hydrogen Soft-Core Numerics}\label{canon58:appD}
$a_0$, $E_1$ with uncertainty propagation.
\section{Photon/Unknot Sector}\label{canon58:appE}
Lossless conditions and pulsed construction.
\section{Swirl Pressure Law\textendash Galaxy-Scale Integrals}\label{canon58:appF}
Extended derivation of Sec.~\ref{canon58:pressure}.
\section{Calibration Protocol Notes}\label{canon58:appG}
How $\{\vnorm,\rc,\rhof,\rhom,\FmaxEM,\FmaxG\}$ are fixed.
\section{Experimental Status \& Bounds}\label{canon58:appH}
Extraction of $\chi_{\rm eff}^{\max}$; bounds table.
\section{Notation, Ontology, Glossary}\label{canon58:appI}
