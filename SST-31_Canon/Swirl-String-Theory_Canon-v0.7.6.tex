...
% TOC Customization
\usepackage{tocloft}
\usepackage{calc} % for \settowidth
...
% ----------------------------
    \newcommand{\PanelSeparated}[3]{% shiftx, shifty, title
        ...
        \begin{scope}[shift={(\Whalf,\Hhalf)},scale=0.72]

        % Two disjoint loops, sized to stay inside the panel
        \draw[line width=0.6pt] (-1.55, 0.65) circle (1.00);
        \draw[line width=0.6pt] ( 1.75, 0.10) circle (0.80);

        % Small shed strand
        \draw[line width=0.6pt] (-0.55,-1.85) -- (0.85,-1.85);
