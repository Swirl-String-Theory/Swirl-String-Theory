...
\newcommand{\canonversion}{v0.7.6} % Semantic versioning: vMAJOR.MINOR.PATCH
...
% TOC Customization
% NOTE (REVTeX): tocloft conflicts with revtex4-2 (\@starttoc already defined).
% Comment it out and use a simple TOC macro instead.
%\usepackage{tocloft}
...
% REVTeX-safe fallback: make \compacttoc just call \tableofcontents.
\makeatletter
\@ifundefined{compacttoc}{%
  \newcommand{\compacttoc}{\tableofcontents}%
}{%
  \renewcommand{\compacttoc}{\tableofcontents}%
}%
\makeatother
...
\begin{figure}[h]
    ...
    \newcommand{\PanelReconnect}[3]{% shiftx, shifty, title
      \PanelFrame{#1}{#2}{#3}
      \begin{scope}[shift={(#1,#2)}]
        \begin{scope}[shift={(\Whalf,\Hhalf)},scale=0.65]
          \clip (-3.55,-3.55) rectangle (3.55,3.55);
          % Schematic "reconnection-like" path (placeholder; not topology-preserving)
          \draw[line width=0.6pt]
            (-3.2,-1.8)
            .. controls (-2.4, 1.4) and (-0.3, 2.6) .. ( 1.2, 1.4)
            .. controls ( 2.5, 0.3) and ( 2.3,-1.1) .. ( 0.6,-1.7)
            .. controls (-0.7,-2.2) and (-1.8,-2.2) .. (-3.2,-1.8);
          \draw[line width=0.6pt]
            (1.0, -0.2)
            .. controls (2.4, 0.0) and (3.0, 0.9) .. (2.1, 1.5)
            .. controls (1.4, 2.0) and (0.6, 1.2) .. (0.9, 0.4)
            .. controls (1.1, 0.0) and (1.1,-0.1) .. (1.0,-0.2);
          \draw[line width=0.6pt] (0.6,-1.7) -- (3.3,-1.7);
        \end{scope}
      \end{scope}
    }
    ...
    \newcommand{\PanelSeparated}[3]{% shiftx, shifty, title
      ...
      \begin{scope}[shift={(#1,#2)}]
        \begin{scope}[shift={(\Whalf,\Hhalf)},scale=0.70]
          \clip (-3.55,-3.55) rectangle (3.55,3.55);
          % Two separated loop components (placeholder)
          \draw[line width=0.6pt]
            (-2.8, 0.4) arc[start angle=180,end angle=0,radius=1.6];
          \draw[line width=0.6pt]
            ( 1.6,-0.4) arc[start angle=180,end angle=0,radius=1.2];
          \draw[line width=0.6pt] (-1.2,-1.2) -- (1.0,-1.2);
        \end{scope}
      \end{scope}
    }
...
\section{Background References for v0.7.6 Additions}
\label{app:v076-refs}
