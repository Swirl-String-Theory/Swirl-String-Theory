...
# With 1 pass, make_depth_schedule() returns [deep_sink], so we get deepest CW and deepest CCW only.
if ULTRA_QUICK_RENDER:
    # Ultra quick: 1 pass CW at deepest + 1 pass CCW at deepest (fastest, full depth)
    CW_PASSES = 1
    CCW_PASSES = 1
    print(f"  Ultra quick: CW_PASSES={CW_PASSES}, CCW_PASSES={CCW_PASSES} (normal: CW={CW_PASSES_BASE}, CCW={CCW_PASSES_BASE})")
    print(f"  Rendering deepest CW and deepest CCW coil only (one pass each at full depth)")
elif QUICK_RENDER:
    # Quick render: 1 pass CW at deepest + 1 pass CCW at deepest (full depth, fewer ops)
    CW_PASSES = 1
    CCW_PASSES = 1
    print(f"  Quick render: CW_PASSES={CW_PASSES}, CCW_PASSES={CCW_PASSES} (normal: CW={CW_PASSES_BASE}, CCW={CCW_PASSES_BASE})")
    print(f"  Rendering deepest CW and deepest CCW coil only (one pass each at full depth)")
else:
