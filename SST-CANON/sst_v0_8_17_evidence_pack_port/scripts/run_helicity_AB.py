...
def _ensure_gui_tabs_on_path() -> None:
    """Locate SST_Dashboard/gui_tabs so ``import sst_helicity`` works."""
    env = os.environ.get("SST_GUI_TABS")
    if env:
        p = Path(env)
        if p.is_dir():
    ...
    candidates = []
    for n in range(2, min(10, len(here.parents))):
        root = here.parents[n - 1]
        candidates.extend([
            root / "SSTcore" / "SST_Dashboard" / "gui_tabs",
            root / "SST-Workbench" /  "SST-dashboard" / "sstcore" / "gui_tabs",
            root / "SST-Workbench" /  "SST-dashboard" / "swirl" / "gui_tabs",
        ])
    for c in candidates:
        if (c / "sst_helicity.py").is_file():
            if str(c) not in sys.path:
                sys.path.insert(0, str(c))
            return

    raise ImportError(
        "sst_helicity not found. Set SST_GUI_TABS to the gui_tabs directory, or "
        "run from a checkout that includes SSTcore/SST_Dashboard/gui_tabs or "
        "SST-Workbench/gui/dashboard/sstcore/gui_tabs."
    )
