...
%! DOI = 10.5281/zenodo.21207672
%! v0.8.17 edition: T-foliation discipline and corpus integration bundle (been_processed).

\newcommand{\paperdoi}{10.5281/zenodo.21207672}
\newcommand{\papertitle}{Swirl-String-Theory Canon-v0.8.17}
\newcommand{\paperkeywords}{Swirl String Theory, SST, Canon, Topological Hydrodynamics, Knot Theory, Relational Time, Delay Dynamics, Epistemic Classification, T-foliation, ropelength convention, vacuum dual medium, research track corpus}
...
\paragraph{Absolute foliation time versus local clock time.}
        SST uses two logically distinct time layers.  The absolute foliation
        parameter \(T\) labels the instantaneous incompressible constraint slice
        \(\Sigma_T\).  Local clock readings are physical rates carried by
        ...
        \(d\tau/dT=S_{(t)}\).  The no-signalling status of the pressure
...
% ===========================
    ...
    \subsection{Pure geometric baseline branch}
        ...
        \begin{align}
            M_0(T)
            =
            2\pi^3\,\rhoM\,\frac{\rc^5}{\lambda_c^2}\,\mathcal{L}_{\mathrm{tot}}(T).
            \label{eq:pure_geometric_mass_baseline}
        \end{align}
        This branch is subordinate to Eq.~\eqref{eq:sst_master_equation}. It records a normalization-level geometric closure used in historical mass-program benchmarks; it does not replace the canonical master functional or the explicit kernel branch below.
    ...
    \subsection{Use policy}
        ...
        \end{enumerate}
        It is deliberately not promoted in v0.8.17 to the same status as the primitive constant chain, the Swirl-Clock, or the delay-selection sector.
...
% ---------------------------
        ...
        \input{SST_CANON-v0.8.17-research-track}
