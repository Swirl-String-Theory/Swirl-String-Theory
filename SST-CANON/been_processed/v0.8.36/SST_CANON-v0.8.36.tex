%! DOI = 10.5281/zenodo.21922231
%! Author = Omar Iskandarani
%! Date = 2026-08-02
%! Affiliation = Independent Researcher, Groningen, The Netherlands
%! ORCID = 0009-0006-1686-3961
%! v0.8.36 edition: Maxwell swirl-tonic, kinetic closure, dynamical-field / reciprocal-stress / mechanical falsifiers, and spectroscopic-response guards (been_processed).

\newcommand{\paperdoi}{10.5281/zenodo.21922231}
\newcommand{\canonversion}{0.8.36}
\newcommand{\papertitle}{Swirl-String-Theory Canon-v0.8.36}
\newcommand{\paperkeywords}{Swirl String Theory, SST, Canon, Topological Hydrodynamics, Knot Theory, Relational Time, Delay Dynamics, Epistemic Classification, swirl tonic, kinetic closure, reciprocal stress, dynamical field closure, spectroscopic response}
...
\begin{document}
    ...
    \begin{abstract}
        \noindent
        This document presents Swirl-String Theory (SST) as a structured canonical synthesis rather than as a claim that every phenomenological sector has already been derived to theorem level. The text records the impedance--resonance and free-to-bound-mode origin of the programme; separates the unfixed quasi-static response coefficient from its legacy numerical reference while retaining the characteristic swirl speed, Compton anchoring, and horn/circulation scale; assigns a minimal relational link-field action as the microscopic research candidate beneath the continuum torsion bridge; records the operational special-relativity postulate that stable vortex matter constructs local spacetime with clocks, rods, and luminal R-phase torsion pulses; adds a conditional action--phase mass--shell route in which the internal carrier phase slows by the Lorentz factor without requiring translational flattening or stretching of the knot; and organizes delay selection, atomic bridges, spectroscopy, knot-geometry certification, KAM-caged dynamical stability, relational time, and topology-based mass within one explicitly stratified framework. Throughout, historical provenance, calibrated identities, SST-internal derivations, predictions, and conjectural extensions are kept distinct so that the canon remains internally coherent, externally comparable, and open to falsification. Version~0.8.33 additionally fixes the differential-form degree rules for any covariant vortex-worldsheet description, introduces a guarded parity-sensitive response interface, and replaces inherited gauge-emergence assertions by an explicit algebra, representation, anomaly, and phenomenology certification ladder. Version~0.8.35 additionally derives the factor $8\pi/3$ from the isotropic incompressible transverse projector, separates radius- and diameter-normalized ropelength conventions, connects the leading trefoil response to an alpha-blind finite-core Euler--Biot--Savart completion, uses helicity as a constraint on parity-even material-twist energy rather than as a linear Maxwell correction, and states the remaining electromagnetic gauge-stiffness matching gates explicitly. Version~0.8.36 additionally canonizes the material velocity as a vorticity-potential (``swirl-tonic'') representation, supplies a Stokes/holonomy convergence falsifier, and explicitly prevents that material representation from being conflated with the independent transverse/link field $\mathbf A_{\mathrm{eff}}$.
    \end{abstract}
...
\subsection{Regime of applicability of ideal-knot geometry}
\label{subsec:ideal_knot_regime_guard}

\paragraph{Compact reference state.}
\textbf{[CANON / ANSATZ / SCOPE GUARD].}
The ideal trefoil is retained as the maximally thick, ropelength-critical
reference geometry for the compact finite-core electron candidate,
\begin{equation}
    \mathbf X_{\rm compact}
    \simeq
    \mathbf X_{\rm ideal,3_1}.
    \label{eq:ideal_trefoil_compact_reference}
\end{equation}
Equation~\eqref{eq:ideal_trefoil_compact_reference} does not assert that the
ideal trefoil minimizes the radius of the smallest enclosing sphere, minimizes
the complete Euler kinetic-energy functional, or represents every excited
state of the electron.  ``Ideal'' refers to the declared ropelength problem and
its thickness convention; it is not a universal dynamical ground-state label.

\paragraph{Finite-core and slender-filament regimes.}
For a resolved tube with effective core radius $a_{\rm core}$, maximum
centreline curvature $\kappa_{\max}$, and minimum nonlocal centreline separation
$d_{\rm sep}$, define
\begin{equation}
    \epsilon_{\kappa}
    ...
    \qquad
    \epsilon_{\rm sep}
    :=\frac{a_{\rm core}}{d_{\rm sep}}.
    \label{eq:ideal_knot_regime_diagnostics}
\end{equation}
The canon distinguishes
\begin{equation}
\begin{array}{lll}
    \text{compact finite-core regime}
    &:&
    \epsilon_{\kappa}=O(1)
    \ \text{or}\ 
    \epsilon_{\rm sep}=O(1),
    \\
    \text{slender-filament regime}
    &:&
    \epsilon_{\kappa}\ll1
    \ \text{and}\ 
    \epsilon_{\rm sep}\ll1.
\end{array}
\label{eq:ideal_knot_regime_split}
\end{equation}
The ideal trefoil belongs primarily to the compact finite-core regime because
its defining constrained optimum can contain active curvature and contact
constraints.  Local-induction, logarithmic slender-body, and perturbative
$B_{\rm eff}\kappa^2$ reductions are not promoted on that state unless
Eq.~\eqref{eq:ideal_knot_regime_diagnostics} and a nonlocal-induction residual
show that a controlled asymptotic regime has actually been reached.

\paragraph{Kirchhoff--Cosserat non-equivalence guard.}
A rod functional may be evaluated as a diagnostic,
\begin{equation}
    E_{\rm rod}^{\rm diag}[\mathbf X,\Omega_3]
    =
    T L[\mathbf X]
    +\frac{B}{2}\int\kappa^2\,ds
    +\frac{C}{2}\int\Omega_3^2\,ds,
    \label{eq:rod_diagnostic_functional}
\end{equation}
where $\Omega_3$ is the material twist strain.  Such an evaluation does not
show that $\mathbf X$ is stationary under this functional and does not supply
canonical SST moduli $B$ and $C$.  An ideal incompressible inviscid fluid has no
material static shear modulus.  Its kinetic energy may nevertheless reduce to
a geometry-dependent first-order vortex Hamiltonian; that reduced Hamiltonian
must not be relabelled as an elastic-rod potential without a separate
hydrodynamic derivation.

Fukumoto's vortex-jet/Kirchhoff correspondence identifies a class of permanent
translating filament shapes with Kirchhoff-rod equilibrium shapes under
specific localized-induction and axial-core-flow assumptions
\cite{Fukumoto2007}.  It is a stationary-shape correspondence, not an
identification of the complete rod and vortex phase spaces.  Consequently
Kirchhoff--Cosserat dynamics, elastic normal modes, and rod constitutive
coefficients are not imported into SST particle dynamics.  Physical elastic
trefoils can also relax to shapes measurably different from perfectly flexible
ideal knots \cite{JohannsEtAl2021}.

\paragraph{Preferred topological fluid bridge.}
For one coherently framed closed thin vortex tube of circulation $\Gamma$, the
hydrodynamic self-linking relation is
\begin{equation}
    \mathcal H
    =
    \Gamma^2 Lk
    =
    \Gamma^2\left(Wr+Tw\right),
    \label{eq:moffatt_ricca_single_tube_helicity}
\end{equation}
under the usual ideal-flow and thin-tube hypotheses
\cite{MoffattRicca1992}.  Equation~\eqref{eq:moffatt_ricca_single_tube_helicity}
provides the canonical topology--fluid bridge.  It carries circulation and
helicity content but introduces no elastic bending modulus.

\paragraph{Use policy.}
The ideal trefoil may be used as a certified compact reference geometry and as
input to finite-core diagnostics.  Kirchhoff--Cosserat quantities may be
reported as comparative geometric diagnostics.  Neither use establishes that
the same geometry is an Euler-energy minimizer, a rod equilibrium, or a
slender-filament solution.  Excited stretched states and the transition from
the compact to the slender regime are retained in the companion research
track, Section~\ref{subsec:rt_compact_to_slender_state}.
...
\section{Formal Foundations}
    ...
    \subsection{Primitive Structure of the Theory}
        ...
        \textbf{Primitive status sets:}
        \begin{align}
            \mathcal{P}_{\mathrm{cal}}
            &=
            \{\vchar,\omega_c\},
            \\
            \mathcal{P}_{\mathrm{open}}
            &=
            \{\rhoSub,\rhoEff\},
            \\
            ...
            &=
            \{\rhoRef\}.
            \label{eq:primitive_status_sets_v0832}
        \end{align}

        Here \(\vchar=\alpha c/2\) and
        \(\omega_c=m_ec^2/\hbar\) are electron-anchored calibration entries.
        The material density \(\rhoSub\) and the quasi-static response
        \(\rhoEff\) remain physically unfixed.  The numerical quantity
        \begin{align}
            \boxed{
            \rhoRef
            :=
            ...
            \label{eq:legacy_density_reference_v0832}
        \end{align}
        is retained only to reproduce inherited tables and benchmarks.

        \textbf{[CANON / NOTATION].}
        \(\vchar\) is a calibrated scalar speed.  The local swirl-velocity and
        ...
        \end{align}
        The symbol \(\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\) without
        explicit arguments is reserved for the characteristic carrier velocity.

        \textbf{[ORTHODOX].}
        \(\omega_c=m_ec^2/\hbar\) is the electron Compton angular frequency.

        \textbf{[SPECULATIVE].}
        Interpreting \(\omega_c\) as the primitive phase frequency of an SST
        carrier is an SST postulate; it is not a local vorticity variable.

        \textbf{[LEGACY REFERENCE NORMALIZATION / PROVENANCE INVALIDATED].}
        The value in Eq.~\eqref{eq:legacy_density_reference_v0832} is not a
        calibrated observable and is not a member of
        \(\mathcal P_{\mathrm{cal}}\).  Its traced VAM-7 provenance fails four
        independent checks: the generating expression has units
        \(\mathrm{kg\,m^{-1}}\), the selected volume was changed by the factor
        \(8/3\) after the first result was judged macroscopically too large, the
        claimed cosmological confirmation contains an error of approximately
        \(1.9\times10^{18}\), and the source alternates between two vorticity
        scales differing by \(2/\alpha\).  The source does identify a historical
        rotational microinertia,
        \(\mathcal J_\omega^{\rm VAM}\simeq
        6.84\times10^{-7}\,\mathrm{kg\,m^{-1}}\), but it does not determine a
        volumetric density.  See
        Subsection~\ref{subsec:rt_historical_rhof_provenance_audit}.

        \textbf{[DEPENDENCY GUARD].}
        The physical coefficient \(\rhoF\equiv\rhoEff\) is therefore an
        \emph{unfixed} quasi-static response.  Symbolic equations may continue to
        use \(\rhoF\), but a numerical evaluation at \(\rhoRef\) must be labelled
        \textbf{[LEGACY-REFERENCE NORMALIZED]}.  Rescaling \(\rhoEff\) does not by
        itself change \(\vchar\), \(\omega_c\), \(\rc=\vchar/\omega_c\), or
        \(\Gamma_0\).  Absolute pressures, energies, forces, impedances, and
        quantum/action amplitudes that depend on \(\rhoEff\) are not predictions
        until an independent class-C or class-Q observable fixes the response;
        see Subsection~\ref{subsec:rt_rhof_scaling_audit_v0832}.


    \subsection{Density Ontology and Backward-Compatible Symbol Rule}
        \label{subsec:density_ontology_v0830}

        \textbf{[CANON / SEMANTIC SEPARATION].}
        The present edition separates material density, effective translational
        inertia, rotational microinertia, and line inertia:
        \begin{align}
            \rhoSub
            &\quad [\mathrm{kg\,m^{-3}}]
            &&\text{material density of the fundamental incompressible substrate},
            \\
            \rhoEff
            &\quad [\mathrm{kg\,m^{-3}}]
            &&\text{quasi-static effective inertial-response density},
            \\
            \Jomega
            &\quad [\mathrm{kg\,m^{-1}}]
            &&\text{rotational/twist microinertia per volume},
            \\
            \muell
            &\quad [\mathrm{kg\,m^{-1}}]
            &&\text{resolved or effective line-mass coefficient}.
        ...
        \begin{align}
            \boxed{
            \rhoF\equiv\rhoEff
            \quad\text{\textbf{[UNFIXED RESPONSE]}}
            }
            \label{eq:rhof_effective_response_alias}
        \end{align}
        while the historical numerical benchmark is denoted separately by
        \(\rhoRef\) as defined in
        Eq.~\eqref{eq:legacy_density_reference_v0832}.  In particular,
        \(\rhoF=\rhoRef\) is not a physical identity.  No equality
        ...
        A displacement-field kinetic term may use
        \begin{align}
            u_{\rm disp}
            =
            \frac12\rhoEff\lVert\partial_t\mathbf A\rVert^2,
            \qquad
            [\mathbf A]=\mathrm m,
        \end{align}
        whereas an angular, twist-rate, or vorticity-square term requires
        \begin{align}
            u_{\rm rot}
            =
            \frac12\Jomega\lVert\partial_t\boldsymbol\theta\rVert^2
            \quad\text{or}\quad
            u_{\omega}
            =
        ...
        \begin{align}
            \Jomega
            =
            \beta_{\omega u}\,\rhoEff\,\ell_{\omega u}^{2},
            \label{eq:rotational_translational_inertia_bridge_target}
        \end{align}
        but $\beta_{\omega u}$ and $\ell_{\omega u}$ are research outputs, not
        ...
        The unfixed $\rhoF\equiv\rhoEff$ belongs to the long-wavelength
        effective-response model and must not be cited as a measured ordinary
        rest-mass density of the vacuum or of a resolved vortex tube.
        The number $\rhoRef$ is only a legacy benchmark.

        \textbf{[CANON / QUASI-STATIC RESPONSE GUARD].}
        If a dispersive or anisotropic effective-inertia tensor is introduced,
        it must be written explicitly as
        $\boldsymbol{\rho}_{\rm eff}(\omega,\mathbf{k})$.  For an isotropic
        ...
        \begin{align}
            ...
            \boldsymbol{\rho}_{\rm eff}(\omega,\mathbf{k})
            \right]
            }
            \label{eq:rhof_quasistatic_response_limit}
        \end{align}
        whenever this limit exists.  Equation~\eqref{eq:rhof_quasistatic_response_limit}
        fixes the meaning of the symbol; it does not assign a numerical value.
        The scalar $\rhoF$ must not be extrapolated to core
        ...
        A conversion such as $\rhoEff=\Jomega/\ell^2$ is not closed until the
        ...
        reference participation ratio of order $10^{-26}$ between the
        microscopic $r_c$ normalization and the legacy value $\rhoRef$; see
    ...
    \subsection{Circulation Quantization}
        ...
        \textbf{[DERIVED within the calibrated chain]}: In SST, $\Gamma_0$ is no longer an independent input but follows from Eq.~\ref{eq:rc_definition}. Since $\rc$ itself carries the [CALIBRATED CHAIN GUARD] of Subsection~\ref{subsec:derived_horn_circulation_radius}, this derived status is relative to the electron-anchored set $(\vchar,\omega_c)$; it must not be cited as an independent hydrodynamic derivation of the circulation quantum.
...
\subsection{Hodge Decomposition and Topological Circulation Sectors}
    ...
    \subsection{Density and Inertia Hierarchy}
        
        We distinguish the following material, response, energetic, and
        line-inertia quantities:
        ...
        \begin{align}
            ...
            \\
            \rhoF\equiv\rhoEff
            &\quad \text{(unfixed quasi-static effective response)},
            \\
            \rhoE
            &= \tfrac12 \rhoEff \, \vchar{}^2
            \quad \text{(response-dependent swirl energy density)},
            \\
            \rhoM
            &= \rhoE / c^2
            ...
            \\
            \muell
            &\quad \text{(resolved or effective line-mass coefficient)},
            \\
            \rhohorn
    ...
    \subsection{Reparametrized Zero-Parameter Corollary}
        ...
        Older SST canon layers often used the triplet $(\Gamma_0,\rhoF,\rc)$ as a primitive parameterization. In the present \canonversion{} architecture, Eq.~\eqref{eq:rc_definition} and Eq.~\eqref{eq:gamma0} show that the geometric pair $(\Gamma_0,\rc)$ is a reparameterization of $(\vchar,\omega_c)$, while $\rhoF\equiv\rhoEff$ remains a separate open response coefficient rather than a calibration entry.
        ...
        Hence any dimensional SST quantity written in terms of $(\Gamma_0,\rhoF,\rc)$ can be rewritten in terms of $(\rhoEff,\vchar,\omega_c)$, with one open response coefficient and no additional geometric calibration input.
    ...
    \label{sec:master-equation}
        ...
        $\rhoF\equiv\rhoEff$ is the unfixed quasi-static response coefficient.
        ...
        \paragraph{Configuration-resolved spectroscopic-response guard.}
        \label{par:canon_configuration_resolved_spectroscopic_response_guard}
        \textbf{[CANON / RESPONSE-DEPENDENCY GUARD].}
        A transition-frequency shift associated with an SST configuration $K$ is not
        an additional mass formula.  It is defined by the variation of a transition
        energy,
        \begin{align}
            \Delta E_i^{(K)}
            &\equiv
            ...
            &\equiv
            \frac{\Delta E_i^{(K)}}{h},
            \label{eq:canon_configuration_transition_energy_def}\\
            \delta\nu_i^{(K)}
            &=
            \frac{1}{h}\,\delta\!\left(\Delta E_i^{(K)}\right)
            =
            \frac{1}{h}
            \sum_A
            ...
            \,\delta q_A^{(K)}
            +O(\delta q^2),
            \label{eq:canon_configuration_spectroscopic_linear_response}
        \end{align}
        where $q_A^{(K)}$ denotes a declared set of independent state variables.
        Admissible examples include $M_K$, an inertia tensor $I_{K,ab}$, resolved
        geometric or contact variables, and the clock factor.  Equation~\eqref{eq:canon_configuration_spectroscopic_linear_response}
        defines only the dependency structure of a future response theory.  It does
        not presently derive atomic levels, selection rules, or transition strengths.

        \textbf{[CANON / NO-DOUBLE-COUNTING RULE].}
        Because $\mathcal H(K)$ is already contained in the canonical kernel $\Xi_K$,
        $\delta\mathcal H_K$ and $\delta\Xi_K$ must not be treated as independent
        fit directions unless an explicit change of variables, projection, or
        orthogonalization is supplied.  One may use either the compressed kernel
        response $\delta\Xi_K$ or a resolved basis such as
        $\{\delta C_K,\delta L_K,\delta\mathcal H_K,\delta k_K\}$, but not both
        without an independently specified dependency matrix.

        \textbf{[CANON / NON-PREDICTION GUARD].}
        The present canon contains no first-principles SST derivation of a complete
        atomic energy functional $E_{n\ell jm}^{(K)}$.  Consequently, generalized
        isotope-shift or King-plot residuals may be used as research diagnostics, but
        they do not by themselves identify an SST sector or constitute a prediction
        of atomic spectroscopy.  The corresponding response programme is retained in
        the research track.
    ...
    \subsection{Summary of Dependency Structure}
        ...
        (\vchar,\omega_c)
            ...
            \\
        \rhoEff
            &\rightarrow (\rhoE,\rhoM,\text{absolute response amplitudes}),
            \\
        \rhoRef
            &\rightarrow \text{legacy-reference numerical benchmarks only}.
        \end{align}
    ...
    \label{sec:Fmax}
        ...
        Within the electron-anchored calibration set
        $\mathcal{P}_{\mathrm{cal}}=\{\vchar,\omega_c\}$, a characteristic force scale emerges naturally from the Compton energy stored over the canonical circulation cross-section. We define the \emph{maximal swirl tension} as
...
\section{Axiomatic Framework}
    ...
    \subsection{Axiom 1: Continuum Substrate}
        ...
        \textbf{Statement.}
        There exists a continuous, incompressible, inviscid medium filling
        $\mathcal{M}$, characterized by a positive constant material density
        $\rhoSub$.
        
        \begin{align}
            \nabla \cdot \mathbf{v} = 0,
            \qquad
            \rhoSub=\text{constant}>0.
        \end{align}
...
\section{Consistency and Classification Framework}
    ...
    \subsection{Covariant vortex-worldsheet form-degree guard}
        \label{subsec:covariant_vortex_worldsheet_guard}

        \textbf{[ORTHODOX DIFFERENTIAL-FORM GUARD / RESEARCH-TRACK INTERFACE].}
        A covariant thin-string description may use a two-form potential
        \(B\in\Omega^2(M)\) and its three-form field strength
        \begin{align}
            H=dB\in\Omega^3(M),
            \qquad
            H_{\mu\nu\rho}=3\nabla_{[\mu}B_{\nu\rho]},
            \label{eq:canon_two_form_degree_guard}
        \end{align}
        where the brackets have unit-weight antisymmetrization.  A closed
        two-dimensional string worldsheet \(\Sigma\) may couple electrically through
        the pullback of \(B\),
        \begin{align}
            S_{\Sigma B}=q_B\int_{\Sigma}X^{*}B,
            \label{eq:canon_worldsheet_B_coupling}
        \end{align}
        because both the integration domain and the pulled-back form have degree two.
        By contrast, \(\int_{S^2}H\) is not a defined flux integral because
        \(H\) has degree three.  In \(3+1\) dimensions an electric string charge is
        instead tested by the dual one-form on a loop \(C\simeq S^1\) linking the
        string,
        \begin{align}
            Q_B[C]
            =
            \frac{1}{g_B^2}\oint_C {*H},
            \label{eq:canon_two_form_linking_charge}
        \end{align}
        whereas an \(H\)-flux, when the topology supplies a suitable cycle, is
        integrated over a three-cycle.

        \textbf{[CANON NON-IDENTIFICATION].}
        Equations~\eqref{eq:canon_two_form_degree_guard}--\eqref{eq:canon_two_form_linking_charge}
        fix only the covariant bookkeeping.  They do not identify \(q_B\) with the
        canonical circulation \(\Gamma_0\), do not identify \(B_{\mu\nu}\) with the
        electromagnetic field, and do not establish that the SST core has already
        been reduced to a worldsheet EFT.  The corresponding action, scale-separation
        assumptions, source conservation, reconnection exceptions, and matching gates
        are confined to Sec.~\ref{sec:rt_covariant_two_form_worldsheet} of the
        companion Research Track \cite{KalbRamond1974,HornNicolisPenco2015}.
...
\section{Swirl--Electromagnetic Bridge}
    ...
    \subsection{Minimal transverse field sector}
        ...
        \textbf{[CANON / DERIVATION GUARD, v0.8.35].}
        Equation~\eqref{eq:div_free_Aeff} selects the transverse radiation sector;
        it is not, by itself, a derivation that the unconstrained SST microdynamics
        has no physical longitudinal radiative branch.  The corresponding
        no-projector theorem target, including a longitudinal null/gap test, is
        assigned to the companion research track in
        Sec.~\ref{sec:rt_maxwell_dynamical_field_closure}.  A numerical calculation
        that enforces Eq.~\eqref{eq:div_free_Aeff} by projection cannot be counted as
        evidence for that theorem.
        ...
        \begin{align}
            ...
            =
            \frac{\rhoEff}{2}\,\lVert \partial_t \mathbf{A}_{\mathrm{eff}}\rVert^2
            -
        ...
        \begin{align}
            ...
            \qquad
            c_\gamma^2 = \frac{K_\gamma}{\rhoEff}.
        ...
        \end{align}
        Within the quadratic bridge model this fixes only the ratio
        \begin{align}
            \frac{K_\gamma}{\rhoEff}=c^2.
            \label{eq:r_phase_torsion_stiffness_target}
        \end{align}
        A legacy-reference numerical benchmark may be quoted separately as
        \begin{align}
            K_\gamma^{\rm ref}
            :=
            \rhoRef c^2
            ...
            \qquad
            \text{\textbf{[LEGACY-REFERENCE NORMALIZED]}}.
            \label{eq:r_phase_torsion_stiffness_reference}
        \end{align}
        Here \(\rhoEff\) is the quasi-static inertial coefficient of the
        displacement field \(\mathbf A_{\mathrm{eff}}\), not the unresolved
        material density \(\rhoSub\).

        \paragraph{Normalization no-go for the source-free radiation sector.}
        \textbf{[DERIVED NEGATIVE].}
        The source-free action is invariant under
        \begin{align}
            \mathbf A_{\mathrm{eff}}
            &\longrightarrow
            s\,\mathbf A_{\mathrm{eff}},
            &
            \rhoEff
            &\longrightarrow
            s^{-2}\rhoEff,
            ...
            s^{-2}K_\gamma,
            \qquad s>0.
            \label{eq:rad_normalization_invariance_v0832}
        \end{align}
        Consequently its dispersion, polarization content, propagation speed,
        and source-free energy-flux ratios depend on the coefficients only
        through \(K_\gamma/\rhoEff=c^2\).  The source-free transverse sector
        cannot determine the absolute value of \(\rhoEff\).  Such a
        determination requires an independently normalized source coupling,
        self-interaction, impedance measurement, or quantum fluctuation
        observable.
    ...
    \subsection{Maxwell-inspired blind mechanical-closure gate}
        \label{subsec:maxwell_blind_mechanical_closure_gate}

        \textbf{[ORTHODOX HISTORICAL INPUT].}
        Maxwell's \emph{On Physical Lines of Force} treats the magnetic-force
        analogue as a medium with unequal principal stresses and uses geometrically
        similar molecular vortices to obtain a pressure-difference scale proportional
        to density times velocity squared, independent of the common linear scale
        \cite{Maxwell1861PhysicalLines}.  The historical molecular-vortex mechanism is
        not adopted as SST microphysics.

        \textbf{[ORTHODOX COARSE-GRAINING / CANON GUARD].}
        The primitive Cauchy stress of the incompressible inviscid Euler substrate
        remains isotropic, $-p\,\delta_{ij}$.  The anisotropic quantity tested here is
        instead the coarse-grained kinetic momentum-flux stress
        \begin{align}
            R_{ij}^{(\ell)}
            :=
            \rhoF\left(
            ...
            \right),
            \label{eq:canon_maxwell_blind_coarse_stress}
        \end{align}
        for a preregistered filter scale $\ell$.  This definition must not be
        reinterpreted as a primitive viscous or elastic shear stress.

        Where $\bar{\boldsymbol\omega}\neq0$, define
        \begin{align}
            \mathbf n_\omega
            &:={\bar{\boldsymbol\omega}\over
            ...
            \label{eq:canon_maxwell_blind_axis_pressures}
        \end{align}
        and $\Delta p_\omega:=p_\perp-p_\parallel$.

        \textbf{[RESEARCH / TARGET-BLIND FALSIFIER].}
        A Maxwell-inspired SST swirl-stress closure is retained only if a frozen
        finite-core campaign independently satisfies: (i) approximate axial symmetry
        of $R_{ij}^{(\ell)}$ about $\mathbf n_\omega$; (ii) alignment of
        $\mathbf n_\omega$ with the least-stress principal direction; (iii) positive
        $\Delta p_\omega$ on the preregistered active-vorticity support; and
        (iv) the similarity exponents
        \begin{align}
            \Delta p_\omega
            \propto
            \rhoF^{\,b_\rho}v_{\rm ref}^{\,b_v}L^{b_L},
            \qquad
            b_\rho\simeq1,
            \quad
            b_v\simeq2,
            \quad
            b_L\simeq0.
            \label{eq:canon_maxwell_blind_similarity_gate}
        \end{align}
        The dimensionless coefficient
        $C_{\rm blind}:=\operatorname{median}(\Delta p_\omega)/(\rhoF v_{\rm ref}^2)$
        is recorded but is not compared with Maxwell's historical coefficient or an
        SST benchmark until the blind report has been frozen and hashed.

        \textbf{[BRIDGE / THEOREM TARGET].}
        Maxwell's reduced-momentum analogy motivates an independent three-channel
        test in which a solver-derived reduced momentum $\mathbf p_{\rm red}$, the
        material/coherent velocity $\mathbf u$, and the independently evolved
        transverse/effective potential $\mathbf A$ must satisfy held-out scalar
        factorization,
        \begin{align}
            \mathbf p_{\rm red}
            &\simeq\beta_u\mathbf u,
            &
            \mathbf A
            &\simeq\beta_p\mathbf p_{\rm red},
            &
            \beta_A
            &\simeq\beta_p\beta_u,
            \label{eq:canon_maxwell_blind_reduced_momentum_factorization}
        \end{align}
        without exposing the SI normalization target during fitting.  Only after
        freezing the blind output may the direct coefficient be compared with the
        existing electron bridge $\beta_A\stackrel{?}{=}m_e/e$ of
        Eq.~\eqref{eq:canonical_em_prefactor_guard}.

        \textbf{[RESEARCH GUARD].}
        Passing these gates would validate only the tested coarse-grained closure and
        factorization.  It would not establish Maxwell's historical molecular-vortex
        mechanism, would not identify the material Euler velocity with
        $\mathbf A_{\rm eff}$, and would not by itself derive electromagnetism from
        Euler dynamics.  The detailed preregistration and optional structural-current
        test are recorded in the companion research track.
    ...
    \label{sec:lorentz_type_force_density_as_swirl_stress}
        ...
        force-density structure.  Its absolute magnitude remains conditional on the
        unfixed response coefficient \(\rhoEff\).  Numerical values obtained by
        substituting \(\rhoRef\) are \textbf{[LEGACY-REFERENCE NORMALIZED]}, while
        \(\rc\) and \(\vchar\) retain their electron-anchored status.
        ...
        \]
        This is a legacy-reference stress benchmark for Lorentz-type
        force-density analogies.  It is not an absolute prediction until
        \(
hoEff\) is independently fixed.
...
% ---------- TIME ----------
    ...
    \subsection{Canonical Electromagnetic--Gravity Closure}
        ...
        \textbf{[CANON / ENERGY-CLOSURE GUARD, v0.8.35].}
        The pressure anchor does not identify a local pressure decrement with a
        local field-energy density.  In particular,
        $\delta p_{\mathrm{swirl}}$ and $\delta\rhoE$ have the same SI dimensions but
        are not thereby the same constitutive quantity.  A mechanical gravity
        closure must instead compare the same declared SST Hamiltonian between
        separated and interacting source configurations and verify that its
        interaction-energy derivative agrees with an independently reconstructed
        force.  The target-blind protocol is recorded in
        Sec.~\ref{sec:rt_maxwell_dynamical_field_closure}.
        ...
        \end{align}
        Hence the legacy-reference pressure/stress benchmark is macroscopically large, while single-pass optical-index shifts are small unless a resonant optical path enhances them. This hierarchy is now part of the canonical interpretation of the EM--gravity bridge.
    ...
    \subsection{Triadic Gravity-Response Corollary}
        ...
        With the legacy reference \(\rhoRef=7.0\times10^{-7}\,\mathrm{kg\,m^{-3}}\),
        \begin{align}
            \frac{1}{\rhoRef c^2}
            &=
            ...
            6.6565\times10^{-6}
        \end{align}
        at canonical swirl speed.  These are legacy-reference-normalized values, not absolute predictions of the unfixed response.
...
\section{Unified Interpretation}
    ...
    \subsection{Gauge and weak-mixing bridge}
        ...
        Equation~\eqref{eq:effective_gauge_algebra} is a target infrared algebra,
        not a result obtained by counting director fields.  In particular,
        \begin{align}
            \dim\!\left[SO(3)^3/SO(3)_{\rm diag}\right]
            &=6,
            \\
            \dim\!\left[\mathfrak{su}(3)\oplus\mathfrak{su}(2)\oplus\mathfrak u(1)\right]
            &=12,
            \label{eq:canon_gauge_dimension_guard}
        \end{align}
        so three \(SO(3)\) directors modulo a common rotation cannot by themselves
        constitute a derivation of the Standard-Model gauge algebra.  Likewise, a
        connection written entirely as \(\mathcal W_\mu=(\partial_\mu O)O^{-1}\)
        is locally pure gauge and has vanishing curvature away from defects by the
        Maurer--Cartan identity.  A nontrivial emergent gauge sector therefore
        requires independently resolved physical modes or singular/holonomy data,
        closure and Jacobi tests, representation and charge separation, anomaly
        cancellation, coupling extraction, and phenomenological validation.  The
        mandatory promotion ladder is stated in
        Sec.~\ref{sec:rt_gauge_emergence_certification_ladder}.
    ...
    \subsection{Retained from the early canon layers}
        ...
        \begin{itemize}
            \item a continuum substrate with unfixed material density $\rhoSub$, represented in the current long-wavelength sector by the unfixed effective response $\rhoF\equiv\rhoEff$,
    ...
    \subsection{Notation and density cleanup}
        Earlier canon versions sometimes conflated material density, effective
        response, rotational microinertia, mass-equivalent density, and saturated
        envelope-equivalent density. The present version adopts the cleaned hierarchy
        \begin{align}
            \rhoSub
            &\quad \text{unfixed material substrate density},
            \nonumber\\
    ...
    \subsection{KAM-caged knot-state stability guard}
        ...
        \textbf{[CANON / COMPACT-STATE EXCLUSION].}
        The compact ideal-knot reference of
        Subsection~\ref{subsec:ideal_knot_regime_guard} is not automatically an
        admissible LIA background.  If either
        \(\epsilon_{\kappa}\) or \(\epsilon_{\rm sep}\) in
        Eq.~\eqref{eq:ideal_knot_regime_diagnostics} is not asymptotically
        small, Eq.~\eqref{eq:canon_kam_lia_bs_split} is only a formal comparison,
        not a perturbation expansion.  Promotion to an LIA-based KAM programme
        requires a separately resolved stretched state, both geometric
        slenderness gates, and a measured small
        \(\varepsilon_{\rm NL}\).
    ...
    \subsection{Ribbon-Invariant Chirality Refinement}
        ...
        For a single closed thin vortex tube with coherent circulation, the
        preferred fluid-mechanical specialization is
        Eq.~\eqref{eq:moffatt_ricca_single_tube_helicity}.  The geometric
        identity alone does not supply energy or stability; the factors of
        $\Gamma^2$ and the ideal-flow assumptions are essential.
    ...
    \subsection{Gauge-Sector Roadmap}
        ...
        \paragraph{Research-track reciprocal-stress audit guard.}
        The finite-thickness contact matrix must also be audited independently of
        any target channel count by the Maxwell-style equilibrium diagnostics of
        Sec.~\ref{sec:rt_maxwell_reciprocal_stress}.  The orthodox historical
        input is the force-polygon/polyhedron reciprocity of
        Maxwell~\cite{Maxwell1864Reciprocal}; the SST use is only a
        \textbf{[RESEARCH TRACK]} dual representation of an already declared
        strut/kink force balance.  In particular, the audit shall report the
        KKT residual, local force-closure residuals, rank and nullities, the
        smallest positive singular scale, and the existence or absence of a
        nonnegative self-stress cone.  Failure to construct a strict reciprocal
        polyhedral complex is \emph{not} by itself a mechanical falsifier,
        because Maxwell explicitly distinguished mechanical solvability from
        perfect geometric reciprocity and allowed redundant force diagrams.
        A physical force--area interpretation is permitted only when the input
        forces have a declared SI force normalization; dimensionless
        length-minimization multipliers are not silently reinterpreted as
        newtons.
...
\section*{Appendices}
    ...
    Early VAM density paper & \texttt{VAM7-DENSITY-2025} & Historical origin of the decimal $7\times10^{-7}$; retained only as an origin record because dimensional audit identifies a rotational-microinertia coefficient in $\mathrm{kg\,m^{-1}}$, not a volumetric density. \\
    ...
    \section{Conversation-Derived Insights}
        ...
        \subsubsection{v0.8.36}
            \textbf{v0.8.36} stacks the configuration-resolved spectroscopic-response guard, Maxwell--SST kinetic closure with a true excitation-gap ledger, Maxwell-blind mechanical falsifiers, reciprocal-stress audit, dynamical field-closure gates, and the material swirl--tonic vorticity-potential representation with Stokes/holonomy falsifier and material--link sector separation on top of v0.8.35.

        \subsubsection{v0.8.35}
            \textbf{v0.8.35} derives the factor \(8\pi/3\) from the isotropic
            incompressible transverse projector, fixes the radius--diameter
            ropelength conversion in the finite-cell \(\alpha\) candidate,
            records the bare-projector and constant-tube-volume no-go results,
            adds an alpha-blind finite-core Euler--Biot--Savart response
            completion with \(c_L=0\), constrains parity-even material-twist
            energy by the helicity identity \(SL=Wr+Tw\), separates a possible
            parity-odd helicity channel from the Maxwell stiffness, and links
            the candidate response to the existing gauge-emergence
            certification ladder on top of v0.8.34.

        \subsubsection{v0.8.34}
        ...
        \subsubsection{v0.8.32}
            \textbf{v0.8.32} removes the inherited
            \(7.0\times10^{-7}\,\mathrm{kg\,m^{-3}}\) value from the primitive
            calibration set, registers it as
            \(\rhoRef\) \textbf{[LEGACY REFERENCE NORMALIZATION / PROVENANCE
            INVALIDATED]}, leaves the physical
            \(\rhoF\equiv\rhoEff\) numerically unfixed, records all four VAM-7
            provenance defects, adds the A/B/C/Q/X response-scaling audit and
            source-free radiation normalization no-go, and converts numerical
            \(\rho_f\)-dependent amplitudes into explicitly
            legacy-reference-normalized benchmarks on top of v0.8.31.

        \subsubsection{v0.8.31}
            \textbf{v0.8.31} audits the compact-rotor reinterpretation of the
            line-inertia sector: it labels the Compton-gap rotor normalization
            as a matching ansatz, records $c_\omega=\vchar$ as a definitional
            identity rather than an independent validation, separates linear
...
\paragraph{Multi-component Gehring-thickness guard.}
        ...
        \subsection{Finite-cell $\alpha$ coincidence and obstruction summary}
            ...
            In the original finite-cell bookkeeping the same prefactor was decomposed
            into a soft-sector mode count $N_p=4$ and a spherical cell factor
            $4\pi/3$, with the response normalization left partly open.  Version~0.8.35
            adds an independent exact kinematic origin:
            \begin{align}
                ...
                \frac{8\pi}{3},
                \qquad |\mathbf t|=1.
                \label{eq:canon_alpha_transverse_projector_identity}
            \end{align}
            The denominator \(3\) is the isotropic second-moment denominator in
            three spatial dimensions.  This establishes the numerical prefactor
            without appealing to the trefoil crossing number or the soft-mode
            count.  It does not establish that the resulting response is the
            electromagnetic inverse coupling.


            \paragraph{Transverse response, convention, and finite-core guard.}
            Define the diameter-normalized ropelength
            \(\mathcal L_D:=L/D\).  Equation
            \eqref{eq:canon_alpha_transverse_projector_identity} gives
            \begin{align}
                \mathscr R_0[\gamma]
                &:=
                ...
                \label{eq:canon_alpha_transverse_leading_response}
            \end{align}
            If Ridgerunner reports the radius-normalized convention
            \(\operatorname{Rop}_{\rm rad}=L/a\) with \(D=2a\), then
            \begin{align}
                ...
                \mathscr R_0
                =
                \frac{4\pi}{3}\operatorname{Rop}_{\rm rad}.
                \label{eq:canon_alpha_radius_diameter_conversion}
            \end{align}
            Hence values near \(32.74\) from Ridgerunner, KnotPlot-derived
            polygons, or Fresnel/Fourier representations are consistent with
            diameter-normalized values near \(16.37\); multiplying
            \(32.74\) by \(8\pi/3\) would double-count the convention factor.

            Because the angular integral is constant for every unit tangent,
            the bare local projector gives
            \(\Delta_{\rm bare}=0\).  A nonzero correction must be a resolved
            ...
            \begin{align}
                ...
                \qquad
                \boxed{c_L=0}.
                \label{eq:canon_alpha_finite_core_completion}
            \end{align}
            The condition \(c_L=0\) prevents a free length counterterm from
            refitting the leading coefficient after the target is known.

            For one framed thin tube,
            \begin{align}
                ...
                =
                Wr+Tw.
                \label{eq:canon_alpha_helicity_sector_constraint}
            \end{align}
            This identity constrains the allowed material-twist sector.  It
            does not justify a linear correction
            \(c_H(Wr+Tw)\) to the parity-even Maxwell stiffness: helicity is a
            pseudoscalar.  The parity-even twist contribution is quadratic and
            obeys the lower bound derived in
            Eq.~\eqref{eq:rt_alpha_twist_energy_bound_v0835}; a linear
            helicity response, if independently derived, belongs to the
            separate parity-odd channel already guarded by the research track.

            The electromagnetic identification remains an open matching step.
            In the dimensionless gauge-field convention
            \begin{align}
                ...
                \int d^4\xi\,f_{\mu\nu}f^{\mu\nu}
                +
                \int d^4\xi\,j^\mu a_\mu+\cdots ,
            \end{align}
            a successful application of the gauge-emergence ladder would give
            \begin{align}
                \mathscr R_{\rm SST}
                \stackrel{\rm open\ EM\ match}{=}
                \frac{4\pi}{g_{\rm eff}^{\,2}}
                =
                \alpha_{\rm eff}^{-1}.
                \label{eq:canon_alpha_em_stiffness_matching}
            \end{align}
            The match requires a protected \(U(1)\) sector, exactly two
            physical transverse modes, conserved universal source
            normalization, and the same coupling in static and radiative
            response.  Neither \(e\), \(\alpha\), the final target correction,
            nor a swirl speed calibrated through \(\alpha\) may enter the
            finite-core calibration.
            ...
            This block does not assert a first-principles derivation of the
            fine-structure constant.  The defensible content is now:
            (i) the exact isotropic transverse-projector identity
            \(8\pi/3\); (ii) an alpha-free sub-per-mille trefoil coincidence;
            (iii) a finite-core response programme whose coefficients remain
            independently unfixed; and (iv) a localized obstruction to
            ppm closure.  Full finite-cell scripts and audit details remain in
            \texttt{finite\_cell\_obstruction.tex} and
            \texttt{Derive\_Constants/}; the Euler--Biot--Savart,
            helicity, twist-energy, calibration, and electromagnetic matching
            programme is recorded in
            Subsection~\ref{subsec:rt_alpha_transverse_response_v0835}.
...
% ===========================
    ...
    \subsection{Purpose and status}
        ...
        \textbf{[CRITICAL NOTE]}: $\rho_\psi$ is a coarse-grained mode-support density of the emergent carrier field, \emph{not} a compression of the incompressible material substrate $\rhoSub$ (Axiom~1, $\nabla\cdot\mathbf v=0$). It is also distinct from the unfixed quasi-static response $\rhoF\equiv\rhoEff$. These are different objects, so the Madelung amplitude does not violate substrate incompressibility.
    ...
    \subsection{Purpose and status}
        ...
        \textbf{[CRITICAL NOTE]} The legacy value
        \(\rhoRef=7.0\times10^{-7}\,\mathrm{kg\,m^{-3}}\) is not a measured
        cosmological density and does not fix the physical response
        \(\rhoF\equiv\rhoEff\). Applying it on galactic scales requires a separate coupling and screening law. Without that law, the galaxy-scale pressure profile is a fit ansatz only and must not be interpreted as a literal dense fluid through which stars experience ordinary hydrodynamic drag.
...
% ---------------------------
        ...
        \input{SST_CANON-v0.8.36-research-track}
...
\newblock DOI: \href{https://doi.org/10.1103/PhysRevD.9.2273}{10.1103/PhysRevD.9.2273}.
...
\newblock DOI: \href{https://doi.org/10.1007/JHEP10(2015)153}{10.1007/JHEP10(2015)153},
\newblock arXiv: \href{https://arxiv.org/abs/1507.05635}{1507.05635}.
            ...
            \textbf{439} (1992), 411--429,
            \newblock DOI: \href{https://doi.org/10.1098/rspa.1992.0159}{10.1098/rspa.1992.0159}.

            \bibitem{Fukumoto2007}
            Y.~Fukumoto,
            \newblock Analogy between a vortex-jet filament and the Kirchhoff elastic rod,
            \newblock \emph{Fluid Dynamics Research} \textbf{39} (2007), 511--520,
            \newblock DOI: \href{https://doi.org/10.1016/j.fluiddyn.2006.12.004}{10.1016/j.fluiddyn.2006.12.004}.

            \bibitem{JohannsEtAl2021}
            P.~Johanns, P.~Grandgeorge, C.~Baek, T.~G.~Sano,
            J.~H.~Maddocks, and P.~M.~Reis,
            \newblock The shapes of physical trefoil knots,
            \newblock \emph{Extreme Mechanics Letters} \textbf{43} (2021), 101172,
            \newblock DOI: \href{https://doi.org/10.1016/j.eml.2021.101172}{10.1016/j.eml.2021.101172}.
            ...
            \bibitem{Dirac1938LargeNumbers}
            P.~A.~M. Dirac,
            \newblock A new basis for cosmology,
            \newblock \emph{Proceedings of the Royal Society of London A} \textbf{165} (1938), 199--208,
            \newblock DOI: \href{https://doi.org/10.1098/rspa.1938.0053}{10.1098/rspa.1938.0053}.
...
\bibitem{CantarellaDeTurckGluck2002Hodge}
            ...
            J.~C. Maxwell,
            \newblock On reciprocal figures and diagrams of forces,
            \newblock \emph{The London, Edinburgh, and Dublin Philosophical Magazine and Journal of Science},
            Series~4, \textbf{27}(182) (1864), 250--261,
            \newblock DOI: \href{https://doi.org/10.1080/14786446408643663}{10.1080/14786446408643663}.
