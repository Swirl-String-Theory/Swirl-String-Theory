...
%! DOI = 10.5281/zenodo.17607006

\newcommand{\canonversion}{\textbf{v0.5.10.2}} % Semantic versioning: vMAJOR.MINOR.PATCH
\newcommand{\papertitle}{Swirl String Theory (SST) Canon \canonversion}
\newcommand{\paperdoi}{10.5281/zenodo.17607006}
...
\newcommand{\compacttoc}{%
  ...
  \setlength{\cftsecnumwidth}{1.4em}%
...
\begin{document}
    ...
    \subsection*{Core Axioms (SST)}
        \begin{tabular}{r@{\quad}p{0.75\linewidth}}
            ...
            & $\displaystyle \Gamma = \oint \mathbf{v}_{\mkern-2mu\scriptstyle\boldsymbol{\circlearrowright}} \cdot d\boldsymbol{\ell} = n\,\kappa,\qquad n\in\mathbb{Z},\qquad \kappa = \frac{h}{m_{\text{eff}}}.$ \\
...
% ------------------------------------------------------------
    \subsection{VAM-v0.2.x: Unified Framework and Topological Quantization}
        ...
        \begin{itemize}
        \item \textbf{Electromagnetism as swirl dynamics:} Variations in swirl density $\partial_t\rho_{\!f}$ generate electromotive effects via a modified Faraday law (\emph{Swirl--EM Bridge}).
...
% Check: [units ok; limit → Newtonian]
    ...
    \subsection{Definitions (Geometry and Symbols) [Definition]}
        ...
        \]
        House constants: $\Gamma_0$, $\rc$, $\rhoF$ from \S~VIII (primitive triplet); circulation quantum $\kappa_{\rm SST} \equiv \Gamma_0 = 2\pi \rc\,\vnorm$ (cf. \S~VII quantization) :contentReference[oaicite:4]{index=4}:contentReference[oaicite:5]{index=5}.
    ...
    \subsection{Route A: Helicity/Quantization Closure [Theorem]}
        ...
        \begin{equation}
        \boxed{\ \Gamma = n\,\kappa_{\rm SST},\qquad \kappa_{\rm SST} \equiv \Gamma_0 = 2\pi \rc\,\vnorm\ }\quad\Rightarrow\quad
    ...
    \]
    \[
        \mathcal{H}\approx 2\chi\,\Gamma_{\rm req}^{2},\qquad n=\Gamma_{\rm req}/\kappa_{\rm SST},\quad \kappa_{\rm SST} \equiv \Gamma_0 = 2\pi \rc\,\vnorm.
    \]
    ...
    \begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{proofs/Mod-Gravity 3-Phase Saw-Shaped-Coil - Stack 2 identical with 30 degrees rotation}
...
% [Sidebar: The road ahead -- perhaps a flowchart of theory components and next steps]
	...
	The Triad construction (Hydrodynamic Triad, Eq.~(33)) defines $\Lambda = 4\pi\rho_m \lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\rVert r_c^{3}$ from Euler fluid mechanics. This yields the correct hydrogen energy scale: using values in Table~\ref{tab:constants}, $\Lambda \approx 2.3\times 10^{-28}$ J·m, and $e^2/(4\pi\epsilon_0)\approx 2.3\times10^{-28}$ J·m, a match.
	...
	\begin{description}[leftmargin=1.3cm,labelsep=0.4cm, itemsep=1ex]
		...
		\item[\textbf{Circulation quantum $\Gamma_0$:}] Primitive quantum of circulation, $\Gamma_0 \approx 6.4\times 10^{3}~\mathrm{m^2/s}$. Appears in $\Gamma = n\Gamma_0$. The notation $\kappa_{\text{SST}} \equiv \Gamma_0$ is also used. In the Rosetta mapping to conventional superfluid notation, $\Gamma_0$ matches $h/m_{\text{eff}}$, but within SST $\Gamma_0$ is treated as primitive.
...
% === Appendix N3: Rosetta→Code Consistency Rule (Invariant-Mass Sector) ===
...
%======================================================
    ...
    \begin{itemize}
        ...
        \item a swirl channel (density/pressure excitations, propagation speed
        $\lVert\mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\rVert$).
    \end{itemize}
