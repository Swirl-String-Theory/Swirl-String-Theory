...
%=========================================================
    \section{Observations on Light Quanta in a Swirl Medium}
%=========================================================
...
%=====================================================================
    \section{The Principle of Pressure Equilibrium in Swirl-String Vortex Models}
%=====================================================================
...
%========================================================================================
% PART IV: APPENDIX MODULES AND APPLICATIONS (Research Track)
%========================================================================================
\part{Modules and Applications}
    ...
    \section{Swirl--Coulomb in SST (near-field pressure vs.\ far-field mediator)}\label{app:swirl_coulomb}
    \paragraph{Scope.}
    This appendix separates two distinct statements:
    (i) the \emph{near-field} pressure deficit around a steady swirl with $v_\theta\propto 1/r$;
    (ii) the \emph{far-field} $1/r$ static potential, which in SST is carried by a local mediator field
    (clock/foliation mode) obeying a Poisson equation on $\mathbb{R}^3$.
    The soft-core profile
    \begin{equation}
    V_{\mathrm{SST}}(r)=-\frac{\Lambda}{\sqrt{r^2+r_c^2}}
    \label{eq:VSST_softcore}
    \end{equation}
    is retained as a convenient regularized \emph{effective} potential with the correct long-range tail and finite core.

    \subsection*{B.1 Near-field: what Euler radial balance actually gives for $v_\theta\propto 1/r$}
    Consider circular flow about the $z$-axis. In the absence of external body forces, radial Euler balance reads
    \begin{equation}
    \frac{1}{\rho}\frac{dp}{dr}=-\frac{v_\theta^2(r)}{r}.
    \label{eq:euler_radial}
    \end{equation}
    For an ideal filament circulation,
    \begin{equation}
    v_\theta(r)=\frac{\Gamma}{2\pi r}
    \quad\Rightarrow\quad
    \frac{dp}{dr}=-\rho\,\frac{\Gamma^2}{4\pi^2}\frac{1}{r^3}.
    \end{equation}
    Integrating gives (up to an additive constant)
    \begin{equation}
    p(r)=p_\infty-\rho\,\frac{\Gamma^2}{8\pi^2}\frac{1}{r^2}.
    \label{eq:pressure_deficit}
    \end{equation}
    Hence the pressure deficit scales as $\Delta p\propto -1/r^2$.
    If one defines a ``potential per unit mass'' by $\Phi:=p/\rho$, then $\Phi\propto -1/r^2$ and the associated force scales as $1/r^3$.
    \emph{Conclusion: a $v_\theta\propto 1/r$ swirl does not, by itself, generate a $1/r$ potential.}

    \subsection*{B.2 Far-field: $1/r$ is the Green function of a local mediator on $\mathbb{R}^3$}
    The SST long-range static interaction is modeled by a scalar mediator $\phi(\mathbf{x})$
    (interpretable as a weak clock/foliation perturbation) with static EFT
    \begin{equation}
    S_{\mathrm{stat}}[\phi]=\int_{\mathbb{R}^3} d^3x\left[\frac{\kappa}{2}(\nabla\phi)^2-\lambda\,\rho_m(\mathbf{x})\,\phi\right],
    \label{eq:stat_action}
    \end{equation}
    where $\kappa>0$ and $\lambda$ are constants and $\rho_m$ is the mass-equivalent source density.
    Varying \eqref{eq:stat_action} yields the Poisson equation
    \begin{equation}
    \kappa \nabla^2\phi(\mathbf{x})=-\lambda\,\rho_m(\mathbf{x}).
    \label{eq:poisson_phi}
    \end{equation}
    For a point source $\rho_m(\mathbf{x})=M\,\delta^{(3)}(\mathbf{x})$, the unique spherically symmetric solution decaying at infinity is
    \begin{equation}
    \phi(r)=\frac{\lambda M}{4\pi\kappa}\frac{1}{r}.
    \end{equation}
    Thus any \emph{local} massless mediator in three spatial dimensions generically produces a $1/r$ potential and $1/r^2$ field at large $r$.

    \subsection*{B.3 Matching the Coulomb coefficient and correcting $\Lambda$}
    We parameterize the far-field static potential energy as
    \begin{equation}
    V(r)=-\frac{\Lambda}{r}\quad (r\gg r_c),
    \end{equation}
    and retain the soft-core regularization \eqref{eq:VSST_softcore}. Dimensional consistency requires
    $[\Lambda]=\mathrm{J\cdot m}$.
    The canonically consistent SST identification is
    \begin{equation}
    \boxed{\;\Lambda = 4\pi\,\rho_{\text{core}}\,\lVert \mathbf{v}_{\!\boldsymbol{\circlearrowleft}}\rVert^{2}\,r_c^{4}\;}
    \qquad [\Lambda]=\mathrm{J\cdot m}.
    \label{eq:Lambda_correct}
    \end{equation}
    This replaces the earlier (dimensionally incorrect) expression and is the form used for all Coulomb-matching instances in the Canon.

    \paragraph{Child-level analogy.}
    A rotating ``water vortex'' creates nearby a deep pit in pressure ($\sim 1/r^2$),
    but a \emph{distant} law with $1/r$ comes from a messenger field that spreads over spheres:
    the larger the sphere ($\propto r^2$), the thinner the influence per surface area ($\propto 1/r^2$).
...
%=============================================================================
    ...
    \section{Knot Taxonomy}
    \subsection{Glossary of Symmetry Table Symbols}
...
\section{Short research status Canonical or Emperical}
    \subsection{Swirl Hamiltonian Density}
% [STATUS: Canonical]
        \label{canon58:appA}
...
%================================================
\subsection{Dimensional Analyses \& Recovery Limits}
% [STATUS: Canonical]
\label{canon58:appB}
\paragraph{Purpose.}
...
%================================================
\subsection{Hydrogen Soft-Core Numerics}
% [STATUS: Canonical]
\label{canon58:appD}
...
%================================================
\subsection{Swirl Pressure Law—Galaxy-Scale Integrals}
% [STATUS: Canonical]
\label{canon58:appF}
...
%================================================
\subsection{Calibration Protocol Notes}
% [STATUS: Empirical]
\label{canon58:appG}
...
%================================================
\subsection{Experimental Status \& Bounds}
% [STATUS: Empirical]
\label{canon58:appH}
...
%================================================
\subsection{Notation, Ontology, Glossary}
% [STATUS: Canonical]
\label{canon58:appI}
...
% =============================================================
\subsection{ 2\texttimes 2 near-degenerate solution (sketch)}
...
% Numbers (with user's constants): \mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}} = 1.09384563\times10^6\ \mathrm{m\,s^{-1}},\ r_c=1.40897017\times10^{-15}\ \mathrm{m}.
\[
...
% permalink: https://press.princeton.edu/books/hardcover/9780691084219/three-dimensional-geometry-and-topology-volume-1
    ...
    \bibitem{Chorin1994} A. J. Chorin, Vorticity and Turbulence, Applied Mathematical Sciences 103, Springer (1994), DOI: 10.1007/978-1-4612-2600-5.
