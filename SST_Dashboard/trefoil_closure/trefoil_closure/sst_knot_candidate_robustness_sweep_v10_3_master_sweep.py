...
from sst_closure_lab_build import MODULE_NAME as SST_CLOSURE_LAB_MODULE_NAME
from sst_closure_lab_build import CPP_FILENAME, build_module, needs_recompile as _closure_lab_needs_recompile

SST_CORE_CPP_PATH = os.path.join(SCRIPT_DIR, CPP_FILENAME)
SST_CORE_MODULE_NAME = SST_CLOSURE_LAB_MODULE_NAME
...
# Which native module supplied calculate_bs_cutoff_energy_scan: "sstcore" | "sst_closure_lab" | "".
...
def _build_local_sst_core() -> None:
    build_module(SCRIPT_DIR, verbose=True)


def resolve_bs_cutoff_scan_module(use_local: bool, auto_compile: bool) -> Tuple[Optional[object], bool]:
    """Load native BS cutoff scan: prefer ``SSTcore`` / ``sstcore`` (pip or CMake), else local ``sst_closure_lab``."""
    ...
    if not os.path.exists(SST_CORE_CPP_PATH):
        return None, False
    try:
        if auto_compile and _closure_lab_needs_recompile(SCRIPT_DIR):
            print(f"[META] Recompiling local {CPP_FILENAME} ...")
        ...
        if SST_CORE_SCAN_AVAILABLE:
            SST_CORE_SCAN_PROVIDER = SST_CLOSURE_LAB_MODULE_NAME
        return mod2, SST_CORE_SCAN_AVAILABLE
    except Exception as exc:
        print(f"[WARN] Local {SST_CLOSURE_LAB_MODULE_NAME} unavailable: {exc}")
        return None, False
