// Local trefoil-closure experiment kernels for Python module `sst_closure_lab`.
// Not the pip/CMake package `sstcore`. C++ reference copy lives in
// SSTcore src/trefoil_closure_kernels.cpp; keep formulas aligned.
...
// 7. Asymmetrische retardatie + DDE fase-sluiting voor bewegende trefoil
double equilibrate_moving_trefoil(
    ...
    if (N < 2) {
        throw std::runtime_error("equilibrate_moving_trefoil: need at least 2 points");
    }
    if (r.shape(1) != 3) {
        throw std::runtime_error("equilibrate_moving_trefoil: points must have shape (N, 3)");
    }
    if (u_ext.size() != 3) {
        throw std::runtime_error("equilibrate_moving_trefoil: u_ext must have length 3");
    }

    const double c_signal = config.contains("c_signal")
        ? config["c_signal"].cast<double>() : 2.99792458e8;
    const double kappa = config.contains("kappa")
        ? config["kappa"].cast<double>() : 1e20;
    const int max_iter = config.contains("max_iter")
        ? config["max_iter"].cast<int>() : 5000;
    const double tol = config.contains("tol")
        ? config["tol"].cast<double>() : 1e-6;
    const double lr = config.contains("lr")
        ? config["lr"].cast<double>() : 0.05;

    const double ux = u_ext[0];
    const double uy = u_ext[1];
    const double uz = u_ext[2];
    const double u_sq = ux * ux + uy * uy + uz * uz;
    const double c_sq = c_signal * c_signal;
    const double factor = c_sq - u_sq;

    if (u_sq >= c_sq) {
        throw std::runtime_error("equilibrate_moving_trefoil: u_ext exceeds c_signal");
    }
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            if (i == j) continue;
            const double dx = r(i, 0) - r(j, 0);
            const double dy = r(i, 1) - r(j, 1);
            const double dz = r(i, 2) - r(j, 2);
            const double r_sq = dx * dx + dy * dy + dz * dz;
            const double r_dot_u = dx * ux + dy * uy + dz * uz;
            tau[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)] =
                (r_dot_u + std::sqrt(r_dot_u * r_dot_u + factor * r_sq)) / factor;
        }
    }

    std::vector<double> theta(static_cast<std::size_t>(N), 0.0);
    for (int i = 0; i < N; ++i) {
        theta[static_cast<std::size_t>(i)] = 3.0 * (2.0 * PI * i) / N;
    }

    double omega = omega_0;

    for (int iter = 0; iter < max_iter; ++iter) {
        double sum_sin_total = 0.0;
        std::vector<double> dtheta(static_cast<std::size_t>(N), 0.0);
        ...
        #pragma omp parallel
        {
            ...
            for (int i = 0; i < N; ++i) {
                ...
                for (int j = 0; j < N; ++j) {
                    if (i == j) continue;
                    const double delta = theta[static_cast<std::size_t>(j)]
                        - theta[static_cast<std::size_t>(i)]
                        - omega * tau[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)];
                    phase_drift += std::sin(delta);
                }
                dtheta[static_cast<std::size_t>(i)] =
                    omega_0 - omega + (kappa / static_cast<double>(N)) * phase_drift;
                local_sum += phase_drift;
            }
            #pragma omp atomic
            sum_sin_total += local_sum;
        }
        #else
        for (int i = 0; i < N; ++i) {
            double phase_drift = 0.0;
            ...
            dtheta[static_cast<std::size_t>(i)] =
                omega_0 - omega + (kappa / static_cast<double>(N)) * phase_drift;
            sum_sin_total += phase_drift;
        }
        #endif

        const double omega_new =
            omega_0 + (kappa / (static_cast<double>(N) * static_cast<double>(N))) * sum_sin_total;
        const double diff = std::abs(omega_new - omega);
        omega = omega_new;

        for (int i = 0; i < N; ++i) {
            theta[static_cast<std::size_t>(i)] += lr * dtheta[static_cast<std::size_t>(i)];
        }
...
PYBIND11_MODULE(sst_closure_lab, m) {
    m.doc() = "Local trefoil-closure lab kernels (not pip SSTcore)";
    ...
    m.def("equilibrate_moving_trefoil", &equilibrate_moving_trefoil,
          "DDE equilibration with asymmetric signal retardation tau_ij");
}