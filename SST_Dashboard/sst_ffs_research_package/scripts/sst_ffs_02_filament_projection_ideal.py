...
Ideal-knot database input:
    python sst_ffs_02_filament_projection_ideal.py --curve ideal --ideal-file ideal.txt --ideal-id 3:1:1 --outdir results
...
def parse_ideal_ab_block(path: Path, ideal_id: str):
    """
    Parse a Brian Gilbert ideal-knot AB Fourier block.

    Database convention:
        X(t)=sum_i A_i cos(i t)+B_i sin(i t),  t in [0,2*pi).

    This convention is verified by the unknot entry:
        A_1=(1,0,0), B_1=(0,1,0), L=2*pi.
    """
    text = path.read_text(encoding="utf-8", errors="replace")
    pattern = re.compile(
        r'<AB\s+(?P<attrs>[^>]*)Id="' + re.escape(ideal_id) + r'"(?P<attrs2>[^>]*)>'
        r'(?P<body>.*?)</AB>',
        flags=re.S,
    )
    m = pattern.search(text)
    if not m:
        prefix = ideal_id.split(":")[0] + ":"
        suggestions = list_ideal_ids(path, prefix=prefix)
        suggestion_text = ", ".join([f"{sid} Conway={conway} L={L}" for sid, conway, L, D in suggestions[:12]])
        if suggestion_text:
            raise ValueError(
                f"Could not find <AB Id={ideal_id!r}> in {path}. "
                f"Available IDs with prefix {prefix!r}: {suggestion_text}"
            )
        raise ValueError(f"Could not find <AB Id={ideal_id!r}> in {path}")

    attr_text = (m.group("attrs") + " " + m.group("attrs2")).strip()
    attrs = dict(re.findall(r'([A-Za-z_][A-Za-z0-9_]*)="([^"]*)"', attr_text))
    attrs["Id"] = ideal_id

    coeff_re = re.compile(
        r'<Coeff\s+I="\s*(?P<I>[0-9]+)"\s+'
        r'A="(?P<A>[^"]+)"\s+'
        r'B="(?P<B>[^"]+)"\s*/>'
    )

    coeffs = []
    for cm in coeff_re.finditer(m.group("body")):
        I = int(cm.group("I"))
        A = _parse_vec3(cm.group("A"))
        B = _parse_vec3(cm.group("B"))
        coeffs.append((I, A, B))

    if not coeffs:
        raise ValueError(f"No <Coeff> entries found for ideal id {ideal_id!r}")
...
def list_ideal_ids(path: Path, prefix: str | None = None):
    """Return (Id, Conway, L, D) tuples from a Gilbert ideal-knot database."""
    text = path.read_text(encoding="utf-8", errors="replace")
    rows = []
    pat = re.compile(
    ...
    for m in pat.finditer(text):
        ...
        if prefix is None or rid.startswith(prefix):
            rows.append((rid, m.group("conway"), m.group("L"), m.group("D")))
    return rows


def generate_ideal_knot_from_ab(path: Path, ideal_id: str, n: int):
    """
    Generate X(t) from ideal-knot Fourier coefficients:
        X(t)=sum_i A_i cos(i t)+B_i sin(i t),  t in [0,2*pi).
    """
    coeffs, attrs = parse_ideal_ab_block(path, ideal_id)
    t = np.linspace(0.0, 2.0 * math.pi, n, endpoint=False)
    X = np.zeros((n, 3), dtype=float)
    for I, A, B in coeffs:
        X += np.cos(I * t)[:, None] * A[None, :]
        X += np.sin(I * t)[:, None] * B[None, :]
    return X, coeffs, attrs
...
def main() -> int:
    ...
    parser.add_argument("--curve", choices=["trefoil", "circle", "csv", "ideal"], default="trefoil")
    parser.add_argument("--input", type=Path, default=None, help="CSV path when --curve csv.")
    parser.add_argument("--ideal-file", type=Path, default=Path("ideal.txt"), help="Brian Gilbert ideal-knot database file when --curve ideal.")
    parser.add_argument("--ideal-id", type=str, default="3:1:1", help="Ideal-knot AB Id, e.g. 3:1:1 for the trefoil.")
    ...
    if args.curve == "trefoil":
        ...
        curve_name = "circle"
    elif args.curve == "ideal":
        X0, ideal_coeffs, ideal_attrs = generate_ideal_knot_from_ab(args.ideal_file, args.ideal_id, args.n)
        ideal_coeff_count = len(ideal_coeffs)
        L_attr = ideal_attrs.get("L", "?")
        D_attr = ideal_attrs.get("D", "?")
        curve_name = f"ideal:{args.ideal_id}:L={L_attr}:D={D_attr}:coeffs={ideal_coeff_count}"
    else:
    ...
    if args.curve == "ideal":
        meta_path = args.outdir / "sst_ffs_02_ideal_metadata.txt"
        lines = [
            f"ideal_file={args.ideal_file}\n",
            f"ideal_id={args.ideal_id}\n",
            f"curve_name={curve_name}\n",
            f"coeff_count={ideal_coeff_count}\n",
        ]
        for k in sorted(ideal_attrs):
            lines.append(f"attr_{k}={ideal_attrs[k]}\n")
