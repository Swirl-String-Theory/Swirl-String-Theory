...
def load_fourier_series_clean(filename):
    ...
    - If given a path starting with ./Knots_FourierSeries, resolve relative to this script's folder.
    - Provides clear errors if the file does not exist.
    """
    import glob
    # Normalize input path
    script_dir = os.path.dirname(os.path.abspath(__file__))
    candidate = filename
    # If it's not absolute, make it absolute relative to current working directory
    if not os.path.isabs(candidate):
        candidate = os.path.abspath(candidate)
    # If the path still doesn't exist, try resolving relative to the script dir
    if not os.path.exists(candidate):
        # Handle common case when caller passed "./Knots_FourierSeries/..."
        rel = filename
        if rel.startswith("./") or rel.startswith(".\\"):
            rel = rel[2:]
        candidate2 = os.path.join(script_dir, rel)
        candidate2 = os.path.abspath(candidate2)
        if os.path.exists(candidate2):
            candidate = candidate2
        else:
            # As a fallback, search within the known local folder
            search_root = os.path.join(script_dir, "Knots_FourierSeries")
            matches = glob.glob(os.path.join(search_root, "**", os.path.basename(filename)), recursive=True)
            if matches:
                candidate = matches[0]

    if not os.path.exists(candidate):
        raise ValueError(f"File {filename} not found. Checked: {candidate}")
...
def analyze_knots(path='.'):
    ...
    import glob
    # Resolve search root relative to this script's folder to avoid cwd issues
    script_dir = os.path.dirname(os.path.abspath(__file__))
    search_root = os.path.join(script_dir, "Knots_FourierSeries")
    fseries_files = glob.glob(os.path.join(search_root, "**", "*.fseries"), recursive=True)
    ...
    for fname in fseries_files:
        ...
        try:
            ...
            label, color = classify_particle(a_mu, os.path.basename(fname))

            print(colored(f"Processing {os.path.relpath(full_path, script_dir)}...", "white", attrs=['bold']))
