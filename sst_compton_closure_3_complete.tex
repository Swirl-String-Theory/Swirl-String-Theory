...
% ── SST house macros ──────────────────────────────────────────────────────────
\newcommand{\rc}{r_c}
\newcommand{\vswirl}{\mathbf{v}_{\scriptscriptstyle\boldsymbol{\circlearrowleft}}}
...
The swirl-string vortex tube is a closed loop.  The appropriate
pressure model is therefore the \emph{cylindrical Laplace pressure}:
the total force $F_{\max}$ acts on the full circumference
$2\pi\rc$ of the tube cross-section, yielding a line tension
\begin{equation}
  \mathcal{T}
  \;=\; \frac{F_{\max}}{2\pi\rc}
  \;=\; \frac{\alpha\,\hbar\,c}{8\pi\,\rc^3}.
  \label{eq:line_tension}
\end{equation}
The inward Laplace pressure on a cylindrical tube of radius $\rc$ is
$P=\mathcal{T}/\rc$~\cite{Donnelly1991}:
\begin{equation}
  P_{\mathrm{Laplace}}
  = \frac{\mathcal{T}}{\rc}
  = \frac{F_{\max}}{2\pi\rc^2}
  = \frac{\alpha\,\hbar\,c}{8\pi\,\rc^4}.
  \label{eq:Plaplace}
\end{equation}
Setting this equal to the kinetic pressure of the core fluid,
$P_{\mathrm{Laplace}}=\tfrac{1}{2}\rhocore\vswirl^2$, and solving:
\begin{equation}
  \rhocore
  = \frac{\alpha\,\hbar\,c}{4\pi\,\rc^4\,\vswirl^2}.
  \label{eq:rhocore_alpha}
\end{equation}
Now substitute $\alpha=2\vswirl/c$
and $\rc=\vswirl/\omC$:
\begin{align}
  \rhocore
  &= \frac{(2\vswirl/c)\,\hbar\,c}
          {4\pi\,(\vswirl/\omC)^4\,\vswirl^2}
  = \frac{2\hbar\,\vswirl\,\omC^4}
          {4\pi\,\vswirl^6}
  = \frac{\hbar\,\omC^4}{2\pi\,\vswirl^5}.
  \notag
\end{align}
...
\begin{equation}
  \boxed{
    \rhocore
    = \frac{m_e c^2}{2\pi\,\vswirl^2\,\rc^3}.
  }
...
side; the derivation is non-circular.  The factor $2\pi$ in the
denominator arises from the circumference of the circulating vortex
(Laplace pressure for a cylindrical tube), not from the cross-sectional
area $\pi\rc^2$.
...
\begin{equation}
  ...
  = \frac{9.1094\times10^{-31}\times(2.9979\times10^8)^2}
         {2\pi\times(1.0938\times10^6)^2
          \times(1.4090\times10^{-15})^3}
  \approx 3.8934\times10^{18}\;\mathrm{kg\,m^{-3}},
  \label{eq:rhocore_numerical}
\end{equation}
in agreement with the SST canonical value
$\rhocore^{\mathrm{canon}}=3.8934358\times10^{18}\;\mathrm{kg\,m^{-3}}$
to five significant figures (relative error $<10^{-6}$).
...
\begin{equation}
  ...
  \label{eq:Ekin_core}
\end{equation}
The core kinetic energy equals \emph{exactly two-thirds of the electron
rest energy}.  This is consistent with the virial theorem for a
three-dimensional harmonic system: $\langle T\rangle = E_{\mathrm{tot}}/2$
gives $E_{\mathrm{kin}}=(2/3)m_ec^2$ when the spherical volume
integral is performed over a uniformly rotating
core~\cite{LandauMechanics}.

\subsection{Condensation factor in closed form}

The condensation factor is now fully determined:
\begin{equation}
  \mathcal{C}
  \;\equiv\; \frac{\rhocore}{\rhof}
  \;=\; \frac{m_e c^2}{2\pi\,\rhof\,\vswirl^2\,\rc^3}.
  \label{eq:C_closed}
\end{equation}
Substituting $\rc=\vswirl/\omC$:
\begin{equation}
  \mathcal{C}
  = \frac{m_e c^2\,\omC^3}{2\pi\,\rhof\,\vswirl^5}
  = \frac{m_e^4 c^8}{2\pi\,\rhof\,\hbar^3\,\vswirl^5}.
...
Numerically, $\mathcal{C}\approx5.562\times10^{24}$, in exact
agreement with the canonical value.
...
\begin{tabular}{lSSS}
  ...
  $\rhocore$ / \si{kg.m^{-3}}
    & 3.8934e18   & 3.89343583e18  & {$<10^{-6}$} \\
  $\mathcal{C}=\rhocore/\rhof$
    & 5.562e24    & 5.562e24       & {$<10^{-6}$} \\
  \bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:convergence} summarises the Biot--Savart convergence
study for the ideal trefoil.

\begin{table}[h]
\centering
\caption{Convergence of the Biot--Savart coefficient $A_K$ and the
  no-contact closure ratio $a_0/\rc$ with integration resolution
  $N_{\mathrm{int}}$ for the ideal trefoil (Gilbert $3_1$).
  Results are independent of $N_{\mathrm{geom}}\geq4000$.
  Target: $A_K=1/(4\pi)=0.07957747$.}
\label{tab:convergence}
\begin{tabular}{rllll}
  \toprule
  $N_{\mathrm{int}}$ & $A_K$ (global) & $A_K/(1/4\pi)$ & $a_0/\rc$ (global) & $a_0/\rc$ (plateau-0.12) \\
  \midrule
   2\,000 & 0.07921 & 0.9954 & 0.9977 & 1.0427 \\
   4\,000 & 0.07851 & 0.9866 & 0.9933 & 0.9997 \\
   8\,000 & 0.07855 & 0.9871 & 0.9935 & 1.0041 \\
  16\,000 & 0.07965 & 1.0010 & 1.0005 & 0.9985 \\
  32\,000 & 0.07979 & 1.0026 & 1.0013 & 0.9989 \\
  \midrule
  $\infty$ (target) & $0.07958$ & $1.000$ & $1.000$ & $1.000$ \\
  \bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{clp{7.5cm}l}
  ...
  (iii)& Energy-scale gap
       & $\rhocore$ derived via Laplace pressure
         and $F_{\max}$ constraint,
         equation~\eqref{eq:rhocore_final};
         $\mathcal{C}$ in closed form,
         equation~\eqref{eq:C_closed}.
         Verified to $<10^{-6}$.
       & \textbf{Resolved} \\
  \bottomrule
\end{tabular}
...
All three open problems are resolved within the primitive framework
$(\rhof,\vswirl,\omC)$.

\paragraph{Route-2 numerical support.}
The robustness sweep over the ideal trefoil (Gilbert $3_1$) was
carried out over $N_{\mathrm{geom}}\in\{4000,8000,16000,32000\}$
geometry points and $N_{\mathrm{int}}\in\{2000,4000,8000,16000,32000\}$
Biot--Savart integration points, with four fit methods
(global, plateau-0.08, plateau-0.12, plateau-0.16) and four
contact strengths $\lambda_K\in\{0,10^{-3},3\times10^{-3},10^{-2}\}$.

Two key observations emerge from the full sweep.  First,
$A_K$ depends only on $N_{\mathrm{int}}$, not on $N_{\mathrm{geom}}$:
all four geometry resolutions give identical results for the same
integration resolution.  This confirms that the Fourier representation
of the Gilbert knot is fully converged at $N_{\mathrm{geom}}=4000$,
and that all remaining resolution dependence is in the Biot--Savart
numerical integration.

Second, all four fit methods at the highest resolution
$N_{\mathrm{int}}=32\,000$ bracket $1/(4\pi)$ from both sides:

\begin{equation}
  A_K^{\mathrm{plateau\text{-}0.16}}
  = 0.07929 < \tfrac{1}{4\pi} = 0.07958 <
  A_K^{\mathrm{global}}
  = 0.07979,
  \label{eq:AK_bracket}
\end{equation}
with all values within $\pm0.3\,\%$ of $1/(4\pi)$.
The corresponding no-contact closure ratios span
$0.998<a_0/\rc<1.001$, all within $0.2\,\%$ of unity.

The global fit at $N_{\mathrm{int}}=16\,000$ gives the result
closest to the target:
\begin{equation}
  A_K = 0.07965,
  \quad
  \frac{A_K}{1/(4\pi)} = 1.000966,
  \quad
  \frac{a_0}{\rc} = 1.000483,
  \label{eq:best_estimate}
\end{equation}
consistent with $A_K=1/(4\pi)$ and $a_0=\rc$ to within $0.05\,\%$.

The bracketing behaviour in~\eqref{eq:AK_bracket} is the strongest
form of numerical support available without an analytical proof:
it shows that $1/(4\pi)$ lies strictly inside the band of estimates
produced by all fit methods at the highest available resolution,
and that the band width ($\sim 0.5\,\%$) is already smaller than
the analytical uncertainty in $\Gamma_0$ from the Compton anchoring
($\sim1.6\,\%$).  We therefore report the no-contact result as:
\begin{equation}
  A_K = \frac{1}{4\pi}\bigl(1.000 \pm 0.003\bigr),
  \qquad
  \frac{a_0}{\rc} = 1.000 \pm 0.002,
  \label{eq:AK_result}
\end{equation}
where the uncertainty is the half-range of all fit methods at
$N_{\mathrm{int}}=32\,000$.

The key epistemic caveat is that $\omC=m_ec^2/\hbar$
introduces $m_e$, $c$, and $\hbar$ as inputs.  Within the current
SST programme these are treated as measured
constants~\cite{MohrTaylor2016}, consistent with the
Zero-Parameter Principle of the canon~\cite{IskandaraniCanon078}:
no new \emph{free} parameters are added, only the replacement of
two calibrated inputs ($\rc$ and $\rhocore$) by a single calibrated
input ($\omC$) that sits higher in the quantum-mechanical hierarchy.
Whether $m_e$ itself is derivable from $(\rhof,\vswirl,\omC)$ is a
question for a future stage of the programme.
